...
Launch-affecting names take the other enforcement path, because a static
reserved entry can pin a named value but cannot clear arbitrary names:
OmegaFlow's Envoy-requirement validation rejects a composed blueprint whose
environment contains `ENV`, `BASH_ENV`, or any name beginning with `LD_` or
`AWSH_`, so those names never exist in the container for either shell launch to
