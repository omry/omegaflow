...
- Approved environment direction with the Hydra blueprint and placement
  amendments approved after re-review; documentation corrections from the
  local design review are open in a successor PR. The trusted implementation boundary
...
application can replace them; its reserved entries — the Envoy command, the two
container ports, and the loopback host publication address — compose after it,
so an application cannot. Composing the reserved entries first would let an
application win under Hydra's ordinary behaviour and leave Reploy forwarding to
a port the Envoy is not listening on. Launch-affecting names take the other
...
before reaching the Envoy handshake. Only the host port is an ordinary typed
value: the loopback publication address is reserved with the other Envoy-fixed
entries, because publishing these unauthenticated single-connection listeners on
a non-loopback address would put them outside the lease-private boundary, where
an external connection could win the first-connection race. Every resolved value
remains visible in the final blueprint.
