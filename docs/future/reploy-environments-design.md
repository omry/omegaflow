...
existing local FIFO protocol remains only for the temporary native migration
path and is not extended for Reploy.
...
the Envoy telemetry channel. Playwright remains in the controller and can
capture the browser while the same workload shell stays alive.
...
directly without a second PTY or an `asciinema record` process. Reploy
separately injects its release-matched public client at session preparation
time; OmegaFlow must not package its own copy of `reploy-session-client`. The
bundled asciinema executable remains only for the temporary native recording
...
- Retain the exact private terminal-output byte log and write the asciicast and
  terminal timeline directly from Envoy terminal and telemetry events without
  introducing another PTY.
...
- Carry workload readiness and path/navigation intent through structured Envoy
  telemetry.
...
2. Exercise fragmented terminal and telemetry bytes, fragmented and invalid
   ...
   exact private raw-output retention, resize, Ctrl-C, command completion, cwd,
