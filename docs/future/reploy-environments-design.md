...
output-through drain barrier before execution. Authored controller input
remains supported, with its echo excluded
...
remains fails the session. Cancellation and planned finalization use the same
cleanup. This supervision is correctness evidence within the same-identity
...
2. **Terminal-only:** Exercise fragmented terminal and telemetry bytes,
   ...
   excluded by `echo`-marked spans, fail-closed cleanup after every operation
