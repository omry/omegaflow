...
container ports, and the clearing of bootstrap-shell startup and `LD_` loader
variables such as `ENV` and `BASH_ENV` — compose after it, so an application
cannot. Composing the reserved entries first would let an application win under
Hydra's ordinary behaviour, leaving Reploy forwarding to a port the Envoy is not
listening on or a startup file running in the bootstrap shell before any
OmegaFlow code does. The defaults are:
