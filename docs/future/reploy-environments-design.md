...
- Preserve expectation checks, replacement output, produced-output metadata,
  and separate failure diagnostics over the single ordered PTY stream.
- Reject recordings configured with `with_env`, application secrets, or other
  secret-dependent session operations with the targeted capability error before
  invoking Reploy. The first adapter does not receive secret values and cannot
  claim legacy secret-output checking until a separate secret-delegation design
  is approved.
...
Completion gate: the non-secret cases in the existing single-pane terminal
contract suite pass against the adapter, plus byte-fragmentation, forged-marker,
early-exit, and partial-output cases. Existing `with_env` and secret-dependent
cases instead verify the targeted pre-session capability error and that Reploy
was not invoked.
