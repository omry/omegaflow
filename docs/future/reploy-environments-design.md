...
- Preserve publication timing: stream only `realtime` `real` PTY output;
  buffer presentation-timed split streams and publish logical stdout then
  stderr only after completion, output-through, and the logical post-enter
  pause, without exposing command duration or raw arrival timing in the cast.
- Preserve the existing assertion input contract by retaining split-stream
  ...
  exact merged post-line-discipline PTY output as logical stdout with empty
  stderr, and evaluating against logical `stdout + stderr` rather than temporal
  presentation order. Never infer pre-line-discipline bytes from termios
  observations.
