...
`operation_started`, probes the plan-selected granted endpoint until its
configured readiness condition succeeds, performs the planned Playwright work,
and then applies the operation's compiled lifetime policy through normal Envoy
...
Output assertions therefore require exclusive observation: an empty shell job
table and fresh output-through drain barrier before execution, no controller
data input before successful completion, and no live job afterward.
Cancellation invalidates the assertions. `suppress`, `replace`, and checked
`real` operations always use this mode; a job they leave behind is terminated
and drained under the same private output policy before the operation fails,
and an undrainable job fails the session.

Unchecked `real` operations may preserve background jobs. Their ranges are
shared, may contain interleaved late output, and cannot satisfy operation-level
output assertions. A later exclusive operation is rejected until the jobs are
gone and a new output-through drain barrier prevents their remaining bytes
from entering its checked range. Produced-output metadata, diagnostics, and
private failure evidence retain shared ranges only with that limitation and
without publishing suppressed bytes.
...
- Carry exclusive-versus-shared observation through the execution boundary.
  Output assertions require exclusive observation, which rejects controller
  data input and starts only after an empty-job-table/output-drain barrier.
- Terminate, drain, and fail jobs left by `suppress`, `replace`, or checked
  `real` operations. Preserved jobs are allowed only for unchecked `real`
  operations, whose shared ranges cannot satisfy later assertions.
...
- For a long-running terminal operation, wait for `operation_started`, probe
  the plan-selected granted endpoint from the controller, run the planned
  browser actions, and apply the operation's compiled lifetime policy through
  normal Envoy cancellation and output-finalization.
...
2. Exercise fragmented terminal and telemetry bytes, fragmented and invalid
   ...
   exclusive checked ranges that cannot be satisfied by echoed input or earlier
   background jobs, fail-closed checked-operation job cleanup, resize, Ctrl-C,
   command completion, cwd, action gates, and ordered output ranges.
...
6. Verify that endpoint IDs resolve only through the trusted `opened` event and
   ...
   workload file, OSC marker, or browser-specific telemetry message.
