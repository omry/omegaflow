...
grace period to return. If it does not, the Envoy terminates and reaps the
...
meet. A workload whose commands need a loader variable sets it inside operation
source, where the persistent shell carries it to operation children as ordinary
shell state without governing any launch. The defaults are:
...
2. **Terminal-only:** Exercise fragmented terminal and telemetry bytes,
   ...
   recorded resize event, gate continuation waiting for its terminal-input
   watermark and taking fatal session teardown without a terminal operation
   result when that watermark times out, both serialized
   inspection-cancellation winners, typed workload-side file and produced-output
   inspection, action gates, and ordered output ranges.
