...
after an unready-to-ready transition observed since `operation_started`; a
terminal result observed first or at handoff, or an endpoint already serving
when the operation started, fails the capture. It then applies the operation's
...
container ports, and the clearing of bootstrap-shell startup variables such as
`ENV` and `BASH_ENV` — compose after it, so an application cannot. Composing the
reserved entries first would let an application win under Hydra's ordinary
behaviour, leaving Reploy forwarding to a port the Envoy is not listening on or
a startup file running in the bootstrap shell before any OmegaFlow code does.
The defaults are:
...
6. **Browser stack:** Verify that endpoint IDs resolve only through the trusted
   ...
   readiness success, with readiness requiring an unready-to-ready transition
   observed after operation start.
