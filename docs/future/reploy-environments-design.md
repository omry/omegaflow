...
`real` operations always use this mode.

Checked output uses a private logical assertion view, not the post-line-
discipline raw bytes. The Envoy records bounded slave output-termios segments;
the controller reverses supported lossless mappings such as standard `ONLCR`
before evaluating `output_contains` and `output_regex`. Unsupported, lossy, or
untracked transformations fail the checked operation. The raw terminal log and
...
2. Exercise fragmented terminal and telemetry bytes, fragmented and invalid
   ...
   processing, typed workload-side file and produced-output inspection, action
   gates, and ordered output ranges.
