...
produced-output metadata, diagnostics, and private failure evidence may inspect
the raw range without publishing it.

Because the shared PTY cannot attribute late bytes from a background
descendant, `suppress` and `replace` require an empty persistent-shell job table
at operation start and completion. A newly created live job is terminated and
drained under the same private output policy before the operation fails; an
undrainable job fails the session. Persistent background jobs remain supported
only for `real` output.
...
- Reject non-`real` execution while shell jobs are live, and terminate, drain,
  and fail any non-`real` operation that attempts to leave a job behind.
...
2. Exercise fragmented terminal and telemetry bytes, fragmented and invalid
   ...
   fail-closed non-`real` background-job cleanup, resize, Ctrl-C, command
   completion, cwd, action gates, and ordered output ranges.
