...
| Host OmegaFlow | Blueprint composition, deployment preparation, public host invocation, final host-result interpretation, retained-run inspection, and publication. |
| Controller OmegaFlow | Recording plan execution, command completion and cwd detection, action gates and markers, terminal-to-browser handoff, browser automation, redaction, media production, artifact finalization, and structured failure preservation. |
...
and timestamps, logs, the decoded Reploy session result, and OmegaFlow failure
metadata into that directory. It closes every file and completes all required
media processing before sending `complete`. When finalization fails, it still
...
Only declared artifacts cross from the private run into published output.
Publishing remains a host OmegaFlow operation after Reploy returns and
continues to use OmegaFlow's allowlist and integrity checks. Reploy's complete
host result is retained alongside the private run even when publication is not
attempted.
...
diagnostics rather than schema errors. It also preserves stderr from the
controller client and host Reploy separately from terminal content. Abrupt host
death may leave only retained partial artifacts plus a Reploy-private incident
receipt; OmegaFlow reports the absent host result without manufacturing
...
- Parse the exact host stdout object, preserve stderr, and keep publishing on
  the host after the controlled session returns.
...
14. Recordings use a controller output directory and finalize retained private
    artifacts before sending `complete`.
