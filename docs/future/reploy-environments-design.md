...
and races the plan-selected granted endpoint readiness probe against typed
operation completion, cancellation, or failure. It performs the planned
Playwright work only when readiness wins while the operation is still active and
the endpoint was observed unready before the operation's `execute` was sent; a
terminal result observed first or at handoff, or an endpoint already serving
before the operation existed, fails the capture. It then applies the operation's
compiled lifetime policy through normal Envoy cancellation and
...
checks, endpoint selection, and navigation intent remain in the controller. A
generic Envoy action gate is used only when a running shell operation
deliberately pauses for a controller action already present in the recording
plan. Envoy protocol v1 carries no browser-specific message or
...
6. **Browser stack:** Verify that endpoint IDs resolve only through the trusted
   ...
   readiness success, with readiness requiring the endpoint observed unready
