...
mount and the two exact endpoint IDs, bind addresses, and container ports —
compose after it, so an application cannot. Composing the reserved entries first
would let an application win under Hydra's ordinary behaviour and disconnect
the Envoy listen arguments from the coordinates Reploy reports. Launch-affecting
names take the other enforcement path, because a static reserved entry can pin
a named value but cannot clear arbitrary names: OmegaFlow's Envoy-requirement
validation rejects a composed blueprint whose environment contains any of
these exact names:

```text
AWSH_BASH BASH_COMPAT BASHOPTS BASH_ENV BASH_XTRACEFD CDPATH ENV
GLOBIGNORE INPUTRC POSIXLY_CORRECT PROMPT_COMMAND PS0 PS1 PS2 PS3 PS4
SHELLOPTS TMOUT
```

It also rejects every name beginning with `BASH_FUNC_`, `LD_`, or `AWSH_`.
This is the normative enumeration referenced by the other design documents,
so none of these names exists in the container for either shell launch to
meet. In particular, `INPUTRC` cannot supply a Readline macro that rewrites or
submits part of the Envoy bootstrap command before OmegaFlow code starts. A
workload whose commands need a loader variable sets it inside operation source,
where the persistent shell carries it to operation children as ordinary shell
state without governing any launch. The defaults are:
...
- private TCP bind defaults and fixed container ports for both endpoints; and
