...
Expectation checks, produced-output metadata, diagnostics, and private failure
evidence may consume the retained raw range regardless of presentation mode.
They never reclassify suppressed workload bytes as cast or timeline events.

The shared PTY cannot attribute bytes emitted after completion by a background
descendant. A non-`real` operation therefore starts only when the persistent
shell has no live jobs and must leave no live job behind. The Bash adapter
checks both boundaries. If the operation creates a background job that remains
live, the Envoy terminates that job, drains its remaining bytes under the same
private output policy, and fails the operation before accepting another
`execute`. Failure to terminate and drain it fails the session. Background-job
preservation remains supported only for `real` operations.
...
- Reject a non-`real` operation while shell jobs are live and fail closed after
  terminating and draining any background job it creates before releasing the
  operation's output range.
...
4. fail-closed rejection and cleanup of background jobs around non-`real`
   operations, with no late output escaping the originating policy;
