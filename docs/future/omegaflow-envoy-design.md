...
realtime runner, so the inverse-transformed PTY bytes are logical stdout and
logical stderr is empty. Stream identity is never guessed from merged PTY
bytes.

PTY-attached assertions do not compare patterns directly with the
post-line-discipline raw range. For an exclusive operation, the Envoy records a
bounded ledger of the slave output-termios state covering each byte segment.
The controller derives its logical stdout by reversing supported lossless
output transformations, including the standard `ONLCR` mapping from command
newline to terminal CRLF. Split-stream evidence bypasses the terminal line
discipline and needs no inverse transform. The raw log and published cast
remain byte-exact and are never normalized. A lossy or unsupported output
transform, or an output-termios change that the Envoy cannot associate with a
byte boundary, fails the checked PTY operation rather than changing assertion
semantics. Thus an existing newline-sensitive `output_contains` or
`output_regex` still sees the command's logical newline bytes.
...
- Preserve the current assertion input contract: split non-interactive output
  into bounded logical stdout/stderr evidence, treat merged interactive PTY
  output as logical stdout with empty stderr, and evaluate patterns against
  logical `stdout + stderr` rather than temporal presentation order.
...
6. newline-sensitive assertions over the logical pre-transformation view while
   ...
   with legacy `stdout + stderr` ordering preserved for split-stream operations
   and merged interactive PTY output treated as logical stdout;
