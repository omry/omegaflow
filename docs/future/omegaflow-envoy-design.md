...
the independent terminal connection. A resize accepted from
`operation_started` through the terminal event of a presentation-timed
operation belongs to that operation's authored span even when the writer sees
it before compiled publication starts. The controller buffers it until the
schedule is known, then uses `output_through` to place it after all authored
output derived from bytes through that frontier and before later authored
output. A frontier covering no authored byte places the resize at the pre-span
time. This prevents command wall time from entering the cast while preserving
the Envoy's output ordering. If a shell-ended drain wins a race with an
