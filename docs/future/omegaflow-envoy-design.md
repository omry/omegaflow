...
buffers it until the schedule is known, then uses `output_through` to place it
after all authored output derived from bytes through that frontier and before
later authored output. A frontier covering no authored byte places the resize
at the pre-span time. This prevents command wall time from entering the cast
