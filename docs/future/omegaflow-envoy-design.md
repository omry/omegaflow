...
clears the shell startup variables that would let application code execute
inside it — `ENV` and `BASH_ENV` above all, and every `LD_` loader variable,
which the dynamic loader consumes before `/bin/sh` reads anything — in the same
composed-after entry that reserves the Envoy command. The later controlled-Bash
filtering cannot reach this shell: it happens after the Envoy is already
running. Left in place, an application `ENV` file would be sourced before the
controller types anything and could consume the input it is about to type, alter
the shell's behaviour, or exit outright, and the Envoy would never start. It
keeps draining the Reploy PTY for bootstrap and Envoy diagnostics while treating
the Envoy terminal channel as the recording stream.
...
is still active, and only if the controller observed that same endpoint unready
— a refused or failing probe — before it sent the operation's `execute`. An
endpoint already serving before the operation exists is a stale listener and
fails the handoff closed. A success after that pre-start unready observation
proves a bind that happened within this operation's lifetime, and because the
observation precedes the operation rather than sampling for a failure after
`operation_started`, a service that binds faster than the first post-start probe
still passes. Any terminal operation result observed first, or at the success
handoff check, also fails the handoff; the controller never records browser
actions against a process that was already serving the endpoint before the
operation it credits.
