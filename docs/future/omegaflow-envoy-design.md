...
4. interactive input, Ctrl-C, resize, and `SIGWINCH` behavior;
5. curses applications and one nested interactive shell;
6. separation of terminal output from telemetry messages;
7. the same-identity threat boundary and non-inheritance of Envoy sockets and
   private descriptors;
8. ordered terminal drain and EOF during graceful shutdown;
9. useful partial diagnostics after Envoy, shell, channel, and controller
   failures; and
10. end-to-end Reploy termination, acknowledgement, retained output, and
    cleanup.
