...
starts with an empty persistent-shell job table and an output-through drain
barrier, accepts no controller data input before successful completion, and
must leave no live job behind. Cancellation invalidates its assertions instead
of evaluating partial input and output. Telemetry `continue` messages and
resize requests are not terminal data input.

`suppress` and `replace` always use exclusive observation. A checked `real`
operation uses it as well. If an exclusive operation creates a live background
job, the Envoy terminates that job, drains its remaining bytes under the same
private output policy, and fails the operation before accepting another
`execute`; failure to terminate and drain it fails the session. Unchecked
`real` operations may preserve background jobs, but their raw ranges are
explicitly shared and cannot satisfy operation-level output assertions. A
later exclusive operation is rejected until the job table is empty and the
Envoy has crossed a fresh output-through drain barrier, so late real output
cannot enter its checked range. Produced-output metadata, diagnostics, and
private failure evidence may retain shared ranges only with that provenance
limitation. None of these consumers reclassifies suppressed workload bytes as
cast or timeline events.
...
- Carry exclusive-versus-shared observation in the controller/Envoy/`awsh`
  ...
  reject controller data input there, and require an empty job table plus a
  fresh output-through drain barrier before its checked range starts.
- Make `suppress`, `replace`, and checked `real` operations fail closed after
  terminating and draining any background job they create. Permit preserved
  jobs only for unchecked `real` operations, whose shared ranges cannot satisfy
  later assertions.
...
4. exclusive checked ranges that reject echoed controller input and late output
   from earlier jobs, plus fail-closed cleanup of jobs created by `suppress`,
   `replace`, or checked `real` operations;
5. interactive input, Ctrl-C, resize, and `SIGWINCH` behavior;
6. curses applications and one nested interactive shell;
7. separation of terminal output from telemetry messages;
8. the same-identity threat boundary and non-inheritance of Envoy sockets and
   private descriptors;
9. ordered terminal drain and EOF during graceful shutdown;
10. useful partial diagnostics after Envoy, shell, channel, and controller
   failures; and
11. end-to-end Reploy termination, acknowledgement, retained output, and
    cleanup.
