...
preceding `resize_applied` normally. Arrival times drive only a live view. The
...
A controller deadline remains cancellable while finalization inspection is
running. Cancellation moves the controller from finalizing to cancelling and
uses the existing supervised-worker race: a worker result accepted first
commits planned finalization, while cancellation accepted first stops and reaps
the worker and returns cancellation without inspection results. Failure to reap
within five seconds remains fatal inspection-cancel-timeout.
