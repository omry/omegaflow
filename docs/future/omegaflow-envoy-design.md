...
6. interactive input, Ctrl-C, resize, and `SIGWINCH` behavior;
7. curses applications and one nested interactive shell;
8. separation of terminal output from telemetry messages;
9. the same-identity threat boundary and non-inheritance of Envoy sockets and
   private descriptors;
10. ordered terminal drain and EOF during graceful shutdown;
11. useful partial diagnostics after Envoy, shell, channel, and controller
   failures; and
12. end-to-end Reploy termination, acknowledgement, retained output, and
    cleanup.
