...
applied resize events from telemetry. Realtime publication uses those arrival
times; presentation-timed publication uses its compiled schedule instead. The
...
- `realtime` plus `real` incrementally publishes decoded PTY events timestamped
...
- Freeze direct asciicast encoding, controller timestamping, resize-event
  ...
  handling, realtime incremental publication, presentation-timed buffering and
  retiming, and the rule that Envoy capture introduces no controller-side PTY.
...
- Publish `realtime` `real` PTY output incrementally, but buffer
  presentation-timed split streams and publish logical stdout then stderr only
  after completion, output-through, and the logical post-enter pause, without
  leaking command duration or raw arrival timing into the cast.
