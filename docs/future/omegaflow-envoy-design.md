...
- Approved direction through the protocol and Awsh Bash prototype. The amended
  protocol and rebuilt delivery plan, including the folded-in corrections of
  the local design review, await a fresh remote review cycle. Production
...
controlled-Bash filtering happens after the Envoy is already running. Left in
place, an application `ENV` file would be sourced before the controller types
anything and could consume the input it is about to type, alter the shell's
behaviour, or exit outright, and the Envoy would never start. It keeps draining
the Reploy PTY for bootstrap and Envoy diagnostics while treating the Envoy
terminal channel as the recording stream.
