...
Bash and its children use the single Envoy-owned slave PTY as their controlling
terminal. The Envoy occupies the role that a recorder such as asciinema
normally occupies at the PTY master, with the additional telemetry channel
required by OmegaFlow.
...
or another PTY between the controller and the Envoy. The existing native path
may continue using the bundled asciinema executable during migration, but it is
not part of Envoy capture. The controller also retains a private raw
terminal-output byte log as the lossless source for workload-output byte ranges
and diagnostics. Synthesized prompt and command events carry distinct timeline
provenance and never enter that raw log or its byte offsets. Because asciicast
events are UTF-8 JSON text, the frozen protocol specifies the incremental
...
result through an internal control path that is distinct from the PTY. Visible
stdout and stderr continue through the PTY while an operation is running.
...
- the Envoy gives the execution side the PTY slave as its controlling terminal
  and standard input, output, and error;
...
controller input in one direction and workload PTY output in the other. It does
not carry JSON, lifecycle state, or presentation markers.
...
### Browser ownership and action gates

Browser automation belongs entirely to controller OmegaFlow. The compiled
recording plan selects each browser action and application endpoint ID. The
controller resolves only that ID through trusted `opened.endpoints`, constructs
the target from the granted scheme, host, and port plus controller-owned path
intent, and drives Playwright. Workload output and Envoy telemetry cannot replace
the granted host, port, or planned navigation target.

Envoy protocol v1 has no browser-specific message and does not carry
workload-originated navigation intent. An `awsh` action gate is only a generic,
planned pause inside one running Bash operation. When the operation reaches a
named gate, controller OmegaFlow may perform the browser or controller action
already associated with that gate in the recording plan and then send
`continue`. Ordinary terminal-then-browser sequencing uses operation completion
and needs no shell gate. Dynamic workload-selected browser navigation is
deferred.
...
remains available only during migration; these slices must not establish a
second permanent capture model.
...
- Compare representative recordings through the native and Envoy paths and
  produce cutover evidence. Do not remove the native path in this slice.
...
path is ready for an explicit migration decision. Native-path removal is a
...
3. interactive input, Ctrl-C, resize, and `SIGWINCH` behavior;
4. curses applications and one nested interactive shell;
5. separation of terminal output from telemetry messages;
6. documentation that same-identity workload processes can deliberately
...
7. ordered terminal drain and EOF during graceful shutdown;
8. useful partial diagnostics after Envoy, shell, channel, and controller
   failures; and
9. end-to-end Reploy termination, acknowledgement, retained output, and cleanup.

The implementation must also record the deferred Reploy capability needed for a
future privileged Envoy/non-root workload split. Only after these checks pass,
the frozen Hydra blueprint contract passes conformance, and the explicit
cutover is approved should OmegaFlow replace the local FIFO-backed
`PersistentTerminalRunner` path.
...
12. OmegaFlow packages only its own runtime. Reploy remains a separately
    released Python dependency, and native-path removal requires a distinct
    cutover approval.
