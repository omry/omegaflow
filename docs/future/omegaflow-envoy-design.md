...
the independent terminal connection. Arrival times drive only a live view. The
...
`execute`. Such an operation has an output-through drain barrier. It accepts
authored controller input, and the echo of that input
...
cleanup. Inability to prove the operation process set empty fails the session.
...
5. runs the already-planned browser actions while the operation remains gated;
   and
6. continues, ends, or retains the operation according to its compiled lifetime
   policy, using normal typed continuation, cancellation, and finalization
   rules.
...
4. exclusive checked ranges that accept interactive input and exclude its kernel
   echo through `echo`-marked spans, plus fail-closed cleanup after every
   operation, including ordinary background jobs, `disown`, `nohup`, `setsid`,
   and rapid double-fork daemonization, while a setup-launched service outside
   the controlled process tree remains unaffected;
