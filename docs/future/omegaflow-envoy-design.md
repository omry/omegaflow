...
workload output. Cancellation invalidates assertions instead of evaluating
partial input and output. Telemetry `continue` messages and resize requests are
not terminal data input.

Output assertions do not compare patterns directly with the post-line-discipline
raw range. For an exclusive operation, the Envoy records a bounded ledger of
the slave output-termios state covering each byte segment. The controller
derives a private logical assertion view by reversing supported lossless output
transformations, including the standard `ONLCR` mapping from command newline to
terminal CRLF. The raw log and published cast remain byte-exact and are never
normalized. A lossy or unsupported output transform, or an output-termios
change that the Envoy cannot associate with a byte boundary, fails the checked
operation rather than changing assertion semantics. Thus an existing
newline-sensitive `output_contains` or `output_regex` still sees the command's
logical newline bytes.
...
- bounded output-termios ledger entries used to derive the private logical
  assertion view;
...
- Derive assertions from a private logical byte view by reversing the Envoy's
  supported output-termios ledger; keep the raw log and cast unchanged and fail
  checked operations on lossy or untracked transformations.
...
6. newline-sensitive assertions over the logical pre-transformation view while
   raw PTY and cast bytes retain their exact CRLF and other terminal behavior;
...
12. useful partial diagnostics after Envoy, shell, channel, and controller
   failures; and
13. end-to-end Reploy termination, acknowledgement, retained output, and
    cleanup.
