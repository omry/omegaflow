...
publication uses its compiled schedule instead. A presentation-timed operation
also adds the duration it discarded to a running session offset that every later
event is published against, so compressing one command shortens the recording
rather than pushing a command-length gap in front of the next one. Transport
...
invalidate assertions instead of evaluating partial input and output. Planned
