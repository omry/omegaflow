...
Controller OmegaFlow writes the asciicast and action timeline directly from
the ordered bytes received on the terminal channel, timestamping output at the
controller boundary and recording applied resize events from telemetry. It does
not place an `asciinema record` process or another PTY between the controller
and the Envoy. The existing native path may continue using the bundled
asciinema executable during migration, but it is not part of Envoy capture.
The controller also retains a private raw terminal-output byte log as the
lossless source for byte ranges and diagnostics. Because asciicast events are
UTF-8 JSON text, slice 1 must freeze the incremental decoding and invalid-byte
policy used for the presentation cast; that conversion never weakens the raw
transport or byte-log contract.
...
- Freeze direct asciicast encoding, controller timestamping, resize-event
  recording, raw terminal-byte retention, incremental UTF-8 decoding and
  invalid-byte handling, and the rule that Envoy capture introduces no
  controller-side PTY.
...
- Retain the exact private terminal-output byte log and write asciicast and
  timeline artifacts directly from terminal and telemetry events without
  starting `asciinema record` or another controller-side PTY.
...
- Preserve terminal/browser joins, readiness gates, screenshots, pointer state,
  and presentation ordering through structured telemetry.
...
### 8. Package, document, and migrate
...
- Compare representative recordings through the native and Envoy paths during
  migration, then remove the redundant native dependency model on a separately
  approved cutover.

Completion gate: release packaging and documented examples pass from a clean
host, and the standard Reploy path is ready for an explicit migration decision.
...
2. continuous byte-for-byte terminal transport and private raw-output
   retention while a command runs;
...
6. the same-identity threat boundary and non-inheritance of Envoy sockets and
   private descriptors;
...
and the remaining slice-1 choices are resolved should OmegaFlow replace the
local FIFO-backed `PersistentTerminalRunner` path.
...
8. Controller OmegaFlow writes Envoy captures directly in asciicast format; the
   Envoy path also retains the exact private raw output and does not add an
   `asciinema record` process or controller-side PTY.
