...
def test_session_executes_when_request_pipe_uses_fixed_fd() -> None:
    pid = os.fork()
    if pid == 0:
        try:
            os.closerange(3, 256)
            held_descriptors = [
                os.open(os.devnull, os.O_RDONLY) for _ in range(17)
            ]
            assert held_descriptors == list(range(3, 20))
            session = AwshSession()
            assert session.request_write == 21
            events: list[dict[str, str]] = []
            terminal = bytearray()
            _collect_until(session, events, terminal, "ready")
            session.send("shutdown")
