...
def test_current_home_completion_is_disabled_without_shell_home_telemetry(
    tmp_path, monkeypatch
) -> None:
    controller_home = tmp_path / "controller-home"
    controller_home.mkdir()
    (controller_home / "wrong-file").write_text("", encoding="utf-8")
    monkeypatch.setenv("HOME", str(controller_home))

    assert _complete_path("~/wrong", 0, str(tmp_path)) is None
...
def test_prompt_input_detects_driver_exit_and_restores_alarm(monkeypatch) -> None:
    handlers = []
    timers = []
    ...
    monkeypatch.setattr(signal, "getsignal", lambda _signal: "old-handler")
    monkeypatch.setattr(signal, "getitimer", lambda _timer: (0.0, 0.0))
    monkeypatch.setattr(
        signal, "signal", lambda _signal, handler: handlers.append(handler)
    )
    monkeypatch.setattr(
        signal,
        "setitimer",
        lambda timer, first, interval=0.0: timers.append(
            (timer, first, interval)
        ),
    )

    def interrupt_input(_prompt: str) -> str:
        handlers[-1](signal.SIGALRM, None)
        raise AssertionError("driver exit did not interrupt input")

    monkeypatch.setattr(builtins, "input", interrupt_input)

    try:
        _input_while_session_running(Session(), "prompt")
    except ProtocolError as exc:
        assert "while waiting for input" in str(exc)
    else:
        raise AssertionError("driver exit was accepted while waiting for input")

    assert timers[-1] == (signal.ITIMER_REAL, 0.0, 0.0)
    assert handlers[-1] == "old-handler"


def test_prompt_input_returns_source_and_restores_previous_timer(
    monkeypatch,
) -> None:
    handlers = []
    timers = []
    ...
    monkeypatch.setattr(signal, "getsignal", lambda _signal: "old-handler")
    monkeypatch.setattr(signal, "getitimer", lambda _timer: (3.0, 4.0))
    monkeypatch.setattr(
        signal, "signal", lambda _signal, handler: handlers.append(handler)
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(builtins, "input", lambda prompt: f"source:{prompt}")

    assert _input_while_session_running(object(), "prompt") == "source:prompt"
    assert timers[-2:] == [
        (signal.ITIMER_REAL, 0.0, 0.0),
        (signal.ITIMER_REAL, 3.0, 4.0),
    ]
    assert handlers[-1] == "old-handler"


def test_prompt_alarm_interrupts_a_blocking_input(monkeypatch) -> None:
    class Session:
        def poll(self) -> int:
            return 9
    ...
    try:
        ...
        raise AssertionError("blocking input was not interrupted")
