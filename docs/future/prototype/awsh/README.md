...
`awsh-v1` tag while carrying fewer fields than the private protocol in
[Envoy Protocol v1](../../../design/envoy-protocol-v1.md), whose `execute` also
carries the execution shape, observation, and inspection plan, whose `completed`
also carries resolved inspection results, and which adds `started_ack` and
`finalize` messages this prototype does not implement. Delivery slice B2 aligns
the driver with the frozen grammar; until then, do not read a prototype frame as
evidence of v1 conformance.
