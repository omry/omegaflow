...
def _remap_descriptors_for_exec(mappings: tuple[tuple[int, int], ...]) -> None:
    targets = {target for _, target in mappings}
    minimum_temporary_fd = max(targets) + 1
    original_sources = {source for source, _ in mappings}
    temporary_sources: set[int] = set()
    prepared = []

    for source, target in mappings:
        if source in targets and source != target:
            source = fcntl.fcntl(
                source, fcntl.F_DUPFD_CLOEXEC, minimum_temporary_fd
            )
    ...
    for descriptor in original_sources | temporary_sources:
        if descriptor not in targets:
            os.close(descriptor)
    ...
    def __init__(self) -> None:
        ...
        if pid == 0:
            try:
                ...
                _remap_descriptors_for_exec(
                    ((request_read, REQUEST_FD), (result_write, RESULT_FD))
                )
                os.execv(
                    AWSH,
                    [
...
def _quote_completion(candidate: str, preserve_home_prefix: bool) -> str:
    if preserve_home_prefix:
        home_prefix, separator, remainder = candidate.partition(os.sep)
        if separator:
            return home_prefix + separator + (
                shlex.quote(remainder) if remainder else ""
            )
        return home_prefix
    return shlex.quote(candidate)


def _decode_completion(text: str) -> str:
    try:
        words = shlex.split(text)
    except ValueError:
        for quote in ("'", '"'):
            try:
                words = shlex.split(text + quote)
            except ValueError:
                continue
            if len(words) == 1:
                return words[0]
...
def _complete_path(text: str, state: int, cwd: str) -> str | None:
    decoded = _decode_completion(text)
    if decoded == "~" or decoded.startswith("~" + os.sep):
        return None
    ...
    matches = sorted(glob.glob(glob.escape(pattern) + "*"))
    candidates = []
    for match in matches:
        home_prefix = decoded.partition(os.sep)[0]
        expanded_home = os.path.expanduser(home_prefix)
        resolved_home = decoded.startswith("~") and os.path.isabs(expanded_home)
        if resolved_home:
            candidate = home_prefix + match[len(expanded_home) :]
        elif relative:
            candidate = os.path.relpath(match, cwd)
            if decoded.startswith("." + os.sep) and not candidate.startswith("."):
                candidate = "." + os.sep + candidate
        else:
            candidate = match
        if os.path.isdir(match):
            candidate += os.sep
        candidates.append(_quote_completion(candidate, resolved_home))
...
def _input_while_session_running(session: AwshSession, prompt: str) -> str:
    previous_handler = signal.getsignal(signal.SIGALRM)
    previous_timer = signal.getitimer(signal.ITIMER_REAL)

    def check_driver(_signal_number: int, _frame: object) -> None:
        status = session.poll()
        if status is not None:
            raise ProtocolError(
                f"awsh exited with status {status} while waiting for input"
            )

    signal.signal(signal.SIGALRM, check_driver)
    signal.setitimer(
        signal.ITIMER_REAL, PROMPT_POLL_SECONDS, PROMPT_POLL_SECONDS
    )
    try:
        return input(prompt)
    finally:
...
def _relay_operation(
    ...
    try:
        ...
        while session.result_open and not closed:
            exit_status = session.poll()
            if exit_status is not None and not completed:
                raise ProtocolError(
                    f"awsh exited with status {exit_status} before "
...
def run_controller(event_path: Path, done_path: Path) -> int:
    ...
    try:
        with event_path.open("ab", buffering=0) as event_stream:
            ...
            while session.result_open:
                try:
                    source = _input_while_session_running(session, _prompt(cwd))
