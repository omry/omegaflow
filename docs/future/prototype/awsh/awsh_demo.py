...
def _complete_path(text: str, state: int) -> str | None:
    ...
    candidates = [match + (os.sep if os.path.isdir(match) else "") for match in matches]
    if text.startswith("~"):
        home = str(Path.home())
        candidates = ["~" + candidate[len(home) :] for candidate in candidates]
...
def _relay_operation(
    ...
    try:
        ...
        while session.result_open and not closed:
            descriptors = [*session.readable_fds, input_fd]
            readable, _, _ = select.select(descriptors, [], [], 0.05)
            session.resize(input_fd)
