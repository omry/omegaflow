...
awsh_control_broker() {
  ...
  while true; do
    command=''
    if ! IFS= read -r -d '' command; then
      return 0
    fi
    case "$command" in
      read)
        field=''
        if IFS= read -r -d '' -u "$request_fd" field; then
      ...
      emit)
        count=''
        if ! IFS= read -r -d '' count || [[ ! $count =~ ^[1-9][0-9]*$ ]] || (( count > 16 )); then
...
readonly -f awsh_control_broker awsh_usage
...
awsh_read_field() {
  ...
  status=''
  value=''
  if ! awsh_control_read_field status || ! awsh_control_read_field value; then
    exit "$AWSH_IO_ERROR_STATUS"
  fi
