...
awsh_run_operation() {
  ...
  set -- "$1" "$(builtin local -p | /usr/bin/sed '/^declare [^ ]* \(awsh_\|AWSH_\)/d')"
  exec 22<&-
  if [[ -n $2 ]]; then
...
while true; do
  awsh_process_request
  # Explicit `declare -g` bypasses the local-declaration promotion filter and a
  # readonly global cannot be repaired. Fail closed after the operation's
  # completion, before consuming another request under poisoned parser state.
  if builtin declare -p awsh_request_schema >/dev/null 2>&1 ||
    builtin declare -p awsh_request_kind >/dev/null 2>&1 ||
    builtin declare -p awsh_operation_id >/dev/null 2>&1 ||
    builtin declare -p awsh_operation >/dev/null 2>&1
  then
    awsh_protocol_error reserved_namespace 'workload created global state in the driver-owned parser namespace'
  fi
done
