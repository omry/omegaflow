...
active or retained split stdout/stderr source before `TIOCSWINSZ`, and make the
...
in B4 conformance; add the active and retained split-stream equivalents with B5.
For exclusive starts, wait for prior writers to exit
and reap, observe EOF on retired split pipes, drain preceding pipe and PTY bytes,
and emit their covering mark before opening the new output range.
...
exclusive ranges for checked, suppressed, and replaced operations. Do not
invent a per-operation numeric descendant-admission guarantee: inability to
retain complete supervision is a fatal session failure. Any future deterministic
