...
cancellation, planned finalization, the five-second monotonic cleanup deadline,
a cancellation racing a command that ends the persistent shell, and a
setup-launched service outside the controlled tree remaining unaffected. Prove
that deadline expiry produces no terminal operation result and hands final
environment termination to Reploy, and that the shell-ended result wins the
cancellation race without losing its reaped status. Do not add a controller
process-lifetime option or a per-operation numeric descendant-admission
guarantee. V1 does not preserve processes across operations; session-lifetime
