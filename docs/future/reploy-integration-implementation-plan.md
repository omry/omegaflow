...
zero-duration schedules. Treat every resize accepted during a
presentation-timed operation as part of that authored span even before schedule
commitment begins; buffer it until the schedule is known and use its
`output_through` to place it after authored output through that frontier and
before later authored output. Cover a long-running operation whose resize
arrives before any authored event is committed. Carry the cumulative
