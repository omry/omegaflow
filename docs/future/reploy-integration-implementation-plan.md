...
- Trusted implementation boundary: `301ed74b6ee0` (PR 8).
- Updated: 2026-08-17.
...
The approved implementation contains the Envoy protocol models and fixtures and
the Awsh Bash prototype. It does not contain an approved production Envoy or an
approved Reploy integration.
...
- For A1 only, removed obsolete plan text does not count toward that limit. A1
  remains one coherent contract-reconciliation PR and must stay below 1,200
  added or replaced handwritten lines.
...
and failure codes. Freeze path and digest compatibility with the native runner,
including undefined variables, `~user`, symlinks, and nested special entries.
...
Implement listeners, handshake, one PTY, persistent Awsh/Bash startup, shared
PTY execution, exact byte relay, and orderly shutdown.

**B4. Operation boundaries and controls**

Implement output barriers, completion, input, resize, cancellation, action
gates, planned finalization, and final drain.
...
**B6. Exclusive observation**

Implement writer census, echo checks, descendant cleanup, and fail-closed
exclusive ranges for checked, suppressed, and replaced operations.

**B7. Workload inspection**

Resolve configured paths in persistent Bash state, perform bounded workload
existence/type/hash inspection in Envoy only after exclusive descendant cleanup
and output drain, and return private typed results without controller filesystem
access or probe commands. Before hashing, terminate and reap every tracked
operation-created descendant, not only terminal writers. Cover mutation races,
