...
- `PROMPT_COMMAND` and prompt-readiness hooks require default `SIGCHLD` and Bash
  `SIGWINCH`, report
...
ignores them, requires `xtrace` disabled and `DEBUG` and `RETURN` traps unset,
and restores the exact signal-trap state after the helper closes. `xtrace` is
reserved disabled and `DEBUG`/`RETURN` are reserved unset because tracing can
act before an adapter function can suppress its own invocation. `SIGCHLD` and
Bash's `SIGWINCH` trap are reserved at their default dispositions. The option
and four traps are checked
without spawning a process before every normal helper; an initial
...
`EXIT`, `ERR`, and ordinary signal traps other than `SIGCHLD` and `SIGWINCH`
remain persistent workload state. A trusted operation may use `xtrace`, a Bash
`SIGWINCH` trap, `DEBUG`, or `RETURN` only transiently and must restore the
required state before a gate or source return. If it
does not, the transition is unsupported; `xtrace`, including a
workload-controlled `PS4`, or a DEBUG trap may act on the first boundary command
before the adapter can fail closed, so no successful completion or preservation
guarantee is made for that path.
...
helper, and persistent Bash's `SIGWINCH` trap must remain default; `DEBUG` and
`RETURN` traps must be unset and `xtrace` must be disabled
at every adapter boundary. Trusted source may use those tracing facilities only
transiently and must remove them before a gate or return. A violation is
unsupported state and may intercept the boundary before the adapter can report
it; supported adapter activity never emits workload tracing or evaluates a
workload-controlled `PS4`. Readline's `enable-bracketed-paste` value, the exact
...
slave. Persistent Bash reserves `SIGWINCH` default while foreground applications
retain normal kernel-generated `SIGWINCH` behavior.
...
1. the complete Bash-state matrix across PTY and split operations, including
   ...
   default `SIGCHLD` and Bash `SIGWINCH`, unset boundary `DEBUG`/`RETURN` traps,
   and disabled
   boundary `xtrace`, with reserved-state corruption failing before another
