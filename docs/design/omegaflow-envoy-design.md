...
launch is killed and reaped and never reaches
...
- `PROMPT_COMMAND` and prompt-readiness hooks report status, physical and
  validated logical cwd, exported environment, and `histexpand`, temporarily
  disable top-level history expansion, and return to input readiness;
...
order, records the initial shell state plus the complete pre-Readline termios
state, and does not emit private `ready` until the prompt path has disabled
history expansion, both helpers have closed, and the Bash parentage, session,
and foreground-group checks pass. The existing startup deadline owns rcfile
execution and this handshake; terminal output is never substituted for
readiness.
...
bracketed-paste terminator bytes in any Bash lexical context, Awsh temporarily
makes the active Readline termios input path byte-transparent and verifies the
result. Envoy injects exactly bracketed-paste begin, one conditional frame with
two byte-identical authored brace branches, bracketed-paste end, and `LF`; the
validated `C-J` binding accepts that line.
Readline must restore the complete pre-Readline termios snapshot before Awsh
accepts `start`. Bash parses the frame with history expansion temporarily
...
state. Readline's `enable-bracketed-paste` value, the exact
...
1. the complete Bash-state matrix across PTY and split operations, including
   ...
   Readline bindings, and history state, with reserved-state corruption failing
   before another submission and independent marker/termios backstops,
