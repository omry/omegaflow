...
Envoy has closed and quiesced the operation input gate, Awsh has flushed the
slave input queue while the matching `prompt_ready` helper remains blocked, that
helper has exited, and the observed Readline-entry transition proves Bash is
input-ready with no prompt helper left for cleanup.
...
descriptor. The Envoy first performs the mandatory operation cleanup, proves
that only external Awsh and persistent Bash remain, and drains output through
the closing operation offset. It then
...
events. Its private post-source `input_close` result and Envoy's matching
`input_closed` request establish the completion-input boundary without adding a
controller message: Envoy first closes and quiesces its writer, then Awsh flushes
the retained slave before releasing Bash into Readline. Any frame mismatch,
flush failure, or deadline expiry fails the session rather than risking an
unframed idle command.
...
2. continuous byte-for-byte terminal transport and private raw-output
   ...
   and flushes an unread multi-byte suffix before Bash re-enters Readline;
