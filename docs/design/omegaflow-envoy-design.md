...
distinct process group, makes that group foreground, installs the slave as the
child's fd 0/1/2, and releases the child to exec only after those steps succeed.
Awsh validates Bash direct parentage, `tcgetsid(slave) == awsh_pid`, and
`tcgetpgrp(slave) == bash_pid` before private readiness. It keeps `SIGTTOU`
