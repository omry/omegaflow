...
sensitive traps are unset. Readonly `trap` and `enable` mediation functions
reserve those traps and the builtins required by adapter status propagation and
the post-`PS0` release signal. Source may change non-reserved controls within
...
the cooperative source contract. Ordinary non-reserved shell state remains
