...
the matching `prompt_ready` helper has exited and the observed Readline-entry
transition proves Bash is input-ready with no prompt helper left for cleanup.
...
7. controlling-terminal/session/foreground-group setup, interactive input,
   ...
   `operation_gate_interrupted`, and the private resize handshake serializing
