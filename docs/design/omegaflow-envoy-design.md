...
also replaces incoming `TERM`, `TERMINFO`, and `TERMINFO_DIRS` with the
manifest-validated launch values from the environment contract:
`TERM=xterm-256color`, with both terminal-database variables naming the
read-only `/omegaflow-runtime/share/terminfo` database. Before starting Bash,
the Envoy verifies that the trusted entry is readable and fails startup if it
is not; terminal lookup cannot fall through to an application-controlled
database under `HOME` or to another search directory.

Application environment required by planned operations is delegated
explicitly, then the Envoy installs the trusted terminal values and an empty
`HISTFILE` as the final launch values so neither an application path nor a
default under `HOME` is opened before Bash accepts driver input. Operations
may deliberately change terminal lookup only after the controlled Bash has
started. The frozen protocol specifies the exact filtering and delegation
contract.
...
`BASH_ENV`, `HISTFILE`, `INPUTRC`, `TERM`, `TERMINFO`, and `TERMINFO_DIRS`,
every `BASH_FUNC_`, `LD_`, and `AWSH_` name, and interactive Bash controls such
as `PROMPT_COMMAND` and `TMOUT`. OmegaFlow's Envoy-requirement validation
rejects a selected application blueprint whose environment contains any of
them. Reserved composition then adds an empty `HISTFILE`, the fixed
`xterm-256color` terminal type, and the manifest-validated read-only terminfo
path; final validation requires those exact values and fails preparation if
the trusted terminal entry is unavailable.
...
8. curses applications and one nested interactive shell, with hostile
   application `TERMINFO`, `TERMINFO_DIRS`, and `$HOME/.terminfo` inputs unable
   to affect either shell launch and with a missing or mismatched trusted
   terminal entry failing before controlled Bash starts;
