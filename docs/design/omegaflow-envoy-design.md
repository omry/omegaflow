...
launches `/bin/bash --noprofile --norc -i` as a direct child on the single PTY
slave and installs a small immutable Bash bootstrap:
...
syntax preflight, Awsh frames one brace group and Envoy injects it as one
...
and the complete regular-file-only trusted locale tree against the mounted
...
sequenceDiagram
    ...
    A->>B: start Bash as direct child
