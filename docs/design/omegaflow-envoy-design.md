...
- `PS0` creates the redirection-only frame-entry marker after Bash parses the
  complete submitted group; Awsh validates it before accepting the helper's
  `start`, then the helper blocks immediately before that source unit begins and
  lets Awsh report `started` before Envoy releases Bash;
...
- the marker proves that Bash parsed the complete submitted brace group before
  Awsh accepts `start`; its creation precedes the blocking `PS0` helper packet.
...
state. Readline's `enable-bracketed-paste` value and the exact
bracketed-paste-begin sequence in both supported line-entry keymaps are also
reserved. The immutable prompt-state path validates those bindings at startup
and every reached prompt boundary, before readiness. An initial mismatch fails
launch and a later mutation is fatal `adapter-state`; the adapter does not
silently reset it. The independent frame-entry marker remains a fail-closed
`adapter-framing` backstop. Thus ordinary persistent key macros cannot rewrite
authored bytes before Bash parses the submitted paste unit.
...
1. the complete Bash-state matrix across PTY and split operations, including
   ...
   reserved hook or bracketed-paste framing corruption failing before another
   submission, with an independent frame-entry backstop,
