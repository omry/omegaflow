...
- Owner-approved direction through the external Awsh-supervisor feasibility
  ...
  when one is present. Current-revision approval and required checks remain VCS
  and CI evidence rather than claims in this document. Production Envoy, runtime, controller,
...
ignored for its own terminal-control ioctls. After the child becomes foreground,
its async-signal-safe pre-exec path clears the inherited signal mask and resets
every catchable real signal in the generated Bash-build inventory to default,
including `SIGINT` and the job-control signals; the supervisor's own disposition
is unaffected. Awsh retains one close-on-exec, control-only slave descriptor
until Bash is reaped, never reads or writes
...
for only the injected source echo and Readline redraw. The sole `PS0` helper
reports `start_prepared` while blocked; after Envoy closes the discard interval
and sends `start_release`, Awsh stops Bash at the outer source barrier and
reports `started`. None of the discarded bytes enters the raw log, socket,
marks, or offsets. Envoy then commits `output_start` and releases Bash only
after the private start acknowledgement.
...
split stdout/stderr pipe immediately before Awsh applies it at Envoy's request.
The controller
...
second signal and does not reset the timer: post-source `input_close` takes
cleanup followed by the Awsh result and `operation_cancelled`, while expiry takes `cancel-timeout` with
...
persistent Bash's resulting cwd and exported environment. It captures that
candidate completion at `prompt_state` but withholds the private result until
Envoy has closed and quiesced the operation input gate, pinned the matching
blocked `prompt_ready` helper by PID/start-time/pidfd identity, terminated and
reaped every other operation descendant, and drained every operation output
source. Awsh then flushes the slave input queue, releases the helper, and
observes the Readline-entry transition proving Bash is input-ready. A final
census proves that the exempt helper is gone and only Awsh and Bash remain.
The immutable primary-prompt hook produces no display bytes, so this wait does
not move a shell-rendered prompt into the operation's raw-output range; the
visible following prompt remains controller-synthesized presentation.
It then sends the resolved inspection plan to the Envoy over its private
descriptor. The Envoy has already completed mandatory operation cleanup and
drained output through the closing operation offset while Bash was blocked; it
revalidates the clean boundary and then
...
serializes the resize ioctl with its prompt and submission termios work,
...
execution. At ordinary return Awsh keeps the post-source prompt helper blocked
while it asks Envoy to close the operation input gate. Envoy serializes that
closure with its sole writer, discards later terminal bytes, and acknowledges
only after the writer is quiescent; Awsh then flushes the slave input queue
before Bash can re-enter Readline. PTY operations keep all three standard
streams on the slave.
...
- `PS0` creates the redirection-only frame-entry marker after Bash parses the
  complete submitted one-top-level-command frame; Awsh validates it before
  accepting the helper's `start`, then the helper blocks immediately before
  that source unit begins while Awsh reports `start_prepared`; after Envoy's
  `start_release`, Bash leaves the helper critical section and the outer
  conditional reaches its command-free retained-sentinel/FIFO redirection
  barrier, where Awsh stops Bash and reports `started` before Envoy publishes
  and releases the operation;
- `PROMPT_COMMAND` and prompt-readiness hooks require the complete generated
  real-signal trap inventory at Bash default, `ERR`/`DEBUG`/`RETURN` unset, and
  `xtrace` disabled, then report
...
closure are never substituted for actual Readline readiness. This startup path
has no operation input and therefore performs no completion-input closure or
flush.

Each immutable hook surrounds its external helper with a signal-safe adapter
critical section. Before spawning the helper, Bash requires `xtrace` disabled,
`ERR`/`DEBUG`/`RETURN` unset, and every catchable real-signal trap in the
generated Bash-build inventory at default. It then temporarily ignores
`SIGINT`/`SIGQUIT`/`SIGTSTP` and restores all three default dispositions after
the helper closes. The shell option and traps are checked through pre-created
private files, without command substitution or another child, before every normal
helper. Startup also compares Bash's exact canonical `trap -l` output with the
build-table entry from which that inventory was generated. An initial mismatch
or signal-inventory drift fails launch, a gate
returns 125 without spawning, and a reached prompt boundary is fatal
`adapter-state`. These reservations are necessary because tracing or any
pending signal trap can run before the adapter function can suppress its own
invocation and invalidate captured prompt state. Non-gate helpers
inherit and verify all three ignored dispositions. The gate helper retains and
verifies ignored `SIGQUIT`/`SIGTSTP`, replaces only `SIGINT` with its handler
before reporting readiness, and translates terminal Ctrl-C into its one
optional interrupt packet. Terminal Ctrl-Z cannot stop or bypass any blocking
helper and has no typed adapter outcome. Awsh reports the Ctrl-C packet to Envoy
as a proposal while the helper remains blocked. Envoy orders it against controller
...
The manifested adapter also carries one generated command-resolution inventory
for every immutable hook. Adapter functions must remain their exact readonly
definitions; every other inventory name is forbidden as a workload function or
alias key; and every required builtin must remain enabled and resolve as a
builtin. A positive guard checks this state at launch and every reached gate,
prompt, or frame boundary before normal helper traffic, temporarily using POSIX
special-builtin precedence only after command-free checks. The immutable caller
uses a non-alias-expanded word for the readonly guard function and passes exact
`BASHOPTS` and `SHELLOPTS` expansions as its two invocation-scoped positional
arguments, preserving workload scalars and caller positional parameters. After
rejecting any other arity, the guard uses the validated
`set`, `unset`, and `shopt` builtins to leave POSIX mode, remove its temporary
environment assignment, restore POSIX mode's `inherit_errexit` side effect, and
require both complete option strings equal to those arguments before the helper. Empty-output
success is not evidence.
Unrelated functions, aliases, and builtin enablement remain persistent Bash
state. Initial mismatch fails launch, a gate returns 125, and later mismatch is
fatal `adapter-state`; deliberate guard-root impersonation remains outside the
same-identity cooperative threat boundary rather than being reported as
preserved state.

`EXIT` is the sole persistent workload trap. Trusted source may use `xtrace`,
`ERR`/`DEBUG`/`RETURN`, or a real-signal trap transiently, but must restore the
required boundary state before a gate or source return. Foreground applications
remain free to install their own signal handlers. A source that leaves reserved
state behind is unsupported: tracing, a pending signal, or a pseudo-signal trap
may act on the first boundary command before the adapter can fail closed, so no
successful completion or state-preservation guarantee is made for that path.
...
result. The source bound accounts for the one-top-level-command frame, its
command-free outer source-barrier condition and failure-state restoration, and
two byte-identical authored branches, and Awsh verifies the complete generated
capsule and private `submit` frame before arming. Envoy injects exactly
bracketed-paste begin, that conditional frame, bracketed-paste end, and `LF`;
the validated `C-J` binding accepts that line.
...
disabled. Its immutable inner `if` condition restores the captured `histexpand`
and returns the captured `$?`; this condition is exempt from `errexit`, while
the one selected authored branch is not. The outer redirection condition's
failure branch instead restores `histexpand`, returns that condition's exact
status, and runs no authored source. The first authored command therefore sees
...
the injected bytes and Readline redraw before `output_start`. Awsh first reports
`start_prepared` while the sole `PS0` expansion's helper remains blocked. Envoy
closes that discard interval and sends `start_release`; Bash then reaps the
helper, restores normal `SIGINT`/`SIGQUIT`/`SIGTSTP`, enters the one top-level
compound frame, force-truncates an Awsh-retained sentinel with `>|` without changing
supported `noclobber`, and blocks in the outer condition's command-free FIFO
redirection. Awsh validates that exact hold point, stops the
selected Bash PID, and reports `started`. Envoy writes `operation_started`,
sends `started_ack`, and opens input before Awsh resumes Bash and releases the
FIFO. An immediate event-driven Ctrl-C therefore reaches restored Bash signal
state instead of disappearing in the helper critical section, while Bash still
cannot produce operation output before `output_start`; when it wins, the outer
failure branch runs no source and preserves status 130, reaching the prompt
when `errexit` is disabled or taking Bash's normal status-130 shell-exit path
when it is enabled. The start transaction
uses paired non-resetting five-second actor-local bounds: Envoy owns private
coordination and publication through `started_ack`, while Awsh owns the complete
accepted `execute` through normal source release or the specified intervening
prompt/shell result. This is not a private Awsh-to-Bash source channel, and the
documents and implementation must not describe it as one.
...
state. `xtrace` must be disabled, `ERR`/`DEBUG`/`RETURN` must be unset, and every
catchable real-signal trap in the generated Bash-build inventory must be at
default at every adapter boundary. `EXIT` is the sole persistent trap. Trusted
source may use reserved tracing or traps only transiently and must restore the
required state before a gate or return. A violation is unsupported state and
may intercept the boundary before the adapter can report it; supported adapter
activity never emits workload tracing, evaluates a workload-controlled `PS4`,
or runs a workload signal trap. Readline's `enable-bracketed-paste` value, the exact
...
Controller terminal Ctrl-C and resize remain Envoy-ordered PTY actions. Awsh
reserves a resize slot only after any active prompt, Readline-entry, or
submission termios transaction closes. Envoy then closes the output frontier
and commits the slot, after which Awsh performs the ioctl on its retained control
slave. The all-real-signal boundary rule includes persistent Bash's `SIGWINCH`
trap, while foreground applications retain normal kernel-generated `SIGWINCH`
behavior. By contrast, request-driven cancellation and finalization interruption
use the atomic Awsh-owned action below. A
...
If `killpg` returns `ESRCH`, Awsh does not report `settled`: disappearance of
one foreground job is not completion evidence. It retains the same serialized
action and non-resetting grace timer, processes any accepted helper/result
boundary, and otherwise coalesces one native monotonic event-loop retry no
sooner than one millisecond later. Every ready helper/result/reap event wins
before that retry, and repeated `ESRCH` schedules another single tick rather
than busy-spinning or resetting Envoy's timer. Because no next `execute` can
begin, a later foreground group still
belongs to the same active operation. The action ends only with one successful
interrupt, typed settlement, an already-selected interruption, or the existing
grace-timeout path.
...
sequenceDiagram
    ...
    B-->>A: PS0 start helper reached
    A-->>W: start prepared; helper still blocked
    W->>A: close redraw discard; release start helper
    B-->>A: signals restored; source FIFO barrier reached
    A->>B: stop exact Bash PID
    A-->>W: operation 17 stopped and ready
    W-->>C: operation 17 started; input remains buffered
    W->>A: started acknowledgement
    W->>W: open operation input gate
    A->>B: continue Bash and release source FIFO
...
events. Its private post-source `input_close` result names the still-blocked
prompt helper by PID and `/proc` start time. Envoy closes and quiesces its writer,
pins that identity, cleans and drains every other operation descendant/source,
and only then sends `input_closed`. Awsh flushes the retained slave before
releasing Bash into Readline. Any identity, frame, cleanup, flush, or deadline
failure ends the session rather than risking an unframed idle command or stale
...
1. the complete Bash-state matrix across PTY and split operations, including
   ...
   the complete generated real-signal trap inventory at Bash default, unset
   boundary `ERR`/`DEBUG`/`RETURN` traps, and disabled boundary `xtrace`, with
   the generated command-resolution inventory rejecting reserved function/
   alias shadowing and disabled required builtins while preserving unrelated
   command state, including both initially enabled and disabled
   `inherit_errexit` across its temporary POSIX guard, with snapshot arguments
   preserving caller scalar and positional state, and
   reserved-state corruption failing before another
   submission and independent marker/termios backstops,
   with the exact paste terminator rejected before operation start, plus the
   one-top-level-command frame and command-free retained-sentinel/FIFO outer
   source-barrier condition preserving saved status, `errexit`, `noclobber`,
   and one `PS0` expansion despite workload command-name shadowing;
2. continuous byte-for-byte terminal transport and private raw-output
   ...
   ordered separately in the presentation timeline, plus completion-input
   closure that quiesces a crossing PTY write, discards later terminal bytes,
   pins the exact blocked prompt helper, cleans every other operation descendant
   and drains its output, and flushes an unread multi-byte suffix before Bash
   re-enters Readline;
...
7. controlling-terminal/session/foreground-group setup, interactive input,
   atomic Awsh-classified cancellation interruption, including fair coalesced
   `ESRCH` retry when Bash advances to another foreground job in the same
   operation without starving typed evidence or resetting the grace timer,
   gate Ctrl-C ordered through
   `operation_gate_interrupted`, completion-input close/closed ordering and
   `TCIFLUSH` failure behavior, and the private resize handshake serializing
   Linux `TIOCSWINSZ` with adapter termios work while preserving one
   kernel-generated application `SIGWINCH` without duplicate signaling, plus an
   immediate Ctrl-C after `operation_started` reaching Bash only after the start
   helper restored normal signal state and producing status 130 rather than
   being discarded, plus Ctrl-Z unable to stop or bypass any start, gate,
   startup prompt-ready, or post-source prompt-ready helper while a following
   ordinary foreground child again receives default `SIGTSTP` and can stop
   normally after each helper closes;
...
11. exact pre-submission output retention, source/redraw discard,
   `start_prepared`/`start_release` and stopped-Bash publication ordering,
   operation-start timeout and every sentinel/FIFO/stop/resume failure, ordered
   terminal drain, explicit shell-ended close handshake, and EOF during graceful
   shutdown;
