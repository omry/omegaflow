...
`tcgetpgrp` before private readiness. It retains one close-on-exec,
control-only slave descriptor until Bash is reaped, never reads or writes
terminal bytes through it, and uses it only for identity checks and the atomic
cancellation lookup. A partial launch is killed and reaped and never reaches
public `ready`. Those steps are
...
previously enabled `extglob` or expansion of a syntax-bearing persistent alias,
is rejected before start by fresh-Bash preflight. Ordinary command-position
aliases whose unexpanded source is valid remain preserved.
...
1. the complete Bash-state matrix across PTY and split operations, including
   ...
   Readline state, with syntax-bearing-alias and other
   parser-state-dependent source rejected before start and
...
7. controlling-terminal/session/foreground-group setup, interactive input,
   atomic Awsh-classified cancellation interruption, Ctrl-C, and Linux
   `TIOCSWINSZ`/`SIGWINCH` behavior without duplicate signaling;
