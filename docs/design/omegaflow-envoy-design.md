...
`SIGINT`/`SIGQUIT` and restores their default dispositions after the helper
closes. The shell option and traps are checked through pre-created private
files, without command substitution or another child, before every normal
...
helper, restores normal `SIGINT`/`SIGQUIT`, enters the one top-level compound
frame, force-truncates an Awsh-retained sentinel with `>|` without changing
