...
Readline entry. It then closes every PTY-slave descriptor it owns. Only the
selected-shell tree may retain slave-side descriptors after readiness, so shell
exit can produce master-side EOF. Later Awsh terminal operations open a
...
absence, startup-export transformation, catchable-signal inventory, and
Readline behavior used by readiness. The
