...
master. Awsh's Bash child calls `setsid`, acquires that slave with `TIOCSCTTY`,
installs it as fd 0/1/2, and makes its new process group foreground before
execing Bash. Awsh validates Bash direct parentage plus `tcgetsid` and
`tcgetpgrp` before private readiness and closes its own slave copy. A partial
launch is killed and reaped and never reaches public `ready`. Those steps are
what make Ctrl-C, job control, resize, and finalization rely on a real terminal
...
direct child on the single PTY slave. The manifested rcfile replaces every user
startup file and installs the small immutable Bash bootstrap:

- `PS0` blocks immediately before an armed source unit begins and lets Awsh
  report `started` before Envoy releases Bash;
- `PROMPT_COMMAND` and prompt-readiness hooks report status, physical and
  validated logical cwd, exported environment, and return to input readiness;
- a readonly action-gate helper coordinates `gate_ready`, continuation, and
  cancellation without using terminal text as telemetry; and
- a redirection-only frame-entry marker proves that Bash entered the complete
  submitted brace group before Awsh accepts its completion report.
...
startup `prompt_ready` while no operation is armed. Awsh accepts only that order,
records the initial shell state, and does not emit private `ready` until both
helpers have closed and the Bash parentage, session, and foreground-group checks
pass. The existing startup deadline owns rcfile execution and this handshake;
terminal output is never substituted for readiness.
...
ignores them, suppresses inherited `DEBUG`/`RETURN` tracing only for adapter
code, and restores the exact state after the helper closes. Non-gate helpers
...
its one optional interrupt packet and status 130. A late cancel, finalize, or
terminal Ctrl-C therefore cannot kill a helper and turn an ordinary result race
into `adapter-state`.

Operation source travels cooperatively through the PTY. After fresh-Bash
syntax preflight and rejection of the exact Readline bracketed-paste terminator
bytes in any Bash lexical context, Awsh frames one brace group and Envoy injects it as one
Readline bracketed-paste unit. Envoy's pre-submission drain preserves legitimate
output; its following serialized discard interval suppresses only the injected
bytes and Readline redraw before `output_start`. The start transaction from
satisfied input barrier through matching `started` has one non-resetting
five-second deadline. This is not a private Awsh-to-Bash source channel, and the
...
resets it. `$?` is captured before helper execution and restored at the next
input boundary, while helper-affected transient values such as `PIPESTATUS` and
`$_` are explicitly unpromised. Operation-created processes never persist
across the typed result, so `$!` and jobs are not persistent evidence. Source
whose grammar depends on unsupported persistent parser state, including a
...
state. The Readline bracketed-paste-begin sequence is also reserved. An
operation may alter that binding while it is already running, but the next
submission's frame-entry check must fail the session before a partial command
can be accepted as the operation's terminal result. The adapter does not
silently reset supported user-visible state.

Ctrl-C, planned finalization, and resize remain Envoy-owned PTY actions. A
...
request first and waits for one matching Awsh `disposition`: `signal` authorizes
exactly one `SIGINT` to the current foreground group; `gate-cancelled` confirms
...
phase. The helper critical section makes a source-to-helper transition after a
`signal` disposition harmless. A cancel crossing accepted finalization updates
...
hash-mismatched asset. Terminal, Readline, and locale lookup cannot fall through
to application-controlled data under `HOME` or to another search directory.
...
1. the complete Bash-state matrix across PTY and split operations, including
   ...
   Readline state, with parser-state-dependent source rejected before start and
   reserved hook or bracketed-paste framing corruption failing predictably,
...
7. controlling-terminal/session/foreground-group setup, interactive input,
   Ctrl-C, resize, and `SIGWINCH` behavior;
