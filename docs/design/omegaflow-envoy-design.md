...
act before an adapter function can suppress its own invocation. `SIGCHLD` is
reserved at its default disposition. The option and three traps are checked
...
workload state. A trusted operation may use `xtrace`, `DEBUG`, or `RETURN` only
transiently and must disable or remove it before a gate or source return. If it
...
helper; `DEBUG` and `RETURN` traps must be unset and `xtrace` must be disabled
...
1. the complete Bash-state matrix across PTY and split operations, including
   ...
   default `SIGCHLD`, unset boundary `DEBUG`/`RETURN` traps, and disabled
