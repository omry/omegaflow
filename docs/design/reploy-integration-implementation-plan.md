...
- Freeze the complete Bash state matrix, immutable hooks, reserved
  bracketed-paste enablement and begin binding in both supported line-entry
  keymaps, prompt-boundary validation before readiness, pre-start rejection of
  the exact paste terminator in authored source, independent fail-closed
  frame-entry check, and fresh-parser limitation.
...
for identity checks and atomic cancellation signaling.
...
sequence and the private shell-ended close handshake. Validate bracketed-paste
enablement and the exact begin binding in both supported line-entry keymaps at
startup and every reached prompt boundary; an invalid state must fail before
readiness, while the frame-entry marker remains an independent framing
backstop. Enforce the protocol's closed
simple-alias grammar from `BASH_ALIASES` at startup and every reached prompt
boundary, preserve common command-and-argument aliases, and fail unsupported
grammar-bearing alias state without transferring aliases into Awsh preflight.
Parser-state-dependent source and source containing the exact bracketed-paste
terminator must be rejected before start. Prove Bash and exec'd descendants inherit no
