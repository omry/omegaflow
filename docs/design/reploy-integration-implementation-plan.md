...
- A2.4 fixes exact private source fields, helper IPC, canonical Bash framing,
  ...
  non-returning fail-stop behavior, split-entry proof, fixed helper-socket
  capacity, and the public operation-start barrier.
...
one non-resetting operation-start deadline. Cover minimum and maximum source,
multiline source,
...
markers, argument-free fail-stop mode, split `INNER_ENTERED` sentinel, and
...
helper-socket request/effective-buffer values. Cover partial helper
...
setup and rollback under the original start timer; default-buffer `EMSGSIZE`,
insufficient effective capacity, record truncation, and an exact maximum packet
...
split FIFO paths, the split-entry sentinel, and fail-closed start phases. Set
and verify the fixed send and receive buffer contract on the helper listener,
every accepted connection, and every short-lived client before use.
...
canonical parser state, private bindings, positive helper markers, split-entry
sentinel, and readonly fail-stop wrapper. Build the same manifested `bin/awsh`
with its fixed socket-helper requests, Linux socket-capacity validation, and
...
canonical frame functions, positive source/start markers, split-entry sentinel,
and fail-stop invocation match the frozen fixtures. Prove the exact staged Awsh
binary passes the socket-capacity and non-returning fail-stop fixtures. Prove
