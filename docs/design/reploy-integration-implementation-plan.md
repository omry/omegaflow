...
- Freeze the complete Bash state matrix, immutable hooks, reserved
  bracketed-paste sequence, pre-start rejection of the exact paste terminator in
  authored source, fail-closed frame-entry check, and fresh-parser limitation.
...
make the next frame-entry check fail the session. Enforce the protocol's closed
