...
- Amend the private protocol for Awsh/Bash identity, cooperative PTY source
  submission, `start_prepared`/`start_release` and stopped-Bash start
  acknowledgement, source rejection, explicit shell exit, post-source
  terminal-input closure and slave-queue flush, shell-ended close, split-stream
  FIFOs, and orderly shutdown.
...
- Freeze the complete Bash state matrix, immutable hooks, and one byte-exact
  ...
  exact post-`submit`, pre-first-byte rollback with retained prompt state. Add
  one top-level compound frame whose command-free retained-sentinel/FIFO outer
  condition runs after start-helper signal restoration, with exact Bash
  stop/resume validation before public start, exactly one `PS0` expansion,
  failure-state restoration without authored source, and preserved saved status
  under `errexit`.
...
pre-start drain and stopped-Bash publication barrier before
`operation_started`, keep operation input closed through the complete event and
`started_ack` writes, and freeze zero-byte boundaries for
...
queued cancel/finalize across `start_prepared`/`start_release` and stopped-Bash
publication, immediate terminal Ctrl-C after the public start event,
...
Build both authored branches from byte-identical source inside one top-level
compound frame. Make its outer condition the exact command-free
retained-sentinel/FIFO barrier, nest the saved-status conditional in its success
branch, and restore history state plus the exact barrier status without authored
source in its failure branch. Preserve the exact conditional arguments and
split-stream FIFO redirections, require exactly one `PS0` expansion, and accept
a received capsule only when it is byte-identical to the canonical generated
form. Add every normative
...
`execute`, `submit`, `start_prepared`/`start_release`,
`started`/`started_ack`, source rejection, gate readiness and
`gate_interrupt`/`gate_interrupt_ack`, every cancel/finalize `disposition`,
post-source `input_close` carrying canonical prompt-helper PID and `/proc`
start-time identity plus `input_closed`, active and idle `shell_exit`, resize
...
two-phase helper release and stopped-Bash start/ack barrier, source preflight,
the complete supported state matrix,
...
test exact `SIGINT`/`SIGQUIT`/`SIGTSTP`
verify/ignore/default-restore behavior around every helper. Non-gate helpers
must inherit and verify all three ignores; a gate helper must retain ignored
`SIGQUIT`/`SIGTSTP` while replacing only `SIGINT` with its typed interrupt
handler. Preserve `EXIT` as the sole persistent trap; reserve
...
Generate the complete immutable-hook command-resolution inventory from the
manifested rcfile/build table. Reserve its adapter functions as exact readonly
definitions, forbid every other inventory name as a workload function or alias
key, and require every adapter builtin enabled and positively resolved as a
builtin at launch and every reached gate, prompt, or frame boundary. Before a
temporary POSIX special-builtin-precedence guard, have the immutable caller use
a non-alias-expanded word for the readonly guard function and pass direct
`BASHOPTS` and `SHELLOPTS` expansions as its exact two positional arguments.
Reject any other arity with command-free syntax. Use only those invocation-
scoped parameters as snapshots so workload scalar names and the caller's
positional parameters remain unchanged. After the guard establishes trusted
`set`, `unset`, and `shopt`, restore non-POSIX state,
remove the temporary environment assignment, restore `inherit_errexit` to its
first-argument value, and require both complete option strings equal to the
arguments before helper
spawn. Never treat empty output as successful evidence. Preserve non-reserved
functions, aliases, and unrelated builtin enablement; cover unsupported
guard-root impersonation under the existing cooperative boundary and owning
timeout rather than claiming it is preserved state.
...
normal `errexit` behavior and see that status at its first command. Create and
validate the operation-private retained sentinel and FIFO before arming. After
`start_release` closes the helper critical section, require the generated
one-top-level-command frame's outer condition to apply command-free left-to-right
`>|`/`<` redirections, force-truncate that exact sentinel without changing
`noclobber`, and block on the FIFO; then stop and observe the exact Bash PID before
`started`, and resume plus release the FIFO only after `started_ack`. Preserve
status and `errexit` behavior. On outer-condition failure, restore history state,
return the exact failure status, and run no authored source; immediate Ctrl-C
must reach prompt status 130 when `errexit` is disabled and shell-exit status 130
when it is enabled. Accept an intervening terminal SIGINT or running
cancel/finalize as the specified active-operation crossing, and fail closed on
every artifact, stop, continuation, rendezvous, cleanup, and timeout error.
Apply and
...
`completed` until Envoy has closed and quiesced the operation input gate, pinned
the matching blocked `prompt_ready` helper by PID/start-time/pidfd identity,
terminated and reaped every other operation descendant, and drained every
operation output source. Awsh then flushes the slave queue, releases the helper,
and observes the active-state transition proving Bash has re-entered Readline;
Envoy's final census must prove the exempt helper is gone and only Awsh/Bash
remain. Make
...
Envoy's acknowledgement only after its sole PTY writer is quiescent, later
input is in discard mode, the exact helper is pinned, and universal cleanup and
output drain are complete except for that helper. Successfully flush the
retained slave with `TCIFLUSH` before applying the post-source entry sentinel;
startup skips this operation-only exchange. Apply the closed byte-transparent termios delta
...
submission-redraw discard, `start_prepared`/`start_release` ordering, operation
input held closed through public start and `started_ack`, post-source input-gate
closure that finishes a
crossing PTY write and discards all later operation bytes before acknowledging
Awsh, the paired non-resetting Envoy/Awsh operation-start bounds, bounded
...
Implement Awsh `submit`/`start_prepared`/`start_release`/
`started`/`started_ack`, stopped-Bash publication, source-redraw discard, output
...
cancel crosses finalization. Cover an immediate Ctrl-C after
`operation_started` while Bash remains stopped or blocked at the source FIFO:
the signal must reach restored Bash state and produce the ordinary status-130
operation boundary rather than disappear or become a lifecycle cancellation.
The one top-level frame must run no authored source, restore history state, and
surface that status at the prompt when `errexit` is disabled or through exact
status-130 shell exit when it is enabled.
On `killpg` `ESRCH`, emit no disposition and do not infer settlement. Retain the
same serialized action and non-resetting grace timer, process any accepted
helper/result boundary, and otherwise schedule exactly one coalesced native
monotonic event-loop retry no sooner than one millisecond later. Process every
ready helper/result/reap event before each fresh controlled-terminal/session
validated foreground-group attempt; repeated `ESRCH` must not busy-spin or reset
Envoy's grace timer. Prove a short-lived first job followed by a long second job
receives exactly one successful same-operation interrupt, while repeated
`ESRCH` ends only in typed settlement or the existing timeout path.
Retain a crossed just-written Awsh result until the
...
without resetting the timer. Prove terminal Ctrl-Z cannot stop or bypass the
start, gate, startup prompt-ready, or post-source prompt-ready helper and that
the next ordinary foreground child receives default `SIGTSTP` and can stop
normally after every helper critical section. Freeze
...
post-source completion by keeping Bash blocked in the verified `prompt_ready`
helper, having Awsh report its peer PID/start time, acquiring and validating an
Envoy pidfd for that exact identity, exempting only it while every other
operation descendant is terminated/reaped and all output sources drain, then
requiring its exit and Readline entry before Awsh sends `completed`, followed by
a final Envoy Awsh/Bash-only census before inspection or a public result. Cover
early helper exit, PID reuse, wrong ancestry, extra helpers, a background child
that ignores `SIGTTOU` and mutates termios, and every cleanup/input-close/
Readline deadline crossing. Cover ordinary
background jobs, `disown`, `nohup`, `setsid`, rapid double-fork daemonization,
cancellation, cancellation received after post-source `input_close` while
mandatory cleanup is in progress, planned finalization, the five-second monotonic cleanup
...
interception, every reserved adapter command shadowed independently by a
function or alias, every required builtin disabled independently, positive
guard evidence under empty-success shadows, preservation of non-reserved
functions/aliases and unrelated builtin enablement, both initially enabled and
disabled `inherit_errexit`, and exact before/after `BASHOPTS`/`SHELLOPTS`
equality across the temporary POSIX guard, likely scratch-name collisions,
caller positional-parameter preservation, and wrong-arity refusal, resize
deferral across each termios-handshake phase, private
...
exactly one `PS0` expansion for that one-top-level-command frame, both successful
source release and barrier failure without authored source, prompt status 130
with `errexit` disabled and shell-exit status 130 with it enabled,
every exact `start_prepared`/`start_release`/`started`/`started_ack` ordering and
mismatch, source-barrier sentinel replacement or metadata drift, failed FIFO
creation/open/release/cleanup, wrong Bash stop identity or signal, failed
`SIGSTOP`/`SIGCONT`, workload `kill`/`builtin`/`command` shadowing, immediate
post-event Ctrl-C before source, both preserved `noclobber` values, queued cancel/finalize and resize deferral
through the barrier, operation-start timeout, partial Bash launch cleanup, every disposition phase,
late Ctrl-C and Ctrl-Z during each helper mode, with Ctrl-Z unable to stop or
bypass the helper and the next ordinary foreground child receiving default
`SIGTSTP` afterward, gate-interrupt winners and losers against
...
transitions during Awsh's atomic classified signal action, `killpg` `ESRCH`
followed by a fresh same-operation foreground group and one successful signal,
repeated `ESRCH` on one coalesced one-millisecond-or-later native retry tick,
ready helper/result/reap evidence winning before each retry, no busy-spin, and
eventual typed settlement or unchanged grace timeout,
post-source helper PID/start-time/pidfd pinning, cleanup of a termios-mutating
background child before active-state capture, early helper exit and identity
reuse/reparent failures, crossed private result/disposition
