...
boundary fail closed when reserved state is invalid; do not claim tracing or a pending signal
trap can be suppressed before its own invocation. Allow trusted source to use
...
entry, initial and later non-default traps across the complete generated
real-signal inventory, generated-inventory drift, gate refusal under invalid
