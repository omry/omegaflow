...
cancellation. Require the initial `pty` boundary-only stream, repetition of the
current stream when no byte selected a new one, and a same-offset `stdout` or
`stderr` mark before the first split-stream byte.
...
resize, final-drain, and Readline-entry deadline owner and start epoch.

Gate: exact capsule, maximum-source, malformed-envelope, and deadline-table
tests pass; skeleton-only submission validation is rejected.
...
active and idle `shell_exit`, resize prepare/apply/result, shell-ended shutdown,
and both `closed` reasons. Add fail-closed private stream decoding with exact
...
`completed` until the matching `prompt_ready` helper has exited and the observed
active-state transition proves Bash has re-entered Readline with no prompt
helper left for Envoy cleanup. Make that immutable primary-prompt hook
...
controller presentation. Apply the closed byte-transparent termios delta
