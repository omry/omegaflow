...
- A2.6 fixes controls and crossed lifecycle races as one fresh design-only
  ...
  `Running`; a controller already locally in `Cancelling` or `Finalizing`
  consumes that committed event as a crossed predecessor, stays on its lifecycle
  path, and schedules no handoff work. If a lifecycle request is accepted first,
  do not send an unsent `continue`, but consume Awsh's exactly-one outcome
...
lifecycle request applies from `Running`. If the controller locally enters
`Cancelling` or `Finalizing` before receiving that committed public event, it
must consume the event as a crossed predecessor, remain on the lifecycle path,
