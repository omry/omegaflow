...
- A2.5 fixes ordinary completion and persistent-state handoff: the
  ...
  traps are adapter-reserved and must remain unset; non-reserved signal traps
...
selection with `shell_exit` carrying any valid reaped status, ordinarily 137,
and either identifier form as the sole accepted post-selection reaping
evidence, with a crossed `protocol_error` processed immediately rather than
discarded and a post-`completed` reap held on the operation-cleanup deadline; a
lifecycle request forwarded after `completed` answered `returned`/`none` and
discarded; a foreground job ending between `tcgetpgrp` and `kill` whose dead
group survives a retry, resolved by signalling the reclaimed group, by
reporting `returned`/`none`, or by the shell being reaped so teardown still
yields `shell_exit`; the resize lane
idle, waiting behind each lease holder, mismatched at each frame, doubled, and
crossed by shell exit; shutdown by `SIGHUP` reporting the real
reaped status, 129 for the default disposition and the handler-selected status
under a permitted exiting `HUP` trap, a `HUP`-ignoring shell reaching the fatal
final-drain result, and both crossing orders; the active, empty, and
post-`completed` `shell_exit` identifier forms, with a pending cancellation or
finalization published under its own schema and no added public field; and a
control write failing after each terminal result being orderly. Leave final
private-schema closure to A2.7.
...
The completion hook must validate the four unset adapter-sensitive traps before
...
foreground process group under a lease, retried under Envoy's grace period
while the terminal still names a vanished group, a blocked gate helper closed
first without an interruption, and no signal once the completion `prompt_state`
...
internally, `SIGKILL` the selected-shell tree, and handle results read after
that selection exactly as the protocol's lifecycle-control section defines
them, including its two exceptions to the discard: `gate_interrupt`, which is
acknowledged under the single action-gate rule, and `protocol_error`, which is
processed immediately as the fatal terminal result and is never followed by
`shell_exit`; drive the resize lane
...
the pending result under its own public schema, adding `shell_ended` only to
`operation_completed` and `operation_failed` as the protocol's shell-end
mapping requires, bounds the reap under whichever deadline that mapping names
as live, and enters the drain; and that a control write failing after an
accepted terminal result is orderly. For that terminal
