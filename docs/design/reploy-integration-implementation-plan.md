...
sentinel, post-`PS0` release signal, and readonly fail-stop wrapper. Build the
...
canonical frame functions, positive source/start markers, post-`PS0` release
signal, split-entry sentinel, and fail-stop invocation match the frozen
fixtures. Prove the exact staged Awsh binary passes the stream-framing and
non-returning fail-stop fixtures. Prove
