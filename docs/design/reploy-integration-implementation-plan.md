...
operation result, observe EOF on the
...
operation result and takes fatal session teardown. Also prove
...
operation result, prevent a later operation, and take fatal session teardown.
Also cover mutation races, cleanup and drain failures, and every inspection
resource limit.
