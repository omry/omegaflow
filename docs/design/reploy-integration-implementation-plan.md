...
adapter-required builtin set. The `CHLD` reservation prevents an adapter-owned
...
and expanded `CHLD`/`SIGCHLD` spellings, `trap 'exit 42' DEBUG`,
`trap 'exit 42' CHLD`, and `enable -n kill`. Also prove
