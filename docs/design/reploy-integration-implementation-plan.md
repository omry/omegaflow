...
verify the protocol's closed byte-transparent termios delta only for injection, and
require normal Readline return to restore the complete pre-Readline state before
Awsh accepts `start`. Keep the frame-entry marker and termios comparison as
