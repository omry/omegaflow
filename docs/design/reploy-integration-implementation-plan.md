...
`DEBUG`, `ERR`, and `RETURN` traps and the exact adapter-required builtin set.
Prove prohibited direct and expanded-argument mutations fail-stop before state
changes, including `trap 'exit 42' DEBUG` and `enable -n kill`. Also prove
