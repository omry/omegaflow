...
as the sole accepted post-selection result; a lifecycle request forwarded
after `completed` answered `returned`/`none` and discarded; a foreground job
ending between `tcgetpgrp` and `kill` re-read and signalled; the resize lane
