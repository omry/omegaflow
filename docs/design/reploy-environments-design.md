...
then uses reserved `wait` to clear Bash-owned child records, restores canonical
unset `INT` with reserved `builtin trap - INT`, verifies `builtin trap -p INT`
is empty, proves the job table empty, revalidates the adapter, and sends
