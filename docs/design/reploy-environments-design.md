...
path, because a static reserved entry cannot clear arbitrary names.
...
`AWSH_`. This is the normative application-environment enumeration referenced
by the other design documents. After that check, reserved composition adds the
sole allowed launch values: an empty `HISTFILE`,
...
is empty and read-only. Final validation requires those exact values. Before
deployment, host OmegaFlow verifies from the trusted runtime manifest the empty
