...
before releasing Bash, invokes the fixed argument-free manifested
...
tracing and execution controls, traps, job-control entry state, or two
source-trigger bindings: source using the reserved namespace and planned
terminal input producing either reserved byte sequence are rejected, while a
runtime failure to regain the cooperative reservation fails the session. The
defaults are:
...
- the fixed Linux helper transport capacity: listener, accepted, and client
  sockets request 1,048,576-byte send and receive buffers and verify effective
  values of at least 2,097,152 bytes before use; lack of that capacity prevents
  readiness, and a later mismatch fails the session;
