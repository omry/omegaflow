...
fixed terminal type and trusted terminal-database path, and the fixed empty
`HISTFILE` launch value — compose after it, so an application cannot. Composing
the reserved entries first would let an application win under Hydra's ordinary
behaviour and disconnect the Envoy listen arguments from the coordinates
Reploy reports. Launch-affecting names otherwise take a validation path,
because a static reserved entry cannot clear arbitrary names.
...
allowed launch values: an empty `HISTFILE`, `TERM=xterm-256color`, and both
`TERMINFO` and `TERMINFO_DIRS` set to the read-only
`/omegaflow-runtime/share/terminfo` database. Final validation requires those
exact values. Before deployment, host OmegaFlow verifies from the trusted
runtime manifest that the database contains the exact `xterm-256color` entry
and that the read-only runtime mount cannot be shadowed. A missing, unreadable,
or hash-mismatched entry fails preparation; neither shell launch may fall
through to an application-controlled database under `HOME` or to another
search directory.

The controlled-session bootstrap therefore never opens an
application-selected history or terminal-database file, even when `/bin/sh` is
Bash. In particular, `INPUTRC` cannot supply a Readline macro that rewrites or
submits part of the Envoy bootstrap command before OmegaFlow code starts. A
workload whose commands need a loader variable, history file, or different
terminal lookup sets it inside operation source, where the persistent shell
carries it to operation children as ordinary shell state without governing
either shell launch. The defaults are:
...
8. **Terminal-only:** Compose typed `reploy.controller` and `reploy.workload`
   ...
   entry in the materialized workload; and prove that a hostile
   `$HOME/.terminfo`, a missing trusted entry, or a runtime-manifest mismatch
   cannot reach the bootstrap shell.
