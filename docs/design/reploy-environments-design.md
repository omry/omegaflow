...
- Owner-approved environment direction with the Hydra blueprint, placement,
  ...
  only to a hash-bound `.review` sidecar when one is present, and these amended
  bytes require a fresh local and remote review cycle. The trusted
...
for a matching disposition: `signal` authorizes one foreground-group `SIGINT`,
...
source crosses into helper work after a `signal` disposition. Awsh emits no
...
2. **Terminal-only:** Exercise fragmented terminal and telemetry bytes,
   ...
   output retained, operation-start timeout, private disposition handshakes,
