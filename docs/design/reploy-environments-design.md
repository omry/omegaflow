...
the intended result without another ioctl or timer reset. A cancel accepted
...
with reserved `builtin trap '' INT`, restores the one canonical unset state
with reserved `builtin trap - INT`, and requires `builtin trap -p INT` to be
empty before reuse. It then refuses ordinary
