...
: The directory containing shared recording configuration, recording scripts,
  ...
  project root. Its nested `.omegaflow` data directory contains generated
  recording state and is ignored rather than version-controlled.
...
PROMPT_COMMAND PS0 PS1 PS2 PS3 PS4 SHELLOPTS TERM TERMINFO
TERMINFO_DIRS TMOUT
```
...
history, Readline, terminal-database, or locale-database file, even when
`/bin/sh` is Bash. The fixed empty `INPUTRC` prevents Readline from falling back
to `$HOME/.inputrc`, so an application macro cannot rewrite or submit part of
the Envoy bootstrap command before OmegaFlow code starts. A workload whose
commands need a loader variable, history file, Readline configuration,
different terminal lookup, or different locale sets it inside operation
source, where the persistent shell carries it to operation children as
ordinary shell state without governing either shell launch. The defaults are:
...
The recording directory's nested `.omegaflow` is the configured
`studio.data_dir`. It contains generated recording state, not the
version-controlled recording configuration, scripts, or `reploy/app`
blueprints beside it. Bootstrap must ensure that nested directory is ignored in
the applicable Git workspace. If no existing rule already ignores it,
bootstrap creates or updates `<recording_dir>/.gitignore` with
`/.omegaflow/`, preserving existing content and avoiding duplicate rules.
...
4. Ensure the recording directory's nested `.omegaflow` data directory is
   ignored, preserving any existing `.gitignore` content.
...
8. **Terminal-only:** Compose typed `reploy.controller` and `reploy.workload`
   ...
   `TERMINFO_DIRS`, `LANG`, `LANGUAGE`, `LOCPATH`, and every `LC_*` name;
   verify the reserved terminal and locale values, trusted terminfo entry,
   complete trusted locale tree, and trusted empty `INPUTRC` in the
   materialized workload; and prove that hostile `$HOME/.terminfo`,
   `$HOME/.inputrc`, and application locale data, a missing or non-regular
   trusted file, or a runtime-manifest mismatch cannot reach the bootstrap
   shell.
9. **Bootstrap stack:** Bootstrap both an in-project recording directory and a
   sibling recording directory; ensure each nested `.omegaflow` data directory
   is ignored without duplicating rules or overwriting existing `.gitignore`
   content; detect a synthetic Go/Python project without executing its code.
...
7. The project metadata and configuration `.omegaflow` directory belongs to
   ...
   ignored rather than version-controlled.
