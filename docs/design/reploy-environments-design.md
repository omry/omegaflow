...
12. **Terminal-only rollout:** Prove omitted production configuration continues
    ...
    `workload_backend=reploy` explicitly, and the Reploy-backed production path
    cannot become the default execution path before the separately reviewed
    host-Envoy parity and cutover gate passes.
...
18. `studio.workload_backend` defaults to `host`. Explicit `reploy` selection
    requires a complete workload blueprint and never falls back after
