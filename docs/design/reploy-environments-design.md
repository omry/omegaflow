...
mount, the two exact endpoint IDs, bind addresses, and container ports, and the
...
allowed launch-control value, an empty `HISTFILE`, and final validation
requires that exact value. The controlled-session bootstrap therefore never
opens an application-selected history file or the default under an
application-selected `HOME`, even when `/bin/sh` is Bash. In particular,
`INPUTRC` cannot supply a Readline macro that rewrites or submits part of the
Envoy bootstrap command before OmegaFlow code starts. A workload whose commands
need a loader variable or history file sets it inside operation source, where
the persistent shell carries it to operation children as ordinary shell state
