...
and kind and computes deterministic file or directory SHA-256 values before
returning typed inspection or produced-output records with the terminal result.
...
2. **Terminal-only:** Exercise fragmented terminal and telemetry bytes,
   ...
   workload-side file and produced-output inspection,
