...
tracing and execution controls, adapter-sensitive traps, adapter-required
...
reserved byte sequence are rejected; readonly shell mediation refuses ordinary
source mutations of the reserved traps or required builtin enablement or
implementation, preflighting a complete expanded request before any partial
state change; and a runtime failure to retain or regain any cooperative
reservation fails the session. Deliberate explicit-builtin bypass remains same-
identity interference, not a supported source capability. The defaults are:
