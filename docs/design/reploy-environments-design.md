...
Awsh, the completion hook sends its `prompt_state` snapshot, and Awsh proposes
`input_close` with only the operation ID and exact completion-helper PID. The
Envoy uses that proposal as the cleanup-timer boundary, terminates and reaps
every remaining operation-created process, reaches EOF, drains its output, and
sends `input_closed` before any final completion report. The hook then uses
reserved `wait` to clear Bash-owned child records, proves the job table empty,
revalidates the adapter, and repeats `prompt_ready`; only after actual Readline
entry does Awsh send `completed` with the final status, physical cwd, and
resolved inspection plan. Other persistent Bash state remains live rather than
entering a protocol frame. The public `operation_completed` schema remains
unchanged, and this private handoff is not a new Reploy lifecycle field.
...
typed operation request. At ordinary return, the completion hook's `prompt_state`
supplies the status, physical and logical cwd, and exported environment; Awsh
proposes `input_close` with only the operation ID and exact completion-helper
PID. Envoy performs the mandatory operation cleanup and sends `input_closed`;
Bash then clears wait records, proves the empty job table and adapter/readiness
conditions, and re-enters Readline. Only then does Awsh resolve configured paths
and send `completed` with the final status, physical cwd, and resolved inspection
plan. The Envoy then proves that external Awsh and the persistent selected shell
are the only processes left in its controlled tree and drains output through the
closing offset before it checks existence and kind.
...
2. **Terminal-only:** Exercise fragmented terminal and telemetry bytes,
   ...
   trap-state validation before reuse,
   completion-side status/cwd/exported-environment handoff plus preservation of
