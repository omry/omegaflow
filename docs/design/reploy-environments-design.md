...
acquiring the lock, preserves its mode and every unrelated byte, avoids an
equivalent duplicate rule, and publishes the complete candidate through an
adjacent temporary file and atomic replacement. A concurrent content change
causes a bounded reread-and-retry rather than a lost update. Failure removes
the candidate and leaves the prior ignore file intact. After an existing or
new rule is selected, every claiming VCS adapter must prove that a
repository-owned rule effectively ignores the exact data directory. A more
specific negation or any unverifiable result fails with the responsible rule;
bootstrap does not rewrite another ignore file or broaden the pattern.
...
9. **Bootstrap stack:** Bootstrap an in-project recording directory, Git and
   ...
   file, concurrent bootstrap, interrupted candidate publication, worktree-root
   and VCS-administrative paths, recognized unsupported VCS state, and a path
   outside every VCS worktree. Verify atomic preservation of unrelated ignore
   content and the final effective-ignore postcondition; detect a synthetic
   Go/Python project without executing its code.
