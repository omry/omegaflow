...
source crosses into helper work during that atomic action. Awsh does not report
`settled` merely because one sampled foreground group vanished: `ESRCH` retains
the same action and timer, reclassifies accepted helper/result evidence, and
otherwise schedules one coalesced native monotonic retry no sooner than one
millisecond later. Ready helper/result/reap evidence wins before each fresh
validated group attempt; repeated disappearance neither busy-spins nor resets
the grace timer. The loop ends when one signal succeeds or the operation reaches
a typed settlement or timeout. Awsh emits no synthetic
cancellation result; Envoy maps the next ordinary completion or explicit shell
reap. This handshake is private OmegaFlow runtime behavior and does not add a
Reploy lifecycle operation.
...
resulting cwd and exported environment. Awsh captures that candidate result at
`prompt_state` and keeps the matching `prompt_ready` helper blocked while Envoy
closes operation input, pins that exact helper identity, cleans and drains every
other operation descendant/source, and acknowledges the boundary. Awsh then
flushes input, releases the helper, and observes the Readline-entry transition;
after Awsh sends the candidate result, a final Envoy census proves only Awsh and
Bash remain before inspection or a public terminal result. The immutable primary-prompt hook emits
no display bytes during that wait; the visible following prompt remains
controller-synthesized presentation and stays outside the operation's raw-output
range. The Envoy revalidates the already-clean Awsh/Bash-only boundary and
closing output offset, then checks existence and kind. Environment-setup
services intentionally remain outside that cleanup,
...
2. **Terminal-only:** Exercise fragmented terminal and telemetry bytes,
   ...
   signal-safe helper critical sections, terminal Ctrl-Z unable to stop or
   bypass any blocking helper with a following ordinary foreground child again
   receiving default `SIGTSTP` and stopping normally afterward,
   generated command-resolution reservations that reject adapter-name function
   or alias shadowing and disabled required builtins without changing unrelated
   Bash state, including exact `inherit_errexit` preservation across temporary
   POSIX lookup using invocation-scoped snapshot arguments that preserve caller
   variables and positional parameters, fair `ESRCH` retry across successive same-operation foreground jobs,
   prompt-helper identity pinning and descendant cleanup before the next
   Readline snapshot,
   every source/gate/completion race for cancellation and finalization, active
   and idle shell exit followed by
