...
`emacs-standard` keymap, and sources no other file,
...
- fixed helper requests to the mode-0600
  ...
  to the short-lived source helper and captured directly into Bash's reserved
  Readline buffer rather than written through the PTY;
- mode-0600 per-operation split stdout and stderr FIFOs below the mode-0700
  ...
  by Awsh, opened only by the canonical Bash source frame, and removed after
  completion cleanup and dual EOF;
