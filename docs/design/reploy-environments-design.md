...
`prompt_state` but does not send it until the matching `prompt_ready` helper has
exited and the observed Readline-entry transition proves Bash is input-ready
with no prompt helper left for cleanup. It then sends the resolved plan to the
Envoy over the private result channel. The immutable primary-prompt hook emits
...
range. The Envoy first performs the mandatory
