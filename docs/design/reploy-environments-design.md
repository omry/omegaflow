...
A dependent OmegaFlow Workload Envoy design slice is authoritative for terminal
control, recording-PTY ownership, shell telemetry, terminal-to-browser
coordination, and their implementation sequence. This document remains
authoritative for the broader Reploy environment, blueprint, bootstrap,
lifecycle, artifact, packaging, and migration direction. It does not define a
second terminal implementation path.
...
Both fields use the same typed `host | reploy` value domain, with semantics
owned by the field. `studio.recording_backend` defaults to `reploy`. The `host`
value is schema-valid so configuration remains forward-compatible, but current
...
The `host` default is the post-cutover product contract, not permission to
route default recordings through an incomplete Reploy-backed host path. Until
a separately reviewed host-Envoy parity and cutover gate passes, omitted
production configuration remains on the existing FIFO-backed host recording
path, the terminal-only isolated milestone selects
`studio.workload_backend=reploy` explicitly through its bounded non-production
entry point, and OmegaFlow does not enable the Reploy-backed production path for
default recordings. Explicit `host` selection against that pre-cutover path
fails capability validation; it never substitutes an isolated workload.

Configuration ingestion preserves that distinction before it applies the
post-cutover default. During migration, the pre-normalization
`studio.workload_backend` input is presence-aware: its supplied-value domain is
still exactly `host | reploy`, but omission remains absent until capability
routing completes. A value in user-owned project or recording configuration,
or a CLI override, is explicit; OmegaFlow's built-in default is not. The
pre-cutover router therefore maps absence to the FIFO path, explicit `host` to
the targeted unsupported-host error, and explicit `reploy` to the isolated
milestone. It then normalizes absence to `host` in the final typed OmegaFlow
configuration and discards the internal presence fact. That fact is not a
user-facing field, a third backend value, or part of either materialized Reploy
blueprint. If composition cannot preserve presence through this routing point,
configuration fails instead of guessing from the normalized `host` value.
...
: The application source root from which `omegaflow bootstrap=project` is run.
  It owns the project metadata and configuration `.omegaflow` directory and is
  the source location that the recorded application environment reproduces.

**Recording directory**
: The directory containing shared recording configuration, recording scripts,
  application blueprint config groups, and authored recording definitions. It
  defaults to `recordings` under the project root but may be elsewhere. It is
  not a second project root. The configured `studio.data_dir` contains
  generated recording state, defaults to the recording directory's nested
  `.omegaflow`, and is ignored rather than version-controlled when it lies in a
  supported worktree.

**Supported worktree**
: A Git worktree or a Git-compatible Sapling worktree that consumes
  `.gitignore`. The VCS adapter supplies worktree-root discovery, administrative
  path identity, tracked-path inspection, and effective-ignore inspection. The
  innermost supported worktree containing a resolved data directory owns its
  ignore policy. When Git and Sapling both claim that same root, the tracked and
  ignore postconditions must hold for both. Recognized but unsupported or
  unverifiable VCS state fails bootstrap rather than being treated as no
  worktree.
...
no terminal operation result. Selection of the ordinary started-operation
lifecycle path starts the existing grace deadline before foreground sampling;
successful ioctl return does not
reset it. Envoy makes that target safe through its controlling-terminal
...
`ioctl(PTY_MASTER, TIOCGPGRP, &foreground_pgid)` and validates every live
`/proc` member of a positive group against the controlled session ID and
pidfd-backed controlled-tree census. A positive group with no live member is a
permitted job-return/Bash-reclaim crossing: Envoy resamples under the same
...
and deadline expiry selects the lifecycle timeout. The kernel then atomically
targets the current group in that same
session; an invalid sample, a live wrong-session or unclassifiable member, or
another inability to prove the invariant is fatal before a signal or terminal
result. Awsh remains responsible for selected-shell and completion-helper
...
If Envoy accepts `gate_ready` before a lifecycle request, it publishes
`operation_ready` and applies a later lifecycle request from `Gated`; if it
accepts the later gate outcome before a lifecycle request, it publishes
`operation_continued` or `operation_gate_interrupted` and applies a later
lifecycle request from `Running`. A controller already locally in
`Cancelling` or `Finalizing` consumes any such committed event as a crossed
predecessor, schedules no handoff work, and stays on its lifecycle path. If
Envoy accepts the lifecycle request first, it consumes Awsh's
...
workload operation running, controller OmegaFlow waits for `operation_started`,
then waits for the exact operation-scoped `operation_ready` gate compiled for
the handoff. The trusted source calls that gate in the intended service's launch
...
`host | reploy` value domain. Recording defaults to `reploy`, the only supported
controller backend. `recording_backend=host` is schema-valid but fails current
capability validation before preparation with a targeted unsupported-controller
error; it reserves a possible future bare-metal controller without implying an
implementation, maintenance commitment, or fallback.
...
After production cutover, the workload runs on the host unless
`studio.workload_backend=reploy` is selected with a complete workload
blueprint. Before cutover, the presence-aware routing contract above selects
the FIFO path, rejects explicit `host`, or selects the isolated milestone
before the final typed default is applied. This choice does not change the
...
Output assertions therefore require exclusive observation and a fresh
output-through drain barrier before execution. All bytes read from a PTY master,
including terminal echo of controller-authored input, remain ordinary PTY
output. The workload owns its terminal modes, and Linux exposes no reliable
boundary proving that line-discipline processing and delivery of resulting echo
are complete, so the Envoy does not try to infer echo provenance.

OmegaFlow rejects `output_contains` and `output_regex` when an interactive
operation or any continuation sends bytes through `text`, `key`, or `control`.
`wait_for` and `pause` send no bytes and remain compatible with output
assertions. `wait_for` matches visible terminal text, including echo, and is
only a sequencing mechanism. Exit status, `file_exists`, and produced-output
inspections remain available after authored input, and a later non-interactive
operation can verify output or file content. Failure cancellation and user
cancellation invalidate assertions. Planned recording-end finalization is a
distinct typed lifetime result: it terminates and drains the intentionally open
...
exclusive observation, as does any presentation-timed operation. This choice
controls evidence and publication, not process lifetime. Planned finalization
uses the same mandatory operation cleanup as natural return and cancellation;
failure to complete it fails closed.
...
both streams to the terminal channel in observed order. PTY operations
...
polled termios state. A PTY assertion is permitted only when the operation and
its continuations send no authored terminal bytes; it then matches the same
CRLF conversion and other terminal transformations as the current realtime
runner. Split-stream evidence bypasses the terminal line discipline and
preserves newline-sensitive stdout-then-stderr checks. The raw terminal log and
published cast retain their exact bytes and are never normalized.
...
does not reset the bounded grace period already started when Envoy selected the
ordinary started-operation lifecycle path before foreground sampling, while
failure is fatal with no terminal
...
cannot be handled by setup. With no earlier operation process retained, every
non-shell process adopted by the Envoy at an operation boundary belongs to the
current operation, so this v1 contract does not require a separate
per-operation cgroup.
...
change to a selected path or traversed entry while its identity, kind, directory
membership, symlink target, metadata, or bytes are inspected — or inability to
establish that stability on the backing filesystem — fails through the typed
inspection-failure path and emits no inspection or produced-output record. The
Envoy computes deterministic file or directory SHA-256 values only from an
accepted stable inspection. Protocol v1 owns the concrete race-detection
algorithm and failure-code mapping that enforce this invariant.
Controller OmegaFlow records
...
Envoy-requirement validation rejects that complete pre-reservation environment
when it contains any of these exact names:

```text
AWSH_BASH BASH_COMPAT BASHOPTS BASH_ENV BASH_XTRACEFD CDPATH ENV
GLOBIGNORE HISTFILE INPUTRC LANG LANGUAGE LOCPATH POSIXLY_CORRECT
MAIL MAILCHECK MAILPATH PROMPT_COMMAND PS0 PS1 PS2 PS3 PS4 SHELLOPTS TERM TERMINFO
TERMINFO_DIRS TMOUT
```

It also rejects every name beginning with `BASH_FUNC_`, `LD_`, `LC_`, or
`AWSH_`. This is the normative effective-environment enumeration referenced by
the other design documents; checking only the application environment mapping
is insufficient because image-config variables otherwise remain inherited. If
preparation cannot resolve and inspect the exact image-plus-application
environment that Reploy would launch, preparation fails. After that check,
reserved composition adds the sole allowed launch values: an empty `HISTFILE`,
`INPUTRC=/omegaflow-runtime/etc/inputrc`, `TERM=xterm-256color`, both `TERMINFO`
and `TERMINFO_DIRS` set to the read-only
`/omegaflow-runtime/share/terminfo` database, `LC_ALL=C.UTF-8`,
`LANG=C.UTF-8`, and `LOCPATH=/omegaflow-runtime/lib/locale`. The `INPUTRC` file
is empty and read-only. Final validation re-materializes the launch environment
...
processes an application-selected mail notification hook, even when `/bin/sh`
is Bash. The fixed empty `INPUTRC` prevents Readline from falling back to
`$HOME/.inputrc`, so an application macro cannot rewrite or submit part of the
...
adapter-required builtin state, POSIX mode, or the readonly-unset
`POSIXLY_CORRECT` variable, job-control entry state, or two source-trigger
bindings: source using the
reserved namespace and planned terminal input producing either reserved byte
sequence are rejected; readonly `trap` and `enable` mediation preflights each
complete expanded request. The `trap` mediator canonicalizes every
selected-Bash `signal_spec` using the selected
...
trap. The fixed rcfile declares `POSIXLY_CORRECT` readonly while unset; on every
selected Bash build this prevents `set`, `shopt`, their explicit-`builtin`
forms, assignment, or unset from enabling POSIX mode or changing the variable.
There is no `set` or `shopt` mediator: those attempts retain the selected
build's ordinary status and diagnostic, as do POSIX queries and requests that
leave the mode disabled. The
completion hook saves source status and may temporarily install ignore
...
deployment's own network namespace and cannot collide. The blueprint requests
no host publication. Reploy attaches the controller and workload to their fresh
lease-owned internal network and reports each endpoint in `opened.endpoints` as
the fixed lease-local `workload` alias plus its container port. The controller
connects only to those reported coordinates. This session-network grant does
not imply general `local: allow`; the accepted initial limitation is coarse
bidirectional reachability to any listening port on the other session
participant until Reploy supplies directional and per-port enforcement.

The container ports are not overridable because controller OmegaFlow launches
the Envoy with listen arguments constructed from those same frozen values.
Letting an application move a container port would leave the Envoy listening on
the reserved default while `opened.endpoints` named the override, and the
handshake would never connect. The endpoint entries therefore compose after the
application, under the same rule that reserves any entry an override can never
legitimately change.

The blueprint deliberately declares no Envoy startup command and does not
replace Reploy's controlled-session `/bin/sh` bootstrap. After Reploy reports
`ready`, controller OmegaFlow reads the retained endpoint values and types the
exact `/omegaflow-runtime/bin/envoy` `exec` invocation over the attachment,
including its fixed listen arguments, controller-generated handshake
`session_id`, and initial dimensions. That invocation is an OmegaFlow bootstrap
contract, not a Reploy blueprint command.
...
Where an override is never legitimate, the Envoy-reserved entry composes after
the application instead, so it cannot be overridden at all.
...
The recording directory's nested `.omegaflow` is the default
`studio.data_dir`. The configured data directory contains generated recording
state, not the version-controlled recording configuration, scripts, or
`reploy/app` blueprints beside it. Bootstrap resolves that configured path once
through its existing ancestors before creating the directory or generated
state. It selects the innermost enclosing supported worktree and rejects a data
directory equal to its root or within that VCS's administrative storage. It
also fails without changing the VCS index or ignore files when the data
directory or any descendant is already tracked.

Inside a supported worktree, success requires an effective repository-owned
`.gitignore` rule. A user-global ignore or an administrative exclude such as
Git's local exclude file does not satisfy that portable repository policy. An
existing applicable `.gitignore` rule inside the worktree does. Otherwise,
bootstrap encodes the resolved worktree-relative directory as one literal,
root-anchored `/<data-dir-relative-to-worktree>/` pattern and appends it to the
worktree root's `.gitignore`. Pattern metacharacters are escaped so the rule
matches only that exact directory subtree; a path that cannot be represented
losslessly in the line-oriented ignore format fails.

The root `.gitignore` must be absent or a regular non-symlink file. Bootstrap
serializes its own writers with a worktree-scoped lock, rereads the file after
acquiring the lock, preserves that snapshot's mode and every unrelated byte,
avoids an equivalent duplicate rule, and publishes the complete candidate
through an adjacent temporary file and atomic replacement. Before replacement,
bootstrap compares the target with the snapshot used to build the candidate; a
detected change causes a bounded reread-and-retry. The lock prevents lost
updates among cooperating OmegaFlow processes. It does not coordinate a
non-OmegaFlow writer that ignores the lock, and the pre-replacement comparison
cannot close the final check-to-replacement race with such a writer. A
publication failure before replacement removes the candidate and leaves the
then-current ignore file untouched. After an existing or new rule is selected,
every claiming VCS adapter must prove that a repository-owned rule effectively
ignores the exact data directory. A more specific negation or any unverifiable
result fails with the responsible rule; bootstrap does not rewrite another
ignore file, broaden the pattern, or attempt a rollback that could overwrite a
later edit.

A data directory outside every recognized VCS worktree needs no ignore-file
change. Thus the in-project default uses `/recordings/.omegaflow/` in the
project root, while a sibling recording directory that is itself a supported
worktree uses `/.omegaflow/` in that worktree root.
...
4. Resolve the configured `studio.data_dir`. Reject a supported worktree root,
   VCS administrative path, or tracked path; establish and verify the exact
   repository-owned ignore rule through the safe worktree-root transaction
   above; then create the data directory. Outside every recognized VCS
   worktree, create it without an ignore-file change.
5. Inspect declarative project metadata without executing project code.
6. Identify every supported project facet.
7. Create a conservative initial application blueprint in the `reploy/app`
   config group.
8. Validate the materialized candidate through Reploy.
9. Report detected facets, added dependencies, and unresolved findings.
...
candidate outside the selectable config group, provisionally beneath the
configured `studio.data_dir`:

```text
<studio.data_dir>/generated/reploy/app/<name>.yaml
```

For a sibling recording directory in the layout above, this resolves beneath
`/src/arbiter-recordings/.omegaflow/generated`, not the project-root
`.omegaflow`. Refresh presents a diff, and the user manually merges any desired
changes into the config-group entry. This avoids hidden ownership metadata and
ambiguous list merge behavior.
...
2. **Terminal-only:** Exercise fragmented terminal and telemetry bytes,
   ...
   compile-time rejection of output assertions on interactive operation chains
   ...
   planned finalization of an asserted open operation, cancellation and
   finalization timeout results carrying `shell_ended` followed by session
   drain, shell-ended drain resolving a crossed outstanding resize without a
   recorded resize event, shell-ended drain resolving both an unstarted crossed
   execute and its deadline-derived cancel without a terminal operation result,
   ...
   and nested-shell fixtures), POSIX-mode enablement and `POSIXLY_CORRECT`
   mutation attempts that cannot bypass that reservation, stale empty
   foreground-group return/reclaim crossings whose signal, `input_close`,
   `shell_exit`, and timeout winners retain the one lifecycle deadline,
...
6. **Browser stack:** Verify that endpoint IDs resolve only through the trusted
   ...
   closed; prove a blocking workload reaches its compiled operation-scoped gate
   from the intended service launch path, then reaches controller-observed
   endpoint health without a workload file, OSC marker, or browser-specific
   telemetry message; prove a temporal unready-to-ready transition without the
   current operation's gate cannot authorize browser work; and prove operation
   completion or failure beats gate or health success, with the endpoint also
   required to be unready before the operation starts as a stale-listener guard.
...
8. **Terminal-only:** Compose typed `reploy.controller` and `reploy.workload`
   ...
   retained. Reject application-provided `TERM`, `TERMINFO`,
   `TERMINFO_DIRS`, `LANG`, `LANGUAGE`, `LOCPATH`, `MAIL`, `MAILCHECK`,
   `MAILPATH`, and every `LC_*` name; verify the reserved terminal and locale
   values, trusted terminfo entry, complete trusted locale tree, and trusted
   empty `INPUTRC` in the materialized workload; and prove that hostile
   `$HOME/.terminfo`, `$HOME/.inputrc`, application locale data, or mailbox and
   mail-path notification hooks, a missing or non-regular trusted file, or a
   runtime-manifest mismatch cannot reach the bootstrap shell.
9. **Bootstrap stack:** Bootstrap an in-project recording directory, Git and
   Sapling sibling recording-directory worktrees, and a custom project-relative
   `studio.data_dir`; choose the innermost worktree and require every VCS that
   claims its root to agree. Prove that a repository `.gitignore` rule is
   reused, while a user-global or administrative exclude still causes the exact
   repository rule to be written; reject already tracked data without changing
   the index. Exercise a literal metacharacter-bearing path, equivalent-rule
   idempotence, a more-specific negation, a symlink or non-regular root ignore
   file, concurrent OmegaFlow bootstrap, a detected pre-replacement target
   change, interrupted candidate publication, worktree-root and
   VCS-administrative paths, recognized unsupported VCS state, and a path
   outside every VCS worktree. Verify atomic preservation of the unrelated
   ignore content in the candidate's accepted source snapshot, the stated
   boundary for non-cooperating external writers, and the final
   effective-ignore postcondition; detect a synthetic Go/Python project
...
12. **Terminal-only rollout:** Prove omitted production configuration continues
    to use the existing FIFO-backed host path, the isolated milestone selects
    `workload_backend=reploy` explicitly, explicit `workload_backend=host`
    fails with the targeted pre-cutover capability error, and omission remains
    distinguishable until routing even though the final typed value normalizes
    to `host`. Prove the internal presence fact is neither user-configurable nor
    serialized into a Reploy blueprint, and that the Reploy-backed production
    path cannot become the default execution path before the separately
    reviewed host-Envoy parity and cutover gate passes.
...
7. The project metadata and configuration `.omegaflow` directory belongs to
   the project root. Generated recording state belongs to the configured
   `studio.data_dir`, defaults beneath `<recording_dir>/.omegaflow`, and is
   protected by a repository-owned ignore rule when it lies in a supported Git
   or Sapling worktree rather than being version-controlled. Already tracked
   state fails bootstrap without automatic index mutation.
...
17. `studio.recording_backend` and `studio.workload_backend` share the typed
    `host | reploy` value domain. Recording defaults to `reploy`, and
    `recording_backend=host` is schema-valid but produces a targeted capability
    error before preparation until a bare-metal controller is deliberately
    introduced; it has no maintained implementation or fallback.
18. `studio.workload_backend` defaults to `host` after capability routing.
    Until cutover, OmegaFlow preserves omission separately from the supplied
    `host | reploy` value domain: omission selects FIFO, explicit `host` fails,
    and explicit `reploy` requires a complete workload blueprint and never
