...
adapter first requires the `SIGCHLD` disposition to be the Bash default. It
redirects the `trap -p SIGCHLD` builtin to a pre-created adapter-private file
without command substitution or another child, and requires that file to remain
empty. A non-default `SIGCHLD` disposition is unsupported persistent state: it fails
initial launch, makes a gate return status 125 without spawning its requested
helper, or is fatal `adapter-state` at the next reached prompt boundary. Status
125 is an adapter refusal, never `gate_ready` or cancellation evidence. The
fatal-report path may temporarily ignore and then restore the
invalid disposition only to deliver its bounded diagnostic; no later source or
prompt readiness is claimed. V1 reserves this disposition because a
...
discard genuine workload-child events.
...
and disables inherited `DEBUG`/`RETURN` tracing only for adapter code. The
reserved-default `SIGCHLD` check above occurs before every normal helper spawn,
so an internal child exit cannot invoke workload code. An exec'd
...
the immutable prompt path, a non-default `SIGCHLD` disposition, or failure to
observe actual Readline entry emits fatal `adapter-state`. These take the
...
traps and `DEBUG`/`RETURN` tracing, default `SIGCHLD` validation before every
normal helper spawn, initial and later non-default `SIGCHLD` failure, late
