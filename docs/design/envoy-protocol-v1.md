...
| Envoy operation start | Envoy; starts after `execute.input_through` is satisfied and Envoy begins the private `execute` write | Within 5 seconds, either complete recoverable source rejection, including split-FIFO cleanup and the public `operation_failed` write, or complete private source validation and syntax preflight, `submit`, the fixed Readline trigger, source-helper delivery, the `PS0` preparation barrier, the pre-start output drain, `start_release`, `started`, the public `operation_started` write, and `started_ack` | Emit a best-effort fatal `operation-start-timeout` diagnostic, close the session channels, terminate and reap the controlled tree, and exit nonzero; the adapter may hold a parsed source unit, so no ordinary operation result or later operation is safe |
...
check proves the actual submitted unit. Awsh rechecks the binary digest before
each fork. A checker executes nothing and receives no selected-shell, PTY,
helper, Envoy, or controller descriptor. Its checker-only `-n` does not change
the live adapter's parser state. A nonzero result from either check produces
private `rejected` with code `source-syntax`; a reserved-name violation
produces `source-policy`. Both become a public pre-start
...
For `pty`, both FIFO fields are empty. For `split`, Envoy exclusively creates
mode-0600 FIFOs at
...
The source-loader function invokes only the absolute manifested Awsh helper,
captures its standard output into `READLINE_LINE`, requires and removes exactly
one final ASCII `x` marker, and sets `READLINE_POINT` to the resulting UTF-8
character count expected by the selected Bash/Readline build. The
helper appends that marker after the canonical frame so Bash command
substitution cannot strip source-significant trailing newlines; no byte follows
the marker. The helper writes no stderr or uncaptured stdout. The canonical
line is:
...
`HISTEXPAND` is `on` or `off`; and `EDITING_MODE` is `emacs` or `vi`. The two
...
The inner `RETURN` makes comments-only source a complete list while preserving
the preceding status. The assignment captures the inner group's result and the
...
`PS0` is the readonly, output-empty command substitution invoking the fixed
`start-prepared` helper. Bash expands it exactly once after accepting and
parsing the complete brace group and before executing `ENTER`. The helper
blocks, Awsh writes `start_prepared`, and Bash cannot execute any frame byte
until the complete `started_ack` sequence below releases it. More than one
`PS0` expansion, any `PS0` byte on the PTY, or source execution before release
is fatal.

The start commitment is ordered as follows:

1. Envoy satisfies `execute.input_through`, writes private `execute`, accepts
   `submit`, and writes the fixed Readline trigger.
...
6. Awsh sends the helper's `accepted` reply. The helper exits with empty
   output, `PS0` completes, and only then may Bash execute `ENTER` and the
   authored source.
...
Every helper invocation is the same fixed executable form:
...
Bash. `accepted` is also the sole successful `prompt_ready` reply. A helper
disconnect before its complete final reply is a fatal adapter failure.
...
heredocs, source and aggregate private/helper frame overflow, invalid UTF-8 and
NUL, the reserved adapter substring, canonical syntax rejection, and source
that would parse only through aliases, `extglob`, POSIX mode, history expansion,
or disabled `interactive_comments`. They prove that helper IPC carries the
exact source, the PTY receives
...
Persistent-state cases prove the preceding status and history/editing state are
restored before source, ordinary source status survives the canonical suffix,
