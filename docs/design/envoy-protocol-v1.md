...
same offset the pre-start failure rule above already requires.
...
It also removes every name beginning with `BASH_FUNC_` or `LD_`. Loader
variables go because the dynamic loader consumes them before Bash reads a single
flag, which would run application-controlled libraries inside the process that
holds the private descriptors. A delegated `LD_` value an application declares
is instead exported by the driver inside the established persistent shell, so
operation children still see it while the driver's own image never loads under
it. Environment names must be non-empty, contain neither `=` nor NUL, and values
cannot contain NUL. Other application values, including `PATH`, are delegated
unchanged after this control-plane baseline.
