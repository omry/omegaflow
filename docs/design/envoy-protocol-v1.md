...
tree. Because v1 never retains a process from an earlier operation, an adopted
...
session failure; the Envoy never continues with a partially cleaned tree.
...
failure fails the operation instead. The Envoy then drains output and emits
`operation_cancelled` with the shell status, normally 130. If the driver does
not return, the Envoy
...
after its bounded census, termination, reap, and drain sequence, fails the
...
unevaluable; and `operation-cleanup` for failure to census, terminate, reap,
reach EOF, or drain operation-created processes at any operation boundary.
