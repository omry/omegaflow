...
| Envoy operation start | Envoy; starts when the terminal-input barrier is satisfied and Envoy begins the private `execute` write | Complete Awsh source validation and framing, receive `submit`, drain legitimate pre-submission PTY output, write the complete terminal submission, suppress its echo/redraw, complete `start_prepared`/`start_release`, observe matching `started` while the exact Bash process is stopped beyond the helper critical section, write `operation_started`, and send `started_ack` within 5 seconds | Emit a best-effort fatal `operation-start-timeout` diagnostic, close the session channels, terminate and reap the selected-shell tree, and exit nonzero; `operation_started` is absent unless its complete write already committed, no terminal operation result is emitted, and the controller retains partial artifacts and asks Reploy to terminate |
| Awsh operation start | Awsh; starts when it accepts one complete private `execute` | Validate and frame source, prepare the source-barrier artifacts, complete helper acceptance and release, stop and report the exact Bash process, accept `started_ack`, and either resume Bash plus release the FIFO or accept the specified intervening prompt/shell result within one non-resetting 5-second budget | Write best-effort private `protocol_error` code `adapter-framing`, close helper and barrier resources, terminate and reap Bash, and exit nonzero; Envoy maps the failure to fatal `operation-start-timeout`, emits no terminal operation result, and preserves any already-committed `operation_started` event |
...
| Envoy operation cleanup | Envoy; starts when it accepts post-source `input_close`, a cancellation/finalization adapter return outside that handshake, or explicit `shell_exit` | Pin and temporarily exempt the exact blocked prompt helper when present; census, terminate, reap, reach operation-stream EOF, drain all other operation-created processes and output, then prove the helper exits and only Awsh/Bash remain, all within 5 seconds | Emit a best-effort fatal `operation-cleanup` diagnostic, close the Envoy-owned operation descriptors and session channels, and exit nonzero; no terminal operation result is emitted, and the controller fails the capture and asks Reploy to terminate |
...
Readline redraw. While the `PS0` helper remains blocked and after accepting
matching `start_prepared`, Envoy drains through `EAGAIN` once more, closes the
discard interval, and sends `start_release`. Awsh does not report matching
`started` until Bash has left that helper's ignored-signal critical section and
is stopped at the generated frame's private source barrier. Discarded bytes
never enter the raw log, terminal socket, output offset, or output mark. No
operation-created process exists during this serialized interval, so every PTY
byte in it is adapter submission presentation rather than operation output. The
controller sends authored operation input only after `operation_started`, and
Envoy does not forward it until the complete event write and matching
`started_ack` write have committed.
...
recording must preserve. Envoy therefore discards terminal input that arrives
while the session is idle. When an operation reaches its post-source prompt
boundary, Awsh keeps Bash blocked in the immutable `prompt_ready` helper and
requests `input_close`. Envoy serializes that request with its sole PTY-master
writer, stops forwarding controller input for the ended operation, and reads and
discards every later terminal-channel byte until another operation starts. It
acknowledges `input_closed` only after no master write remains in progress. Awsh
then flushes the slave input queue before it can release Bash into Readline, so
bytes already written to the line discipline but not consumed by the workload
cannot become an unframed idle command. Input a gated operation is still
entitled to receive is unaffected, since such an operation has not ended, and so
is anything the workload already read.
...
only processes left in the controlled tree. During post-source completion,
this boundary runs while Bash is still blocked in the matching
`prompt_ready` helper. Awsh reports that helper's peer PID and `/proc` start
time in `input_close`; Envoy opens a pidfd, verifies the same process identity,
current Bash ancestry, and live helper role, and temporarily exempts exactly
that pinned process from termination. It still terminates and reaps every other
operation descendant and drains every operation output source before replying
`input_closed`. An early helper exit, identity change, additional helper, or
remaining non-exempt descendant is fatal. After the acknowledgement, Awsh
releases the helper and requires its connection to close; Bash synchronously
reaps it and enters Readline, after which Awsh captures the next
`READLINE_ACTIVE` state and reports `completed`; Envoy then verifies the
pinned helper is gone and that only Awsh and Bash remain, without opening a new
cleanup window in which operation code can run. The operation-cleanup deadline
...
Inspection is the final part of an exclusive operation boundary. While Bash was
blocked in the post-source helper, Envoy completed universal operation cleanup
and output drain except for that pinned helper. After Awsh releases the helper,
observes `READLINE_ACTIVE`, and sends the result and resolved plan, Envoy proves
the helper is gone and only persistent Awsh/Bash remain. Only then may it
inspect or hash workload paths. It runs the complete resolved plan
...
an unacknowledged public cancellation. The first submission byte is the start
commit point: from there through the complete `start_prepared`/`start_release`
exchange and matching stopped-Bash `started`, Envoy finishes the
non-interruptible bounded submission transaction. A complete cancel or finalize
received on telemetry during that interval is queued but not accepted; after
Envoy emits `operation_started` and completes the matching `started_ack` write,
it immediately accepts that request and takes the ordinary running cancellation
or finalization path. A controller already in
...
mark. After terminal submission and matching `start_prepared`, Envoy closes the
discard interval with a drain through `EAGAIN` and sends `start_release`. Awsh
lets the helper close, waits for Bash to reach the command-free source barrier,
stops that exact Bash process, and only then reports matching `started`. Envoy
snapshots `output_start`, completes the `operation_started` write, sends
`started_ack`, and opens operation input. Awsh then resumes Bash and releases
the source barrier. Thus “every preceding byte” means every byte before the
explicitly excluded submission interval, not the private source echo/redraw
inside it. Failure to drain or write the legitimate preceding bytes fails the
operation before it opens a range; failure to complete any private start step
takes fatal `operation-start-timeout` instead.

`output_start` snapshots the raw-log offset at `operation_started`, and the
snapshot happens while Bash is stopped beyond the helper critical section
rather than after authored source is released. Awsh's `started` result is a
barrier, not a notification: if Envoy snapshotted after releasing it, a fast
...
the policy was supposed to withhold. The Bash `PS0` helper blocks through
`start_prepared`; after `start_release`, Bash leaves that helper, restores its
normal signal state, reaches the private source barrier, and is stopped by
Awsh. Awsh writes `started` and waits for Envoy's `started_ack` before resuming
Bash and releasing the barrier. Envoy takes the offset and completes the event
write before sending that acknowledgement, so neither operation output nor an
immediately event-driven Ctrl-C can precede the boundary at which Bash is ready
to receive it. An
...
`cancel` reply to a blocked authored gate helper, `settled` after accepting the
post-source `prompt_state`, throughout its matching `prompt_ready`/cleanup/
Readline-entry handshake, or after writing the operation result, and
...
interrupt succeeds. If signaling a sampled group fails with `ESRCH`, that proves
only that one foreground job vanished. Awsh emits no disposition, retains the
same cancellation action, processes any already-readable helper or shell
result, and reclassifies the operation. If no typed completion boundary has
won, it coalesces exactly one retry on a native monotonic event-loop timer no
sooner than one millisecond later. Awsh processes every then-ready helper,
result, and Bash-reap event before that retry; a typed boundary cancels the
pending retry. On a retry it performs another fresh `tcgetpgrp` lookup,
validates that positive group against the same controlled terminal/session and
active operation, and tries once. Another `ESRCH` schedules the same single
tick rather than spinning.
A later group is still part of this operation because Awsh accepts no next
`execute` until the current action and result close. The loop ends only when one
`SIGINT` succeeds, accepted prompt/result evidence makes the action `settled`,
another interruption makes it `already-interrupted`, or the original
non-resetting grace period takes its existing timeout path. It never turns
`ESRCH` alone into completion evidence. Envoy's existing grace timer remains
the sole public timeout and is never reset by a retry. Every other lookup or
signal failure is fatal. Envoy never selects a foreground group itself.
...
intent. If the persistent Bash adapter reaches post-source `input_close`, Envoy
completes the same universal process cleanup required by normal completion
while the prompt helper remains blocked; a cleanup failure takes the fatal
session-failure path instead. Awsh then releases the helper and reports the
status, and Envoy emits `operation_cancelled`, normally with status 130. If
...
A `cancel` can also arrive after Envoy accepts post-source `input_close`, while
it is completing mandatory operation cleanup, or later while it resolves the
inspection plan. There is nothing to signal in either phase: authored source
has returned, and during cleanup Bash remains blocked in the pinned prompt
helper. Envoy records cancellation, completes the matching private `settled`
disposition, and finishes the already-started cleanup under its existing
non-resetting five-second deadline; it neither signals Bash nor starts another
grace period. Cleanup
failure still takes the fatal session-failure path and emits no terminal
operation result. After successful cleanup, Awsh releases the helper and sends
`completed`; Envoy skips inspection and emits `operation_cancelled` with the
returned status, matching request reason, and no inspection results.
...
remaining operation-created process during the post-source `input_close`
handshake while the prompt helper remains blocked, supervises reap, drains
final output, emits any remaining split-stream evidence,
...
A post-source `input_close` before that timer expires enters mandatory cleanup;
after successful cleanup and the resulting Awsh `completed`, Envoy skips
inspection and emits `operation_cancelled` with the returned status and
cancellation reason. If the same timer expires first, the Envoy
...
and enters the `shell_ended` drain. If cancellation arrives after `input_close`
while mandatory cleanup is active, Envoy records it, completes the matching
private settled disposition without releasing the prompt helper, finishes
cleanup under its existing non-resetting deadline, and, after the helper is
released and `completed` arrives, skips inspection and emits
`operation_cancelled` with the returned status and no inspection results.
Cleanup failure remains fatal with no terminal operation result. Once an
inspection worker is running, the same serialized
...
A `finalize` can arrive after Envoy has accepted natural post-source
`input_close`, during mandatory cleanup or inspection. From that accepted
completion boundary through terminal-result commitment, the observed result
wins. During cleanup Envoy neither signals persistent Bash nor starts another
grace period; it finishes the existing cleanup deadline, takes the ordinary
fatal no-result path if cleanup fails, releases the pinned prompt helper, and
continues through the resulting `completed`, inspection, and normal completion.
During inspection it likewise leaves the worker running. The operation
completes with the status Awsh actually returned — the `Finalizing --> Idle`
completion edge exists
...
signal normally. It follows the same all-real-signal boundary rule below rather
than a resize-only exception.
...
awsh-v1, input_close, OPERATION_ID, PROMPT_HELPER_PID, PROMPT_HELPER_START_TICKS
...
`PROMPT_HELPER_PID` and `PROMPT_HELPER_START_TICKS` are nonzero canonical
decimal integers naming the accepted post-source `prompt_ready` peer and field
22 of its `/proc/PID/stat` identity. Awsh obtains both while retaining the live
helper connection. Envoy opens a pidfd and requires the same start time, current
Bash ancestry, and controlled-tree membership before exempting that exact
identity during completion cleanup; a PID number without those checks is never
an exemption.

`start_prepared` reports that Awsh has accepted the matching Bash `start`,
validated the frame-entry marker and entry-sentinel termios state, restored and
verified the workload termios snapshot, and kept the `PS0` helper blocked.
Envoy accepts it only during the matching committed submission. Envoy then
closes the submission-discard interval before sending `start_release`; the
release is not a public start and does not open operation input. Awsh accepts
`start_release` only after its matching `start_prepared`, replies to and closes
the helper, and waits for Bash to reach the private source barrier described
below. It sends `started` only after it has stopped the exact Bash PID at that
barrier. `started_ack` then authorizes Awsh to resume Bash and release the
barrier. A duplicate, mismatched, reversed, or out-of-state member of this
four-frame transaction is fatal private protocol failure.
...
`input_close` is Awsh's post-source proposal to end terminal input for the
matching active operation. Awsh writes it only after accepting that operation's
post-source `prompt_state` and matching `prompt_ready`, and it keeps the helper
blocked. Envoy serializes the proposal with its sole PTY-master writer,
transitions that operation's input gate permanently closed, completes any master
write already in progress, and thereafter reads and discards terminal-channel
bytes as idle input. It then pins and verifies the named helper, performs the
universal operation cleanup while exempting only that still-blocked helper,
drains all operation output sources, and proves no other operation descendant
remains. Only then may it send matching `input_closed`. Awsh accepts that
acknowledgement only in this completion handshake, successfully calls `tcflush`
with `TCIFLUSH` on its retained control-only slave, and only afterward applies
the Readline entry sentinel and replies to the helper. This ordering proves
that the flush covers every byte Envoy could have written, that no later byte
from the ended operation can reach Bash, and that no operation descendant can
mutate termios after the next active-state snapshot. The handshake is not used at
startup and does not create a public event. A duplicate, mismatched, or out-of-state frame,
an invalid or prematurely exited helper identity, an Envoy writer-quiescence or
cleanup failure, a failed flush, or deadline expiry is fatal;
crossed cancellation or finalization remains `settled` and never reopens input.
...
the stopped-Bash source-barrier release, or fatal termios cleanup is active,
Awsh retains prepare until that transaction
...
helper packet. `ESRCH` from signaling a sampled group is the sole non-fatal
signal error, but it produces no disposition: Awsh processes any accepted
completion evidence and otherwise coalesces one native monotonic event-loop
retry no sooner than one millisecond later. Every ready helper/result/reap event
wins before that retry; if none settles the action, Awsh performs one fresh
validated `tcgetpgrp`/`killpg` attempt for the same active operation. Repeated
`ESRCH` schedules another single tick rather than busy-spinning, and never
resets Envoy's original grace timer.
...
adapter first validates its generated command-resolution inventory. The
manifested rcfile and Bash-build table name every command word used by any
immutable hook after workload source can run, including adapter functions, the
fixed helper path, and required Bash builtins such as `builtin`, `bind`, `trap`,
`enable`, `readonly`, `unset`, `set`, `shopt`, `type`, and `umask`. Adapter function
names resolve only to their readonly manifested definitions. Every other name
in that inventory is forbidden as a workload function or alias key, and every
required builtin must remain enabled and resolve as that builtin; unrelated
functions, aliases, and builtin enablement remain workload state.

The immutable caller invokes the manifested readonly guard function through a
syntactically non-alias-expanded function word and passes exactly two arguments
expanded directly from the current readonly `BASHOPTS` and `SHELLOPTS`. Function
positional parameters are invocation-scoped, so those snapshots overwrite no
workload scalar and do not change the caller's positional parameters. The guard
rejects any other arity with command-free parameter and conditional syntax. It
then requires `POSIXLY_CORRECT` absent and the reserved `posix` option disabled
before it enters temporary POSIX lookup mode. That mode lets still-enabled
special builtins take precedence over functions. The guard rejects every
reserved function or alias collision and requires positive canonical
`type`/`enable` evidence for every required builtin; absence of output is never
success. The generated checks do not invoke `builtin` until they have positively
established that the name has no function or alias collision and resolves to
the enabled builtin. On success the now-trusted `set`, `unset`, and `shopt`
builtins restore non-POSIX mode, remove the temporary `POSIXLY_CORRECT`
assignment, and restore `inherit_errexit` to the state encoded in its first
argument; the guard then requires the complete current option strings to equal
its two arguments before any helper spawn. This exact fixed-build transition is required
to have no other option side effect. Initial failure prevents `ready`; a gate
refuses status 125; a reached prompt or frame boundary is fatal `adapter-state`.
A workload that disables or deliberately impersonates a guard root is
unsupported persistent state with the same boundary-interception and
owning-timeout limits as invalid tracing or signal traps; it is never classified
as preserved state.

The adapter then requires `xtrace` to be disabled, `ERR`, `DEBUG`, and `RETURN`
trap definitions to be empty, and every catchable real-signal trap disposition
to be the Bash default; `EXIT` is the sole persistent trap. The generated Bash
build table carries the exact canonical `trap -l` output and the derived closed
ordered catchable real-signal inventory for that build. During startup the
adapter redirects the `trap -l` builtin to a pre-created private file and
requires an exact match before it reports state. At every boundary it checks the
shell option flag directly and redirects `trap -p` for every reserved trap to
pre-created adapter-private files, without command substitution or another
child, and requires every file to remain empty. Missing, additional, or renamed
signals therefore make the Bash build unsupported. A trap-state mismatch is
unsupported persistent state: it fails initial
...
fatal-report path never saves, suppresses, invokes, or restores invalid workload
traps, and it never evaluates a workload-controlled `PS4` to make `xtrace` safe.
Because Bash can trace a boundary command or dispatch a pending signal before
that command checks the state, invalid tracing or signal-trap state may emit
output, mutate shell state, run `PS4` substitutions, or prevent the check or
report; that unsupported path has no adapter-transparency promise and the owning
deadline path applies. No later source or prompt readiness is claimed. V1
reserves every asynchronous Bash signal trap because any one can invalidate
already captured cwd, environment, history, or termios state. `SIGCHLD` also
protects helper exits, and `SIGWINCH` protects resize handshakes. Foreground
applications remain free to install their own signal handlers.
...
`prompt_state`. At a post-source boundary it first completes the private
`input_close`/`input_closed` exchange and successfully flushes the slave input
queue with `TCIFLUSH`; startup has no operation input and skips those steps. Awsh
then derives `ENTRY_SENTINEL` by setting `ICANON` and `ECHO` in that snapshot
without changing any other field, applies it with `tcsetattr(TCSANOW)`, and
verifies an exact read-back. Only then does Awsh reply `accepted`. After the
helper exits, Bash can enter Readline. Envoy writes no PTY input and Awsh accepts
no `execute` until `tcgetattr` observes both sentinel bits cleared, as the fixed
Bash/Readline build is required to do on entry, and captures that complete state
as `READLINE_ACTIVE`. This sentinel transition, not terminal output or helper
closure, proves that Bash is blocked in Readline. The non-resetting five-second
Readline-entry deadline begins when Awsh accepts the `prompt_ready` packet and
covers any required input-close exchange and flush, sentinel apply/read-back,
acknowledgement, helper closure, and transition observation. Failure is
`shell-launch` initially or fatal `adapter-state` later; the outer startup or
operation deadline may expire first.

At a post-source boundary, accepting `prompt_state` captures the candidate
operation status, cwd, and resolved inspection plan but does not write private
`completed`. Awsh keeps that operation in its completion handshake, classifies
a crossed cancel or finalize as `settled`, and withholds `completed` until Envoy
has closed the operation input gate, pinned the matching `prompt_ready` helper,
completed universal operation cleanup while that helper remains blocked, and
drained every operation output source; Awsh then flushes the slave input queue,
replies to the helper, requires it to close, and observes the resulting
`READLINE_ACTIVE` transition. Bash cannot leave the
prompt hook and enter Readline until it has synchronously reaped that external
helper. The observed transition therefore proves that the exempt helper is
gone, and Envoy's final census proves that only Awsh and Bash remain. The
immutable primary-prompt hook contributes no
display bytes: after its `prompt_ready` command substitution closes, the `PS1`
value passed to Readline is empty. The wait through `READLINE_ACTIVE` therefore
cannot append a shell-rendered prompt to the operation's raw-output range; the
following visible prompt remains controller-synthesized presentation. A missing
packet, helper close, or transition is fatal
`adapter-state` under the existing non-resetting Readline-entry, operation-
cleanup, and outer operation deadlines; no private or public terminal operation
result is emitted.
...
only for that armed operation. Awsh sends `start_prepared` while keeping the
helper blocked, replies `start` only after Envoy's matching `start_release`, and
then holds the generated frame at its source barrier until the later
`started`/`started_ack` exchange completes. `gate` is accepted only for the
active operation and authored gate and blocks for `continue` or `cancel`.
`gate_interrupt` is accepted at most
...
adapter critical section. Before spawning the helper, Bash requires the complete
reserved trap state above, temporarily ignores `SIGINT`, `SIGQUIT`, and
`SIGTSTP`, and restores all three default dispositions after the helper closes.
V1 does not attempt
to suppress tracing or workload traps from inside an adapter function, because
`xtrace`, `DEBUG`, or a pending signal can act before that function's first
check. The reserved-disabled `xtrace`, reserved-unset `ERR`/`DEBUG`/`RETURN`, and
reserved-default real-signal checks occur before every normal helper spawn, so a
supported boundary's internal command or child exit cannot invoke workload
tracing or signal-trap code. An exec'd
non-gate helper inherits and verifies all three ignored dispositions before it
exchanges a packet. A gate helper inherits and verifies ignored `SIGQUIT` and
`SIGTSTP`, replaces only inherited ignored `SIGINT` with its handler before
sending `gate`, and translates one such signal into the optional
`gate_interrupt`. If the signal is recorded before the initial packet
...
never used to bypass that public ordering. Terminal Ctrl-Z therefore cannot
stop a blocking helper or let Bash continue past an unresolved helper
handshake; it has no private or public typed outcome. After the helper has
closed, Bash restores the default `SIGINT`/`SIGQUIT`/`SIGTSTP` dispositions
before accepting operation source or prompt input. For `start`, the generated frame is one
top-level outer conditional whose command-free redirection condition is the
source barrier below; no authored command can execute between helper closure
and that barrier. A signal that
...
start, gate, prompt-state, or prompt-readiness helper or erase its report,
terminal `SIGTSTP` cannot stop or bypass one, and terminal Ctrl-C at a steady
gate still makes the gate function return 130.
...
Before arming the immutable Bash hooks, Awsh creates two operation-private
source-barrier artifacts in its mode-0700 Bash runtime directory: one
mode-0600 regular sentinel containing one byte, opened and retained by Awsh with
its exact device and inode recorded, and one mode-0600 FIFO with no writer.
Both pathnames use only the fixed runtime prefix and validated operation ID and
are emitted as canonical shell-quoted literals. Awsh requires each path absent
before creation and validates type, owner, mode, and link count. These are
cooperative synchronization artifacts, not a security boundary.
...
```text
if >| SOURCE_BARRIER_SENTINEL < SOURCE_BARRIER_FIFO; then
    ...
    __awsh_restore_input_state HISTEXPAND "$?"
fi
```

The complete generated submission excluding those two source copies is at most
32,768 bytes, including the nested control syntax, source-barrier redirection,
failure-state restoration, and both copies of bounded split-stream redirection
text. Therefore the
...
After writing `submit` and before Envoy writes the first terminal-submission
byte, Awsh may accept private `cancel`. It requires the operation-correlated
frame-entry marker to remain absent, clears the operation arming, restores the
complete termios state to the exact retained `READLINE_ACTIVE` value, and
requires an exact read-back before returning `disposition(cancel, disarmed)`.
Any failed absence check, state change, or read-back is fatal
`adapter-framing`, never `disarmed`. Bash remains blocked in the same Readline
entry, with the prompt boundary's captured `HISTEXPAND` and `STATUS` retained
for the next accepted `execute`; the adapter-idle history-expansion suppression
is not workload-visible because Envoy discards idle terminal input and the
controller sends authored input only after `operation_started`. The next
generated frame restores that retained `HISTEXPAND` before its authored source
executes. Awsh accepts no other private request or helper transition inside this
serialized rollback.

The immutable readonly `__awsh_restore_input_state` function accepts only the
literal validated `HISTEXPAND` supplied by Awsh and one decimal status operand.
The normal source path supplies the literal validated `STATUS` from Awsh; the
outer barrier-failure branch supplies the immediately preceding redirection
condition's exact `$?` without an intervening command. The function restores
the workload's history-expansion setting and returns exactly that status. The
preceding prompt boundary requires `ERR`, `DEBUG`, and `RETURN`
traps to be unset, every real-signal trap to be default, and `xtrace` to be
disabled before this generated function can be dispatched; the function does
not claim that it can suppress tracing or a pending signal before its
own invocation. Its exact name is the forbidden alias key above. Running it as
the inner `if` condition prevents a
...
unselected byte-identical copy is parsed but never expanded or executed. If the
outer barrier condition fails, its `else` branch restores `HISTEXPAND`, returns
that failure status, and runs no authored source. Normal Bash `errexit` behavior
therefore applies to that adapter-command failure just as it would to another
failed top-level command.
...
adapter critical section with `xtrace` still disabled, `ERR`, `DEBUG`, and
`RETURN` traps still unset, and every real-signal trap still default. Its
frame-entry
...
read-back before it reports `start_prepared`. The frame-entry marker is not a
telemetry report and source receives no path contract.

Envoy drains legitimate preceding PTY output, opens the submission-discard
interval, writes the complete sequence, and waits for Awsh's `start_prepared`.
The Bash `PS0` path has already created the valid frame-entry marker when its
helper sends `start`, and that helper remains blocked. After accepting
`start_prepared`, Envoy drains the remaining redraw through `EAGAIN`, closes the
discard interval, and sends `start_release` without emitting a public event or
opening operation input. Awsh replies to the helper and requires its connection
to close. Bash synchronously reaps that helper, restores default
`SIGINT`/`SIGQUIT`/`SIGTSTP`, and then evaluates the generated frame's outer
redirection condition. Redirections are applied left to right: Bash first
force-truncates the retained source-barrier sentinel with `>|`, preserving a
workload's supported `noclobber` setting, and then blocks opening the FIFO for
reading because Awsh has not opened a writer. No alias, function, builtin,
executable, or `PATH` entry is resolved.

Awsh observes the retained sentinel descriptor reach size zero and requires the
path still to name the recorded mode-0600 regular inode with the expected owner
and one link. It sends `SIGSTOP` to the exact selected Bash PID and accepts only
a `waitpid(..., WUNTRACED)` report for that PID stopped by `SIGSTOP`. Because
the FIFO has no writer, Bash cannot reach input-state restoration or authored
source whether the stop lands between the redirections or during the FIFO open.
Only then does Awsh report `started`. Envoy commits `output_start`, completes
the `operation_started` write, sends `started_ack`, and opens operation input.
Awsh requires `SIGCONT` of the exact stopped Bash to succeed and retries a
nonblocking writer open on the validated FIFO until Bash's reader is present;
after a successful rendezvous it closes the writer, so the outer condition
returns zero. The immutable inner frame condition then restores `HISTEXPAND`
and returns `STATUS`, and only then can the selected authored branch execute.

Terminal SIGINT or an accepted running cancellation/finalization may win after
the public event but before the FIFO rendezvous. The signal is delivered or
left pending against Bash's restored default state, never discarded by the old
helper critical section. If Bash consequently abandons the barrier, Awsh stops
the writer-open retries when it observes the matching prompt boundary or exact
shell exit and reports the ordinary active-operation status, including status
130 for terminal Ctrl-C.
The outer failure branch restores `HISTEXPAND` and preserves that exact status;
with `errexit` disabled Bash reaches the matching prompt with status 130, while
with `errexit` enabled normal Bash behavior exits the shell with status 130.
The lifecycle request still uses its normal serialized disposition. An
unexpected Bash state, stop reason, sentinel/FIFO change, signal failure, or
failure to rendezvous or reach a valid result inside the non-resetting start
bound is fatal. If Readline
...
Awsh removes the frame-entry marker and both source-barrier artifacts at the
operation boundary.
...
After an Awsh result — which for ordinary completion is withheld through input
closure and slave-queue flush, the post-source `prompt_ready` helper close, and
the observed `READLINE_ACTIVE` transition — Envoy performs mandatory descendant
cleanup, drains the PTY master
through the result boundary, closes its keepers, drains both FIFO
...
distinct process group `shell_pid`, duplicates the slave onto file descriptors
0, 1, and 2, and closes every descriptor except those three and its two barrier
ends. It reports that setup through the barrier and waits.
...
the child. In the child-only, async-signal-safe pre-exec path, raw syscalls set
an empty signal mask and reset every catchable real signal in the generated
Bash-build inventory to `SIG_DFL`, including `SIGINT`, `SIGQUIT`, `SIGTTOU`,
`SIGTTIN`, and `SIGTSTP`. Only then does the child close the barrier descriptors
and exec interactive Bash. No inherited ignored or blocked signal therefore
becomes persistent Bash state, while Awsh's own `SIGTTOU` disposition remains
unchanged. Bash is therefore the initial foreground process-group leader inside
the Awsh-owned controlling-terminal session and can perform normal job control.
...
| Indexed and associative arrays, functions, aliases, and positional parameters | Arrays, non-reserved functions, and positional parameters are preserved for ordinary expansion and execution. Aliases are preserved only while every `BASH_ALIASES` value satisfies the closed simple-alias grammar and its key is outside the generated adapter command-resolution inventory. Grammar-bearing aliases or reserved function/alias names are unsupported persistent state; an invalid initial table fails launch and a later invalid transition is fatal `adapter-state` at the reached boundary, never silently reset. Fresh-Bash preflight remains alias-blind because every supported alias expansion is parser-neutral. Source rejected in preflight never reaches persistent Bash. |
| Command resolution and builtin enablement | The generated adapter inventory reserves every immutable-hook command name and the enabled state of every builtin it requires. Adapter functions remain their readonly manifested definitions; other reserved names cannot be workload functions or alias keys. Required builtins remain enabled and resolve as builtins. Unrelated function names, aliases, and builtin enablement persist. The immutable caller passes exact option snapshots as two invocation-scoped guard arguments through a non-alias-expanded function word; no workload scalar or caller positional parameter is scratch storage. An invalid initial state fails launch, a gate refuses 125, and a reached prompt or frame boundary is fatal `adapter-state`; the adapter never deletes a workload function, rewrites an alias, or silently re-enables a builtin. Deliberate guard-root impersonation is outside the cooperative threat boundary and has the same no-transparency/owning-timeout handling as a boundary-intercepting trap. |
...
| User traps | `EXIT` is the sole preserved trap. `ERR`, `DEBUG`, and `RETURN` must remain unset, and every catchable real-signal trap in the generated build inventory must remain at Bash default at every adapter boundary. Trusted source may install any of those traps transiently only if it restores the required state before calling a gate or returning; foreground applications may install their own handlers. A remaining trap is unsupported persistent state: startup mismatch fails launch, a gate refuses its helper with status 125, and a reached prompt boundary is fatal `adapter-state` with no terminal operation result. Because `DEBUG`, tracing, or a pending signal may act before the boundary check, the violating path has no transparency or preservation promise and may instead end the shell or reach cancellation timeout. Immutable helpers temporarily ignore `SIGINT`/`SIGQUIT`/`SIGTSTP` inside their signal-safe critical section and restore default before operation source or prompt input. `EXIT` remains workload-owned and shell status still comes from Awsh's reap. |
...
| Terminal modes and pending input | The workload-visible Linux termios state is preserved. At each `prompt_state`, Awsh snapshots it, applies and verifies `ENTRY_SENTINEL` while the blocking `prompt_ready` helper prevents Readline entry, and acknowledges that helper only afterward. At a post-source boundary it first makes Envoy close and quiesce the operation input gate, pin the exact blocked helper, clean and reap every other operation descendant, and drain operation output; only then does Awsh flush the retained slave with `TCIFLUSH` and release the helper. Unconsumed operation bytes cannot reach Readline, and no surviving operation child can mutate termios after the next snapshot; startup skips this operation-only cleanup. Clearing both sentinel bits proves actual Readline entry and defines `READLINE_ACTIVE`; no input or `execute` is accepted sooner. Injection requires that exact active state and applies the closed byte-transparent delta. Normal Readline return must restore `ENTRY_SENTINEL`, after which Awsh restores and verifies the workload snapshot before accepting `start`. A mismatch is fatal `adapter-framing`; no adapter mode reaches authored source. |
...
A source that leaves `xtrace` enabled, `ERR`/`DEBUG`/`RETURN` installed, or any
real-signal trap non-default may run workload code before the immutable
prompt-boundary check, including `PS4` substitutions or a pending trap before
the helper that would report completion. Awsh
...
pending ordinary signal trap and a `DEBUG` trap that suppress the boundary, and
`xtrace` with a side-effecting `PS4` that intercepts it, including cancellation
timeout. `extdebug` remains
...
the immutable prompt path, enabled boundary `xtrace`, any invalid reserved trap
state, or failure to observe actual Readline entry emits
...
process-group launch, an empty inherited signal mask and default dispositions
for the complete generated catchable real-signal inventory in Bash, and legal
...
`submit`/`start_prepared`/`start_release`/`started`/`started_ack` ordering; the
five-second operation-start deadline; the exact retained-sentinel truncation,
FIFO hold, Bash stop observation, continuation, and normal or interrupted FIFO
release; every `disposition` request-kind/phase combination,
...
ordering, post-source `input_close`/`input_closed` writer quiescence, the exact
prompt helper present and pinned when Envoy begins operation cleanup,
slave-queue flush before helper release, `completed` withheld until the matching
helper has closed and `READLINE_ACTIVE` is observed, and the final Awsh/Bash-only
census after `completed` but before inspection or a public result; an output-empty primary-prompt hook with no real prompt
bytes admitted before the delayed result, exact ignore/verify/default-restore
behavior for `SIGINT`/`SIGQUIT`/`SIGTSTP`,
reserved-disabled `xtrace`, reserved-unset `ERR`/`DEBUG`/`RETURN`, default
dispositions for the complete generated real-signal inventory before every
normal helper spawn, initial and later invalid reserved tracing state, late
Ctrl-C during each helper mode, Ctrl-Z unable to stop or bypass the start,
gate, startup prompt-ready, or post-source prompt-ready helper, and a following
ordinary foreground child receiving default `SIGTSTP` and stopping normally
after each critical section, endpoint
...
sides of that interval retain exact order. An immediate Ctrl-C sent after
observing `operation_started` must be delivered against Bash's restored signal
state, produce the ordinary active-operation status-130 path when it wins before
source, and never disappear inside the earlier `PS0` helper critical section.
...
post-`submit` cancellation before byte zero restoring exact `READLINE_ACTIVE`,
retaining the prompt snapshot, and clearing arming only after proving the frame
marker absent, plus fatal handling for every failed rollback step. A following
operation must restore the retained history setting before authored source.
With persistent `histexpand` both on and off, authored
...
before the first authored command. The corpus freezes the one-top-level-command
frame, its command-free outer barrier condition and two byte-identical inner
branches, and proves exactly one `PS0` expansion plus both saved-status paths,
including saved status 1 with `errexit` enabled after `! true`; only the
selected branch executes and authored failures retain normal `errexit`
behavior. Barrier failure runs no authored source, restores history state, and
preserves status 130 at the prompt with `errexit` disabled or as shell-exit
status with it enabled. Both values of `noclobber` complete the barrier without changing the
workload setting, and workload functions or disabled builtins named `kill`,
`builtin`, or `command` cannot affect it. Wrong sentinel inode, metadata, link
count, or content transition; wrong stopped PID or signal; failed stop,
continuation, or FIFO rendezvous; duplicate or crossed private frames;
unexpected Bash exit; queued cancel/finalize; resize deferral; and timeout each
have focused fail-closed or specified crossing coverage. Whitespace- and comment-only
...
empty values, assignment first words, every generated reserved command key,
and corrupted `BASH_ALIASES`. Command-resolution cases independently define a
function or alias for each reserved adapter word, disable each required builtin,
and combine shadowing with empty-success output; none may admit a normal helper
packet. They also prove every non-reserved function/alias name and unrelated
builtin enablement persists, both initially enabled and disabled
`inherit_errexit` survive the temporary POSIX guard, and the complete
`BASHOPTS` and `SHELLOPTS` strings are identical before and after every
successful guard. They set likely adapter-scratch scalar names and caller
positional parameters before every boundary and prove the exact values remain,
and reject wrong guard arity without normal helper traffic. Remaining
persistent-state cases cover
cwd/environment/arrays/functions/aliases/
...
selected-group-exit race in which `killpg` returns `ESRCH` while Bash advances
from `sleep .01` to `sleep 100`, a fresh validated lookup and exactly one
successful signal to the later same-operation group, repeated `ESRCH` using one
coalesced one-millisecond-or-later native timer tick without busy-spin, ready
helper/result/reap evidence winning before every retry, and eventual typed
prompt/result settlement or the original unchanged grace timeout, crossed
completion-helper/result ordering, both serialized result winners, and the
no-second-interruption crossed-cancel rule.
...
Completion-input cases make a command such as `read -n 1` receive a multi-byte
authored text step and leave the suffix unread. They cover a PTY-master write
already in progress when Awsh proposes `input_close`, terminal bytes reaching
Envoy after it sends `input_closed`, and the next operation. Envoy must finish
the crossing write before acknowledgement, discard every later byte, pin the
exact blocked helper by PID/start-time/pidfd identity, clean and reap every
other operation descendant, and drain every operation output source. Awsh must
then flush the slave queue before releasing `prompt_ready`; no suffix may execute
as an idle command, make Readline miss its sentinel transition, or contaminate
the next operation. A background child that ignores `SIGTTOU` and mutates
termios is killed before the active snapshot; early helper exit, PID reuse,
wrong ancestry, an additional helper, duplicate or mismatched private frames,
cleanup or `tcflush` failure, and deadline expiry take the fatal path with no
