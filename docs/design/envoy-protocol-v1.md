...
guess. Both rejection codes map to an empty-range public `operation_failed`
...
is the complete v1 classification mechanism. The Readline bracketed-paste-begin
binding and the bracketed-paste terminator bytes in source are reserved
separately. The immutable frame-entry hook and its redirection-only marker at
the start of every framed brace group detect mutation of the binding: if Bash
returns to a prompt without entering that hook and producing its valid marker,
Awsh reports fatal `adapter-framing` and Envoy ends the session without an
operation result.
...
| Indexed and associative arrays, functions, aliases, and positional parameters | Preserved for ordinary expansion and execution. Fresh-Bash preflight cannot import persistent aliases and maps every parse rejection conservatively to `source-unsupported`, including source whose grammar is valid only after expanding a syntax-bearing alias and genuinely malformed source; ordinary command-position aliases whose unexpanded source is valid keep their persistent Bash effect. Functions retain their Bash execution effect. Source rejected in preflight never reaches persistent Bash. |
...
Bash lexical context, persistent
