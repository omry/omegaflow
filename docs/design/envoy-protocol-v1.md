...
has not ended, and so is anything the workload already read. Without that drain
a write racing its own operation's terminal result would stay queued and be read
by a later operation, which would then run or answer with input authored for a
step that is over.
