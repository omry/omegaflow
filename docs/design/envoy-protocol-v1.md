...
one, which is what lets the golden fixtures freeze one shell-exit sequence.
...
rather than clamped is a resize applied during a presentation-timed operation,
which is placed on that operation's authored schedule as described under Resize;
nothing else is retimed. Sender-stamped realtime output never triggers this,
since marks are already non-decreasing. The rule guards the seam where an
authored schedule hands back to sender time, which is the only place the two
clocks meet.
