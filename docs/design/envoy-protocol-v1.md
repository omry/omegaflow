...
interrupt succeeds. Envoy never performs a later foreground-group lookup.
...
foreground process group. Envoy does not send a second signal.
...
helper packet. A failed lookup or signal is fatal private failure, never a
successful disposition or a retry against a later group. For `gate-cancelled` it
...
After arming the immutable Bash hooks, Awsh requires the operation-correlated
frame-entry path to be absent, frames one brace group whose first element is the
immutable frame-entry hook, and returns the complete NUL-free UTF-8 Readline
sequence in `submit`. Inside the adapter critical section, with inherited
tracing suppressed, that hook runs the marker's redirection-only creation in a
subshell with the Bash builtin `umask 077`. The parent shell's exact umask is
therefore unchanged, while the absent marker is atomically created as one
mode-0600 regular file in Awsh's private Bash runtime directory. Awsh accepts
frame entry only after validating that type, owner, and mode. The marker is not
a telemetry report and source receives no path contract.
...
`PROMPT_COMMAND`, the frame-entry hook, and the gate hook, then makes the
adapter-owned names readonly without reading terminal input or any application
file. At Bash's first prompt,
...
are reserved separately. The immutable frame-entry hook and its redirection-only
marker at the start of every framed brace group detect mutation of the binding:
if Bash returns to a prompt without entering that hook and producing its valid
marker, Awsh reports fatal `adapter-framing` and Envoy ends the session without
an operation result.
...
`tcgetsid(slave)` and `tcgetpgrp(slave)` against that PID before sending private
`ready`. It retains one close-on-exec, control-only slave descriptor until Bash
is reaped; Awsh never reads or writes terminal bytes through it and uses it only
for the documented terminal identity checks and atomic cancellation lookup.
Failure at any step closes the helper
...
| Indexed and associative arrays, functions, aliases, and positional parameters | Preserved for ordinary expansion and execution. Fresh-Bash preflight cannot import persistent aliases, so source whose grammar is valid only after expanding a syntax-bearing alias is `source-unsupported`; ordinary command-position aliases whose unexpanded source is valid keep their persistent Bash effect. Functions retain their Bash execution effect. Source rejected in preflight never reaches persistent Bash. |
| Process file-creation mask (`umask`) | Preserved with normal Bash semantics. The frame-entry hook performs its mode-0600 marker creation in a tracing-suppressed subshell with the Bash builtin `umask 077`; it neither reads nor changes the parent shell's mask. |
| User traps, including `EXIT`, `ERR`, `DEBUG`, and `RETURN` | Preserved while the unsupported `extdebug` option remains disabled; the adapter installs no persistent workload-visible trap. Immutable helper functions suppress inherited debug/return tracing and temporarily ignore `SIGINT`/`SIGQUIT` only inside the signal-safe adapter critical section, then restore the exact user definitions and tracing state before operation source or prompt input. `EXIT` therefore remains workload-owned and shell status still comes from Awsh's reap. V1 makes no trap-persistence promise after source enables `extdebug`. |
...
| Adapter hooks and helper identity | `PS0`, `PS1`, `PS2`, `PROMPT_COMMAND`, the frame-entry and gate functions, fixed helper executable/modes, and literal helper socket argument are immutable/reserved. An attempted direct mutation fails as readonly; indirect corruption or a missing hook report is fatal. |
...
`extdebug` can make a nonzero workload `DEBUG` trap skip the following command,
including the immutable `PROMPT_COMMAND` helper that would otherwise validate
the boundary. If that combination suppresses the helper, Awsh retains the
operation as active because it has no completion evidence. The recording
plan's operation deadline therefore follows the ordinary typed-cancel path; if
the adapter still cannot return, the cancellation grace period ends in the
existing `cancel-timeout` session failure. Envoy never emits
`operation_completed` or claims post-operation shell state for that unsupported
transition. The B2 corpus covers an initially enabled `extdebug` launch
failure, an enabled value observed at a reached boundary, and `extdebug` with a
nonzero `DEBUG` trap suppressing the boundary through cancellation timeout.
...
performed only for `signal`, typed gate cancellation without a signal, crossed completion-
