...
then may it inspect or hash workload paths. Cleanup or drain failure emits no
inspection results. The Envoy emits
...
If `cancel` arrives during inspection, the Envoy abandons the inspection
immediately and emits the same `operation_cancelled` result. That is how the
event is already shaped, and it follows the rule that cancellation invalidates
assertions rather than evaluating them: without it, an operation the controller
had bounded could keep hashing up to 16 GiB after its deadline had passed.
