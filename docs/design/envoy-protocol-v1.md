...
post-source `prompt_state`, throughout its matching `prompt_ready`/Readline-entry
handshake, after writing the operation result, or after observing that the
...
awsh-v1, input_close, OPERATION_ID
...
bytes as idle input. Only then may it send matching `input_closed`. Awsh accepts
that acknowledgement only in this completion handshake, successfully calls
`tcflush` with `TCIFLUSH` on its retained control-only slave, and only afterward
applies the Readline entry sentinel and replies to the helper. This ordering
proves that the flush covers every byte Envoy could have written and that no
later byte from the ended operation can reach Bash. The handshake is not used at
startup and does not create a public event. A duplicate, mismatched, or out-of-state frame,
an Envoy writer-quiescence failure, a failed flush, or deadline expiry is fatal;
...
adapter first requires `xtrace` to be disabled, `ERR`, `DEBUG`, and `RETURN`
...
has closed the operation input gate, Awsh has flushed the slave input queue, the
matching `prompt_ready` helper has received its reply and closed, and Awsh has
observed the resulting `READLINE_ACTIVE` transition. Bash cannot leave the
prompt hook and enter Readline until it has synchronously reaped that external
helper, so the observed transition also proves that no prompt helper remains
for Envoy's operation census. The immutable primary-prompt hook contributes no
...
`adapter-state` under the existing non-resetting Readline-entry and outer
operation deadlines; no private or public terminal operation result is emitted.
...
ordering, post-source `input_close`/`input_closed` writer quiescence and
slave-queue flush before helper release, `completed` withheld until the matching
prompt helper has closed and `READLINE_ACTIVE` is observed, no prompt helper
present when Envoy begins operation cleanup, an output-empty primary-prompt hook with no real prompt
...
the crossing write before acknowledgement, discard every later byte, and Awsh
must flush the slave queue before releasing `prompt_ready`; no suffix may execute
as an idle command, make Readline miss its sentinel transition, or contaminate
the next operation. Duplicate or mismatched private frames, `tcflush` failure,
and deadline expiry take the fatal path with no terminal operation result.
