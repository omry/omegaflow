...
| Envoy operation cleanup | Envoy; starts when it accepts matching `input_close` for ordinary return, or when it begins mandatory cleanup for cancellation, finalization, or shell-exit handling | For every cleanup trigger, complete census, termination, reap, operation-pipe EOF, and operation-output drain within 5 seconds. For ordinary return only, that same deadline continues through sending `input_closed`, Bash wait-record, empty-job-table, adapter, and completion-side Readline readiness checks, receiving `completed`, proving the final census, performing the final PTY drain and output-through barrier, and running workload inspection. | Emit a best-effort fatal `operation-cleanup` diagnostic, close the Envoy-owned operation descriptors and session channels, and exit nonzero; no terminal operation result is emitted, and the controller fails the capture and asks Reploy to terminate |
...
dynamically unload an adapter-required builtin. Other source-visible changes do
not persist into a later operation; failure to regain the canonical entry state
fails the session.
...
exported environment before `completed`, leaving the report stale. Requiring
the `CHLD`/`SIGCHLD` trap to remain unset keeps adapter-owned helper and
cleanup child exits from mutating persistent state after that snapshot while
still allowing Bash to reap children normally. This reservation belongs only
to the selected persistent Bash; a nested shell started by recorded source may
install and own its own child-exit trap.
...
and completion-side Readline readiness, the `completed` frame below carries only
the final status, physical cwd, and already-resolved inspection plan.
...
and adapter/readiness conditions, and re-enters Readline before Awsh sends
`completed` with the final status, physical cwd, and resolved inspection plan.
Envoy is Awsh's direct parent and the Linux subreaper; Awsh is the selected shell's direct parent and reaps
...
At ordinary return, the completion hook's `prompt_state` snapshot supplies the
status, physical and logical cwd, and exported environment needed for later
resolution. Awsh proposes `input_close` with only the operation ID and exact
completion-helper PID; Envoy completes mandatory cleanup and sends
`input_closed`; Bash then clears wait records, proves the empty job table and
adapter/readiness conditions, and re-enters Readline. Only then does Awsh
resolve configured paths and send `completed` with the final status, physical
cwd, and resolved inspection plan. The controller never resolves a workload
path, starts a probe operation, or infers filesystem state from terminal output.
...
accepts `completed` only after `input_closed`, the Bash-owned cleanup and
Readline readiness checks, and the final process census; it then has the
resolved plan and drains output through the operation's closing offset. Only
then may it inspect or hash workload paths.
...
The exact ordinary-return fields and order are:
...
The hook next sends the ordinary completion `prompt_ready` and blocks again.
Awsh accepts it only after the first helper has exited and been reaped and its
own helper phase is idle. Under one terminal-control lease it repeats the
startup termios handoff: set `ICANON` and `ECHO`, verify the remaining state
against the completion `prompt_state` snapshot, reply `accepted`, and observe
...
the inspection plan it resolved from the accepted physical/logical cwd and
exported environment. The logical cwd and environment do not need another wire
copy; functions, aliases, positional parameters, unexported variables, and
...
absent and the complete census to contain only external Awsh and selected Bash.
It then performs the fresh final PTY drain and covering output mark, runs the
already-defined workload inspection, and only then publishes the unchanged
public `operation_completed` shape. All of that remains under the original
non-resetting operation-cleanup deadline.
...
inherited exported environment directly rather than putting them in argv.
...
kernel socket buffer. The
complete helper message set through A2.5 is:
...
no operation is armed. Awsh accepts only that order. The `prompt_state` message
records the initial status, physical and validated logical cwd, bounded
exported environment, history-expansion state, and Readline editing mode.
...
ordering; the completion hook repeats it after source return. Awsh accepts the
same arity at startup and at ordinary completion, but accepts a completion
report only while the matching operation is active and no other helper phase is
live. It validates the completion helper's peer PID and carries that identity
in `input_close` before withholding `accepted` through Envoy cleanup.
When Awsh accepts the `prompt_state` message, it also reads and records the
complete pre-Readline termios state from the controlling terminal. Failure to
validate the complete message prevents readiness rather than truncating state.
...
process remains. Completion `prompt_ready` uses the same output-empty
`accepted` reply and termios transition as startup; it does not repeat the
state fields or carry an operation result.

The repeated adapter validation requires the `CHLD`/`SIGCHLD` trap to remain
unset across this boundary. The completion helper and cleanup children can exit
after the initial `prompt_state` snapshot; a trap installed by recorded terminal
operation source could otherwise run at that point and mutate the live cwd or
exported environment before `completed`.
...
After accepting startup or completion `prompt_ready`, Awsh sets both `ICANON`
and `ECHO` on its
...
Ordinary-completion cases freeze the existing `prompt_state` arity at startup
and completion and prove that functions, aliases, positional parameters,
