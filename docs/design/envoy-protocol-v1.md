...
The external-supervisor amendment preserves every controller request and event
shape. Its only public vocabulary additions are the pre-start
`operation_failed` codes `source-invalid` and `source-unsupported`, required to
represent Bash source that Awsh rejects without starting or damaging the
persistent shell. Process identity and source-submission changes otherwise stay
inside the private Envoy-to-Awsh contract.
...
prompt state/readiness or writing the operation result, and
`already-interrupted` after an earlier accepted finalization/cancellation has
already chosen the one interruption. Only `signal` makes Envoy immediately send
exactly one `SIGINT` to the then-current PTY foreground process group. Every
other disposition sends no PTY signal. This private round trip and any signal
remain inside the original grace period, which never resets on progress.
...
causes the one foreground-group `SIGINT`; `gate-cancelled` means Awsh released
...
including in zero-duration authored spans. After `TIOCSWINSZ`, Envoy explicitly
sends `SIGWINCH` to the PTY's current foreground process group; this keeps
delivery deterministic across the supported Linux PTY path. `resize_applied`
requires both actions to succeed. If either action fails, Envoy emits the
...
helper, and closes the slave after validating Bash launch. Envoy's parent ends,
the PTY master, and every controller or Reploy socket remain close-on-exec and
are never duplicated into the child. Bash and ordinary descendants receive
neither private Envoy-to-Awsh descriptor.
...
The result is an interruption instruction and acknowledgment, not an operation
terminal result.

Awsh serializes request classification with helper-connection state and its
single result writer. For `signal` it records the request and writes the
disposition before accepting a later helper packet. For `gate-cancelled` it
...
is never used to classify backend phase.

Awsh's initial `ready` must arrive within the existing `hello`/`ready` budget.
Envoy requires `AWSH_PID` to equal the child it launched, requires `SHELL_PID`
...
awsh-helper-v1, prompt_state, STATUS, PHYSICAL_CWD, LOGICAL_CWD_OR_EMPTY, EXPORTED_ENV_JSON
...
absolute and resolves to the reported physical cwd. A helper that cannot encode
...
once and only from that same waiting gate connection; Awsh serializes it against
private cancel/finalize and continue, returns `cancel`, and lets the helper
return status 130 without manufacturing a public cancellation request.
`prompt_state` captures status and
state before another hook can alter them, and `prompt_ready` proves Bash has
returned to input readiness. Awsh acknowledges each accepted state packet before
the helper exits. It validates the Unix peer as the configured workload uid and
...
and disables inherited `DEBUG`/`RETURN` tracing only for adapter code. An exec'd
...
optional `gate_interrupt`; an already-recorded private interruption instead
makes Awsh answer the initial `gate` with `cancel`. After the helper has closed,
Bash restores the exact user traps and
tracing state before accepting operation source or prompt input. A signal that
...
`adapter-state`. `SIGWINCH` retains its normal default helper disposition and is
not substituted for a lifecycle interruption.
...
parser, would consume it as framing. Awsh then fresh-parses `SOURCE` with the
fixed backend. Either condition produces `rejected` with `source-invalid`;
source whose grammar depends on unsupported persistent parser state produces
`source-unsupported`. Both map to an empty-range public `operation_failed`
before `operation_started`, and Bash remains available.

After arming the immutable Bash hooks, Awsh frames one brace group with a
redirection-only frame-entry marker and returns the complete NUL-free UTF-8
Readline sequence in `submit`. The marker is a redirection that atomically
creates one mode-0600 operation-correlated file in Awsh's private Bash runtime
directory; it is not a telemetry report and source receives no path contract.
...
`PS0` helper blocks through Awsh until `started_ack`. After accepting `started`,
Envoy drains the remaining redraw through `EAGAIN`, closes the discard interval,
commits `output_start`, emits `operation_started`, and acknowledges Awsh before
the source runs. A missing or duplicate frame-entry marker is fatal
`protocol_error` code `adapter-framing`; no partial Bash command becomes a
terminal operation result. Awsh removes the marker at the operation boundary.
...
brace group's stdout/stderr redirections while stdin remains the slave PTY.
After an Awsh result, Envoy performs mandatory descendant cleanup, closes its
keepers, drains both readers through EOF, removes the operation directory, and
only then emits the public terminal result. An exec'd child receives only the
standard descriptors required by its execution shape; no FIFO descriptor or
path remains in a later operation. These paths are correctness mechanics under
the cooperative same-identity contract, not hostile-workload isolation.
...
After an idle `shutdown`, Awsh closes the selected shell under the final-drain
deadline, reaps it, writes `closed` with reason `shutdown`, the real status, and
the last valid cwd, then closes its result descriptor and exits zero. Envoy
waits for both the frame and process exit. A missing frame, nonzero Awsh exit,
or remaining selected-shell process takes the fatal final-drain path.
...
executing source, `signal` authorizes Envoy's one foreground-group `SIGINT`; for
...
profile or `~/.bashrc`. Neither path honors `AWSH_BASH`. Before launching
...
`PROMPT_COMMAND`, and the gate hook, then makes the adapter-owned names readonly
without reading terminal input or any application file. At Bash's first prompt,
...
no operation is armed, acknowledges each normally, records the initial cwd and
exported environment, and rejects any other startup helper packet as fatal
`adapter-state`. Only after the second helper closes does Awsh perform the
parentage and controlling-terminal checks below and emit private `ready`. The
existing `hello`/`ready` deadline covers rcfile execution, both helper exchanges,
and those checks; Bash output during startup is ordinary retained PTY output,
never readiness evidence.

Fresh-Bash syntax preflight rejects malformed source and syntax that depends on
persistent parser state such as a previously enabled `extglob`. The Readline
bracketed-paste-begin binding and the bracketed-paste terminator bytes in source
are reserved separately. A redirection-only marker at the start of every framed
brace group detects mutation of the binding: if Bash returns to a prompt without
entering that marker, Awsh reports fatal `adapter-framing` and Envoy ends the
session without an operation result.

Awsh forks the selected Bash child while it still holds the passed PTY slave.
In that child and before exec, it calls `setsid`, acquires the slave as the
controlling terminal with `TIOCSCTTY`, duplicates the slave onto file
descriptors 0, 1, and 2, closes every other descriptor, and makes the new
session/process group the PTY foreground group with `tcsetpgrp`. The exec'd
interactive Bash is therefore session leader, process-group leader, and initial
foreground process. After the immutable bootstrap reaches prompt readiness,
Awsh validates the Bash PID as its direct child and validates both
`tcgetsid(slave)` and `tcgetpgrp(slave)` against that PID before closing its copy
of the slave and sending private `ready`. Failure at any step closes the helper
endpoint and slave, terminates and reaps any Bash child or group already
created, reports bounded `shell-launch` failure when possible, and exits
nonzero. Envoy never publishes `ready` for a partial launch.
...
split execution only the evaluated brace group's stdout and stderr replace fd 1
and fd 2 with the selected operation-scoped FIFO opens. Ordinary exec'd
descendants inherit no extra reader, keeper, helper, control, or operation
...
| Indexed and associative arrays, functions, aliases, and positional parameters | Preserved. Actual source is parsed by persistent Bash, so aliases and functions retain their Bash parsing/execution effect. Fresh-Bash preflight proves fixed-profile syntax only; source rejected there as `source-unsupported` never reaches persistent Bash. |
| User traps, including `EXIT`, `ERR`, `DEBUG`, and `RETURN` | Preserved; the adapter installs no persistent workload-visible trap. Immutable helper functions suppress inherited debug/return tracing and temporarily ignore `SIGINT`/`SIGQUIT` only inside the signal-safe adapter critical section, then restore the exact user definitions and tracing state before operation source or prompt input. `EXIT` therefore remains workload-owned and shell status still comes from Awsh's reap. |
| `$?` | The operation status is captured before any prompt helper runs, reported to Awsh, and restored as the status visible when Bash next accepts source. |
...
| `set -o` execution options | `allexport`, `braceexpand`, `emacs`/`vi`, `errexit`, `errtrace`, `functrace`, `hashall`, `histexpand`, `history`, `ignoreeof`, `keyword`, `noclobber`, `noglob`, `nolog`, `notify`, `nounset`, `physical`, `pipefail`, `verbose`, and `xtrace` retain their Bash values and normal effects. `monitor` and `interactive-comments` are reserved enabled adapter invariants. `noexec`, `onecmd`, `posix`, and `privileged` are unsupported persistent modes: a changed value that reaches a boundary is fatal `adapter-state` or explicit `shell_exit`, never silently reset or reported as preserved. |
| `shopt` options | The supported preserved inventory is `autocd`, `assoc_expand_once`, `cdable_vars`, `cdspell`, `checkhash`, `checkjobs`, `checkwinsize`, `cmdhist`, `compat31`, `compat32`, `compat40`, `compat41`, `compat42`, `compat43`, `compat44`, `complete_fullquote`, `direxpand`, `dirspell`, `dotglob`, `execfail`, `expand_aliases`, `extdebug`, `extglob`, `extquote`, `failglob`, `force_fignore`, `globasciiranges`, `globskipdots`, `globstar`, `gnu_errfmt`, `histappend`, `histreedit`, `histverify`, `hostcomplete`, `huponexit`, `inherit_errexit`, `lastpipe`, `lithist`, `localvar_inherit`, `localvar_unset`, `mailwarn`, `no_empty_cmd_completion`, `nocaseglob`, `nocasematch`, `noexpand_translation`, `nullglob`, `patsub_replacement`, `progcomp`, `progcomp_alias`, `shift_verbose`, `sourcepath`, `varredir_close`, and `xpg_echo`. `promptvars` and `interactive_comments` are reserved enabled; `login_shell` and `restricted_shell` are reserved disabled. Source whose grammar is rejected by the fixed-profile fresh parser, including source requiring a previously enabled `extglob`, is `source-unsupported`. |
| In-memory history and history controls | Bash-owned and preserved according to the options above. `HISTFILE` is forced empty only at controlled launch; source may deliberately change it afterward. History text is not operation-boundary evidence. |
| Readline editing mode and bindings | Emacs and vi modes and non-conflicting bindings persist. Bracketed paste must remain enabled and its begin sequence is reserved. Mutation may occur during an operation, but a missing frame-entry marker makes the next submission fatal `adapter-framing`; the adapter never resets it silently. |
| Adapter hooks and helper identity | `PS0`, `PS1`, `PS2`, `PROMPT_COMMAND`, the gate function, fixed helper executable/modes, and literal helper socket argument are immutable/reserved. An attempted direct mutation fails as readonly; indirect corruption or a missing hook report is fatal. |
...
best-effort fatal `awsh-failed`. A Bash prompt report without the required
frame-entry marker emits fatal `adapter-framing`. Both take the
...
`setsid`/`TIOCSCTTY`/foreground-group PTY setup; private `ready` identity
validation; manifested-rcfile launch and the exact startup
`prompt_state`/`prompt_ready` sequence before private `ready`;
...
orderly `closed` with the selected shell's reaped status. Bash and an exec'd
...
single-packet non-gate modes, the optional same-connection `gate_interrupt`,
start and gate blocking replies, prompt-state-before-prompt-ready
ordering, exact save/ignore/verify/restore behavior for user `SIGINT`/`SIGQUIT`
traps and `DEBUG`/`RETURN` tracing, late Ctrl-C during each helper mode, endpoint
...
syntax rejection, rejection of the exact bracketed-paste terminator before
arming when it appears in each Bash lexical context, persistent
...
and mutation of the reserved bracketed-paste-begin binding makes the next
frame-entry check fail fatally rather than accepting a partial command result.

Split-operation cases prove stdin remains the PTY; stdout and stderr stream
before completion; background writers are terminated before dual EOF; only the
...
and no operation descriptor or path reaches a later operation. Gates,
...
cases freeze every private request/disposition phase, the one signal authorized
only by `signal`, typed gate cancellation without a signal, crossed completion-
helper/result ordering, both serialized result winners, and the no-second-
interruption crossed-cancel rule.
...
accepted immediately after `signal`; every case preserves one interruption,
one disposition, the captured state, and the original non-resetting timer.
Resize races cover both shell-end outcomes: `resize_applied` resolves a resize
...
`TIOCSWINSZ` or
foreground-process-group `SIGWINCH` delivery in both idle and active-operation
states produces fatal `resize-failed`, no
