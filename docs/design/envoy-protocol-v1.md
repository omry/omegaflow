...
fresh parser did not see. The Readline bracketed-paste-begin binding and the
bracketed-paste terminator bytes in source are reserved separately. The
immutable frame-entry hook and its redirection-only marker at the start of every
framed brace group detect mutation of the binding: if Bash returns to a prompt
without entering that hook and producing its valid marker, Awsh reports fatal
