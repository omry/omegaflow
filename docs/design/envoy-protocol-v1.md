...
during the authored schedule to its current frontier. Every resize accepted
after the controller sends `execute` and before it leaves `Starting` belongs to
that span's closing seam, regardless of the operation's timing mode. The
controller retains that classification if cancellation moves it from
...
operation's authored schedule is known. Its `output_through` then places it
immediately after every authored output event derived from bytes through that
raw frontier and before any later authored output event. If the frontier covers
no authored output byte, the resize uses the pre-span absolute time. Publishing
any of these resizes at its own `elapsed_us` would expose discarded command
duration or controller scheduling. The writer therefore publishes it at
exactly the selected authored frontier; it does not interpolate toward the next
authored event, use sender time, or move authored events. Events already
