...
adapter first requires the `SIGCHLD` disposition to be the Bash default and the
`DEBUG` and `RETURN` trap definitions to be empty. It redirects the Bash
...
disposition only to deliver its bounded diagnostic; it never saves, suppresses,
or restores a `DEBUG`/`RETURN` trap. If such a trap prevents the check or report,
the owning deadline path applies. No later source or prompt readiness is
claimed. V1 reserves `SIGCHLD` because a
...
and requires both `DEBUG` and `RETURN` traps to be unset. V1 does not attempt to
suppress either trap from inside an adapter function, because a `DEBUG` trap can
run before that function is invoked. The reserved-unset `DEBUG`/`RETURN` and
reserved-default `SIGCHLD` checks occur before every normal helper spawn, so an
internal command or child exit cannot invoke workload tracing code. An exec'd
...
be unset before this generated function can be dispatched; the function does
not claim that it can suppress a trap before its own invocation. Its exact name
is the forbidden alias key above. Running it as the `if` condition prevents a
...
adapter critical section with the reserved `DEBUG` and `RETURN` traps still
unset. Its frame-entry
...
A source that leaves a `DEBUG` or `RETURN` trap installed may make that trap run
before the immutable prompt-boundary check, including before the helper that
would report completion. Awsh retains the operation as active when it receives
no completion evidence. The recording deadline still sends ordinary
...
trap state, a gate refusal, a reached-boundary fatal report, and a `DEBUG` trap
that suppresses the boundary through cancellation timeout. `extdebug` remains
...
traps, reserved-unset `DEBUG`/`RETURN`, default `SIGCHLD` validation before every
