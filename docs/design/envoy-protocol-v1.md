...
the Bash default, and the `DEBUG` and `RETURN` trap definitions to be empty. It
...
reserved-default `SIGCHLD` checks occur before every normal helper spawn, so a
...
the immutable prompt path, enabled boundary `xtrace`, a non-default `SIGCHLD`
disposition, or failure to observe actual Readline entry emits fatal
`adapter-state`. These take the
...
`SIGCHLD` validation before every normal helper spawn, initial and later invalid
reserved tracing state, late
