...
deadline, publishes the pending result, and enters the `shell_ended` drain.
Which result is pending decides whether it gains a field: a pending
`operation_completed` or `operation_failed` carries `shell_ended: true`, while
a pending `operation_cancelled` or `operation_finalized` is published exactly
as its own schema defines it, because `shell_ended` is not a field of those two
events and this slice adds no public field. The controller still learns the
shell is gone from the Envoy-initiated `draining` and `closed` that follow,
both carrying reason `shell_ended`. That reap, census, and drain stay under the
