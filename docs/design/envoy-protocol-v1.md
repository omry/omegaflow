...
Host OmegaFlow generates a fresh 128-bit random lowercase hexadecimal
...
supervised writer set to be empty and the fresh pre-start output drain barrier
under Output ordering to complete before `operation_started`. Replacement text
and authored presentation delays stay controller-private and are not sent to
the Envoy.
...
Tracking itself has no protocol-level numeric admission limit. The Envoy
retains a supervision identity for every live operation-created descendant,
but subreaper status, pidfds, and `/proc` census are discovery and identity
mechanisms rather than a pre-fork enforcement boundary. Failure to acquire a
required pidfd, complete the census, or retain the descriptors and state needed
for supervision is therefore a fatal session failure; the Envoy never continues
with a partially supervised process set. Host or container PID/fd exhaustion
may surface first and is likewise a failed Reploy lifecycle, not a typed claim
that the Envoy rejected a particular fork. A future deterministic ceiling must
be a Reploy-owned kernel-enforced workload/session process domain, such as a
cgroup PIDs limit with stated overhead and cleanup semantics. Per-operation
process admission is not part of protocol v1.
...
The Envoy first proves that every potential writer retained from an earlier
operation has exited and been reaped. It then waits for EOF on every retired
split-stream pipe, drains every byte ordered before those EOF and reap
boundaries from the retained pipes and PTY master, writes the bytes to the
terminal socket, and emits the covering output mark. Only after that fresh
drain may it snapshot `output_start`, emit `operation_started`, and acknowledge
the driver's `started` result so Bash can evaluate the new source. Observing an
empty supervised-writer set without completing this drain is not a barrier:
bytes already buffered in a retired pipe or the PTY can still be in flight.
Failure to census, reap, observe EOF, drain, or write the preceding bytes fails
...
active or retained split stdout/stderr pipe — appends and writes those bytes,
...
exceed the retained-writer budget; `shell-ended-unresolved`
