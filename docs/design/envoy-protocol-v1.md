...
adapter validates every key and value in the special `BASH_ALIASES` associative
array before sending the startup or later `prompt_state`.
...
captures the complete Linux termios state from its control-only slave while
Bash is outside Readline. `prompt_ready` proves Bash has disabled top-level
history expansion and returned to input readiness. Awsh acknowledges each
...
The complete v1 submission capsule begins after preflight. Awsh snapshots the
active Readline termios state through its control-only slave and derives a
...
frame-entry path to be absent and constructs `TERMINAL_SUBMISSION` as exactly
the six-byte bracketed-paste begin sequence `ESC [ 2 0 0 ~`, one NUL-free UTF-8
conditional frame, the six-byte bracketed-paste terminator, and one `LF` byte.
The frame has this grammar, with the two `SOURCE` fields byte-identical and the
two optional split-stream `REDIRECTIONS` fields identical:
...
`STATUS`. It suppresses inherited `DEBUG` and `RETURN` tracing around its own
adapter instructions and restores that tracing state before selecting the
authored branch. Its exact name is the forbidden alias key above. Running it as
the `if` condition prevents a nonzero saved status from invoking `errexit`; Bash
...
adapter critical section with inherited tracing suppressed. Its frame-entry
...
mode and requiring the complete current termios state to equal the pre-Readline
state captured with the preceding `prompt_state`. The marker is not a telemetry
report and source receives no path contract.
...
never returns normally, Awsh restores its active-mode termios snapshot only for
fatal cleanup; capsule termios state is never workload-visible. A missing or
duplicate frame-entry marker, termios mismatch, or restoration failure is fatal
...
history expansion before startup `prompt_ready`. Only after the second helper
closes does Awsh perform the
...
the temporary termios delta makes the PTY input path byte-transparent. The
...
for the documented terminal identity checks, termios capsule operations, and
atomic cancellation lookup.
...
| `set -o` execution options | `allexport`, `braceexpand`, `emacs`/`vi`, `errexit`, `errtrace`, `functrace`, `hashall`, `history`, `ignoreeof`, `keyword`, `noclobber`, `noglob`, `nolog`, `notify`, `nounset`, `physical`, `pipefail`, `verbose`, and `xtrace` retain their Bash values and normal effects. `histexpand` also retains its workload-visible value, but the submission capsule temporarily disables it while Bash reads and parses the top-level generated frame and restores it before authored source executes. Consequently top-level authored `!` text is byte-exact and is not history-expanded; parsing explicitly requested later by workload code observes the restored value. `monitor` and `interactive-comments` are reserved enabled adapter invariants. `noexec`, `onecmd`, `posix`, and `privileged` are unsupported persistent modes: a changed value that reaches a boundary is fatal `adapter-state` or explicit `shell_exit`, never silently reset or reported as preserved. |
...
| Terminal modes | The workload-visible Linux termios state is preserved. At each `prompt_state`, Awsh snapshots the complete pre-Readline state. For injection only, it applies and verifies the closed byte-transparent capsule delta to the active Readline state. Normal Readline return must restore the complete pre-Readline snapshot before Awsh accepts `start`; a mismatch is fatal `adapter-framing`. No capsule mode reaches authored source. |
...
complete pre-Readline termios snapshot, or input-state condition failure emits
fatal `adapter-framing`. Invalid reserved Readline or history state detected by
the immutable prompt path emits fatal `adapter-state`. These take the
...
pre-Readline termios, before private `ready`;
...
normal Readline restoration to the complete prompt snapshot, and fatal setup or
restoration mismatch. With persistent `histexpand` both on and off, authored
