...
stateDiagram-v2
    ...
    Finalizing --> Cancelling: controller cancel during inspection
...
If finalization has already stopped the operation and its inspection worker is
still running, the controller-owned operation deadline may still send `cancel`.
The controller moves from `Finalizing` to `Cancelling`, and the same serialized
...
stops and reaps the worker and emits `operation_cancelled` with the status the
driver returned from finalization and no inspection results. Failure to reap
within the existing five-second grace remains fatal
`inspection-cancel-timeout` with no terminal operation result.
...
results. A worker that remains blocked past
