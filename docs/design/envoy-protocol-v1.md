...
phase is `running`. This rule alone governs every `gate_interrupt` Envoy reads,
whatever else has happened to the operation, and the timeout and shell-exit
sections below defer to it rather than restating it. Envoy always writes
`gate_interrupt_ack`, because Awsh blocks in `gated` until that acknowledgement
arrives and cannot be left waiting; when Awsh has already exited, that write
lands in a closed pipe and its failure is the ordinary crossed-write case
below. What varies is publication. While the controller is still in a state
that has a gate transition, Envoy commits public `operation_gate_interrupted`
with its barrier and writes the acknowledgement before any later private
request for that operation. When Envoy has already written `continue` for that
gate before it reads `gate_interrupt`, the interruption wins: Awsh discards the
crossed `continue`, because there is nothing left to release, and writes no
...
event through the existing `Continuing --> Running` edge. Once Envoy has
accepted a `cancel` or `finalize`, or selected timeout teardown, the controller
has left `Running`, `Gated`, or `Continuing` for `Cancelling`, `Finalizing`, or
a selected timeout outcome, and none of those has a gate transition; because
the state machine fails closed on an unspecified transition, Envoy publishes no
gate event from that point on. It still writes `gate_interrupt_ack` under the
rule above, and it discards `gate_ready` and `gate_continued` outright, writing
nothing for them because Awsh does not wait on either. The lifecycle or timeout
result resolves the gate in every one of those cases, so an ordinary deadline
race at gate entry or release cannot turn cancellation, finalization, or
timeout teardown into a fatal session failure. A `continue` naming any other
gate, or arriving after the acknowledgement, is a protocol failure.

#### Cancellation and planned finalization

Envoy forwards at most one lifecycle request per operation, and only while the
operation is still running from its point of view: the first `cancel` or
`finalize` it accepts after it has accepted `start_released` and before it has
accepted `input_close` or `completed`, including a cancel queued across the
start commit. Envoy starts the cancellation grace period when it writes that
forwarded request. A request first accepted once Envoy has observed return is
not forwarded at all: the public cancellation and finalization rules already
make it Envoy-local, sending no signal, starting no grace period, and
continuing under the existing cleanup or inspection deadline, and there is
nothing left for Awsh to interrupt. Every later lifecycle request for the
operation is Envoy-local for the same reason, so Awsh never delivers a second
signal and never owes a second `disposition`. The `returned` phase therefore
covers only the narrower race in which Envoy forwarded the request while the
operation still looked running and Awsh read it after accepting the
completion-side `prompt_state` connection.

Awsh classifies the selected backend's phase at the moment it reads the request
and answers it with exactly one `disposition`, written before it accepts
another helper request or writes any later result. The vanished-group retry
below is the one qualification: while that retry is in progress the phase is
not yet fixed, so Awsh may still accept the completion-side `prompt_state`
connection, which resolves the phase as `returned` and is then reported with
`none`, or may find the shell reaped first and write the terminal `shell_exit`
with no `disposition` at all. Awsh never writes more than one `disposition` for
a request, never writes one after `shell_exit`, and never writes a later result
while a `disposition` it owes is still unwritten:
...
`gate_ready`, `gate_continued`, or `disposition` — is a crossed result: Envoy
discards it and writes no `input_closed` for it. Two results are not discarded.
A `gate_interrupt` is handled by the single action-gate rule above, which
acknowledges it without publishing a gate event. And a `protocol_error` is
never discarded: it is Awsh's own fatal terminal result, it replaces rather
than precedes the `shell_exit` above because Awsh exits nonzero after writing
it, and waiting for reaping evidence that cannot arrive would convert a
specific private failure into a generic fatal EOF at the deadline. Envoy
therefore processes it immediately under the fatal `protocol_error` rules
below, which supersede the selected timeout outcome and emit no terminal
operation result.
...
with status zero. The reaped status is 129 for the default untrapped
disposition, where interactive Bash runs any `EXIT` trap and then terminates by
the signal so that `EXIT` trap cannot substitute an exit status. Recorded
source may instead install a permitted `HUP` trap that exits, in which case
Bash leaves by that handler's normal-exit path and `closed` carries the
handler-selected status, which an `EXIT` trap on that path may change again.
`closed` always reports what Awsh actually reaped; 129 is the default case, not
a required value. A persistent shell whose recorded source ignored or trapped
`HUP` without exiting keeps Awsh waiting; the already-running Envoy final-drain
deadline then terminates the controlled tree and the session takes that fatal
drain result rather than a clean `closed`. Recording plans must not leave
persistent Bash ignoring `HUP`.
...
zero. A `gate_interrupt` Awsh had already written before the reap is not
retracted: the result pipe preserves order, so Envoy reads it before
`shell_exit`. What Envoy does with it is decided by the action-gate rules
above, not by the reap. While the controller is still in a state that has a
gate transition, Envoy emits `operation_gate_interrupted` and writes
`gate_interrupt_ack`; once a lifecycle request has been accepted or timeout
teardown selected, it publishes no gate event and only writes the
acknowledgement. Either way Awsh has exited by then, so that acknowledgement
goes into a closed pipe and its write failure is the ordinary crossed-write
case below. Envoy then reads `shell_exit` and takes the shell-end path, whose
terminal result supersedes the gate.
...
shell ended while Envoy was still closing the operation: Envoy proves the final
census with the shell, its descendants, and the exited Awsh absent, performs
the drain, runs any declared inspection, publishes the pending result, and
enters the `shell_ended` drain. That reap, census, and drain are bounded by
whichever deadline is live at the moment the exit is accepted, and one always
is. Before the final census and output-through barrier that is the
operation-cleanup deadline, which is still running. After that barrier the
cleanup deadline has ended by its own rule and workload inspection is running,
so the controller-owned operation deadline bounds the reap, with inspection
cancellation keeping its existing worker stop-and-reap timeout. Neither
deadline is started, reset, or extended by the exit. Which result is pending
decides whether it gains a field: a pending `operation_completed` or
`operation_failed` carries `shell_ended: true`, while a pending
`operation_cancelled` or `operation_finalized` is published exactly as its own
schema defines it, because `shell_ended` is not a field of those two events and
this slice adds no public field. The controller still learns the shell is gone
from the Envoy-initiated `draining` and `closed` that follow, both carrying
reason `shell_ended`. An empty identifier while the session is idle takes the
idle shell-end drain already defined.
...
| `gate_interrupt` read after a lifecycle request was accepted or timeout teardown selected | lifecycle request or selection | under the single action-gate rule: write `gate_interrupt_ack`, publish no gate event; the lifecycle or timeout result resolves the gate |
| `gate_ready` or `gate_continued` read after a lifecycle request was accepted or timeout teardown selected | lifecycle request or selection | discard it; publish no gate event and write nothing; the lifecycle or timeout result resolves the gate |
...
| Awsh result read after timeout selection | selection | discard the crossed result and await `shell_exit` as reaping evidence, except `gate_interrupt`, which the action-gate rule acknowledges, and `protocol_error`, which is processed immediately as the fatal terminal result |
...
| `shell_exit` with an empty identifier after `completed` | pending result | publish the pending result under its own schema, adding `shell_ended` only to `operation_completed` or `operation_failed`, then drain |
...
`gate_interrupt_ack`, the crossed `continue` discarded without
`gate_continued` while `operation_gate_interrupted` resolves it, and a
`gate_ready` and a `gate_continued` each queued before an accepted `cancel`
and then discarded with no public gate event and no fatal transition. They
reject a reused, malformed, idle, unstarted, second-blocked, out-of-session, and
...
closing a foreground and a backgrounded helper without an interruption; and the
`returned` phase from the `prompt_state` connection through `completed` with
`none` and no signal. They prove the completion hook's `INT` bracket keeps both
a lifecycle signal and terminal Ctrl-C from aborting the hook or killing the
direct-exec helpers, restores a source-installed `INT` trap and the default
disposition, and that an interruption before the bracket yields one completion
reporting status 130. Timeout cases cover `input_close`, `completed`,
`gate_ready`, `gate_interrupt`, and `disposition` read after Envoy's selection
and discarded, a `protocol_error` read after selection processed immediately
instead of discarded, `shell_exit` with either identifier form and any valid
reaped status, ordinarily 137, as the sole accepted post-selection result,
including a shell that exited just before selection and one whose crossed
`completed` makes the identifier empty, and a blocked helper dropped without a
reply. Lifecycle cases also cover a `cancel` or `finalize` forwarded after Awsh
wrote `completed`, answered `returned`/`none` and discarded by Envoy, and a
foreground job ending between `tcgetpgrp` and `kill` where the terminal keeps
naming the dead group across at least one retry, resolved either by signalling
the reclaimed group or by reporting `returned`/`none` when the operation
returns first, and never by a `signalled` report without a delivered signal.
Resize-lane cases cover an idle lane, a lane waiting behind the startup and
completion handoffs and a lifecycle signal, a helper phase waiting behind the
lane, dimension mismatch at each of the four frames, a second outstanding
exchange, and shell exit at each phase with no resize result. Shutdown cases
cover the default untrapped `HUP` reap at status 129 with and without an `EXIT`
trap, a permitted `HUP` trap that exits so `closed` carries the
handler-selected status, a persistent shell ignoring `HUP` reaching the fatal
final-drain result, and both crossing orders against shell exit. Shell-exit
cases cover the active, empty, and post-`completed` identifier forms, a pending
`operation_cancelled` and a pending `operation_finalized` published without
`shell_ended` while a pending `operation_completed` carries it, the dropped
helper and gate peers, a `gate_interrupt` that precedes `shell_exit` on the
pipe processed normally with its acknowledgement failing into the closed pipe,
and a control write that fails after each terminal result being orderly. Every
`protocol_error` code is proved fatal with no terminal operation result.
