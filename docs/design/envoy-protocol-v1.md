...
`operation_cancelled` with an empty range. The wait is bounded anyway: the
barrier only waits for bytes the controller has already written, so exceeding
the terminal input barrier wait means they are never arriving, and the Envoy
fails the operation rather than holding the session.
