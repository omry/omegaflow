...
reserved trap state above, temporarily ignores `SIGINT` and `SIGQUIT`, and
restores their default dispositions after the helper closes. V1 does not attempt
...
Bash restores the default `SIGINT`/`SIGQUIT` dispositions before accepting
operation source or prompt input. For `start`, the generated frame is one
...
`SIGINT`/`SIGQUIT`, and then evaluates the generated frame's outer redirection
condition. Redirections are applied left to right: Bash first
...
bytes admitted before the delayed result, exact ignore/verify/default-restore behavior for `SIGINT`/`SIGQUIT`,
