...
accepted during a presentation-timed operation before schedule commitment
begins. It is placed onto that schedule as described under Resize; nothing else
is retimed. Sender-stamped realtime output never triggers this, since marks are
already non-decreasing. The rule guards the seam where an authored schedule
...
synthesized prompt and typing, the serialized writer assigns a resize dequeued
during the span to its current authored frontier. For a presentation-timed
operation, the controller classifies every resize accepted from
`operation_started` through the operation's terminal event as part of that
span, even when the writer dequeues it before compiled publication begins. The
controller buffers each such resize until the operation's authored schedule is
known. Its `output_through` then places it immediately after every authored
output event derived from bytes through that raw frontier and before any later
authored output event. If the frontier covers no authored output byte, the
resize uses the pre-span absolute time. Publishing any of these resizes at its
own `elapsed_us` would expose discarded command duration or controller
scheduling. The writer therefore publishes it at exactly the selected authored
frontier; it does not interpolate toward the next authored event, use sender
time, or move authored events. Events already assigned to the frontier precede
it, and later queued events at the same absolute time follow it. Multiple
resizes at one frontier retain queue order, including in zero-duration authored
spans. The kernel delivers `SIGWINCH` normally. If
...
descriptors. Blueprint validation already rejects every exact application name
and prefixed family removed by this launch filter — the normative enumeration
the Reploy environment design owns — before anything is deployed. Reserved
composition then adds an empty `HISTFILE`, and final validation requires
that exact value for the bootstrap environment. The Envoy independently
discards the incoming `HISTFILE`, delegates all other permitted application
values including `PATH`, and installs an empty `HISTFILE` as the final
controlled-Bash launch value. Thus neither shell opens an application-selected
history path or the default under an application-selected `HOME` before
accepting OmegaFlow input. A workload whose commands need a loader variable or
history file sets it inside operation source, where persistent Bash carries it
to operation children as ordinary shell state while the driver's own image
never launches under it. Environment names must be non-empty, contain neither
`=` nor NUL, and values cannot contain NUL.
...
`INPUTRC`, enforcement of an empty `HISTFILE` for both shell launches, a
matching and mismatching handshake `session_id`, every startup and control-write
deadline epoch, and resize placement before, between, and after authored events,
including multiple resizes at one frontier and zero-duration spans.
