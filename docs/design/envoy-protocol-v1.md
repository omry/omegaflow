...
The two clocks meet through one running offset. The controller maps the
operation's authored schedule onto the session timeline starting at the last
committed absolute time, and when the operation ends it sets a session
presentation discard to the difference between the sender time closing the
operation and the last absolute time its authored schedule committed. Every
later event publishes at its source time minus that discard. That closing sender
time needs no new field: a mark is already emitted immediately before any event
carrying `output_through`, so the mark that closes the operation's range carries
it. The first event after the operation therefore lands exactly at the end of
the authored schedule, and every later gap keeps its real length. The
per-operation mapping alone is not enough: it places the compressed operation
correctly but leaves the session clock running at real time, so the next
realtime mark would land a full command duration later and reopen exactly the
pause the operation exists to remove. With the running discard a presentation
operation never rewinds the cast, never exposes how long the command actually
took, and does not displace what follows it, while `elapsed_us` and the private
raw log keep the real clock untouched.
...
source time, after the presentation discard is applied, precedes the last
committed absolute time is committed at that last time instead. The only event
