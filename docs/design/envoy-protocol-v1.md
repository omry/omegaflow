...
retracted and is not discarded: the result pipe preserves order, so Envoy reads
it before `shell_exit` and processes it normally, emitting
`operation_gate_interrupted` and writing `gate_interrupt_ack` under the action-
gate rules. Awsh has exited by then, so that acknowledgement is written into a
closed pipe and its write failure is the ordinary crossed-write case below.
Envoy then reads `shell_exit` and takes the shell-end path, whose terminal
result supersedes the released gate.
