...
- `returned`: Awsh has accepted the completion-side `prompt_state` connection
  and has not yet written `completed`. There is nothing to interrupt; Awsh
  reports `none` and the ordinary return continues unchanged.

A failure to open the lease, read the foreground group, or deliver the signal
is fatal with `protocol_error` code `lifecycle-signal`. A lifecycle request
that names an operation Awsh has not started, or arrives after `completed`,
is a protocol failure with code `request-state`; Envoy never writes one in
those states because it discards such requests publicly. `finalize` and
`cancel` are identical on the Awsh side; the request name is carried only so
the disposition can be matched.
...
census. No private teardown request exists. Any result Envoy reads after that
selection other than `shell_exit` — a late `input_close`, `completed`,
`gate_ready`, `gate_interrupt`, or `disposition` — is a crossed result: Envoy
discards it and writes no `input_closed` or acknowledgement for it. Awsh reaps
the killed shell, drops any blocked helper without a reply, and writes
`shell_exit` with the active operation identifier and status 137; that frame
is the required reaping evidence for the already-selected timeout, and Envoy
withholds the public timeout result until census, reap, and drain succeed.
...
drain, runs any declared inspection under the
controller-owned operation deadline, publishes the pending result with
`shell_ended: true`, and enters the `shell_ended` drain. An empty identifier
while the session is idle takes the idle shell-end drain already defined.
...
Envoy's selection and discarded, `shell_exit` with the active identifier and
status 137 as the sole accepted post-selection result, and a blocked helper
dropped without a reply. Resize-lane cases cover an idle lane, a lane waiting
behind the startup and completion handoffs and a lifecycle signal, a helper
phase waiting behind the lane, dimension mismatch at each of the four frames,
a second outstanding exchange, and shell exit at each phase with no resize
result. Shutdown cases cover the reaped `HUP` status 129 with and without an
`EXIT` trap, a persistent shell ignoring `HUP` reaching the fatal final-drain
result, and both crossing orders against shell exit. Shell-exit cases cover
the active, empty, and post-`completed` identifier forms, the dropped helper
and gate peers, a crossed `gate_interrupt` discarded on `shell_exit`, and a
control write that fails after each terminal result being orderly. Every
