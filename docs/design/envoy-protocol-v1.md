...
the unset `CHLD`, `INT`, `DEBUG`, `ERR`, and `RETURN` traps, adapter-required builtins,
...
The repeated adapter validation requires the `CHLD`/`SIGCHLD` and
`INT`/`SIGINT` traps to remain unset across this boundary, while preserving
every non-reserved signal trap. The completion hook installs temporary `INT`
ignore immediately after saving source status, restores the canonical unset
state with reserved `builtin trap - INT`, and requires `builtin trap -p INT` to
be empty before final adapter validation. The completion helper
and cleanup children can exit after the initial `prompt_state` snapshot; the
final state-bearing `prompt_ready` is authoritative for the live Bash state
used by `completed` and path resolution.
