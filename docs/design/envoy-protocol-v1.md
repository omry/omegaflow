...
| One logical stream evidence chunk | 1–196,608 decoded bytes |
| Logical stdout or stderr per operation | 8 MiB decoded bytes |
...
| --- | --- |
| `hello` | `session_id` |
| `execute` | `operation_id`, `source`, `execution_shape`, `timing`, `publication`, `observation` |
...
`operation_output` is valid only for a split-stream operation. `stream` is
`stdout` or `stderr`; `data_base64` is non-empty canonical RFC 4648 base64. The
decoded chunk and accumulated per-stream evidence obey the limits above.
Telemetry sequence order preserves order within each logical stream. PTY
operations never emit `operation_output`: their exact raw-log range is logical
...
For split execution the Envoy also emits each observed chunk as
`operation_output` before the terminal result. Those events preserve logical
stream identity but do not enter the raw terminal log themselves; the same
bytes forwarded on the terminal connection do. The completion barrier remains
the cross-channel authority.
...
incrementally using their controller-boundary arrival times. Presentation-
timed `real` operations buffer split-stream evidence while the command runs;
after completion, the output-through barrier, and the logical post-enter
pause, the controller publishes logical stdout followed by logical stderr.
...
status for planned finalization.
...
operation-scoped stdout/stderr descriptors. Slice 2 must prevent ordinary
operation children from inheriting the driver descriptors. Split-stream
descriptors are inherited only as required by the evaluated command tree and
are supervised and closed at its typed boundary.
...
- `envoy.jsonl`: exact Envoy event encodings, including binary-safe logical
  stream evidence and planned finalization; and
