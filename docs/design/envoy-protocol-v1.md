...
the `DEBUG`, `ERR`, and `RETURN` traps unset; the adapter-required `builtin`,
`enable`, `kill`, `return`, and `trap` builtins enabled; the fixed idle
...
including enabling `errexit`, but it may not mutate the three adapter-sensitive
...
ordinary behavior. `trap` refuses any request that would mutate `DEBUG`, `ERR`,
or `RETURN`. `enable` refuses any disable, dynamic-load or replacement, or
dynamic-unload request naming an adapter-required builtin. A mixed request that
...
`DEBUG`, `ERR`, and `RETURN`, including `trap 'exit 42' DEBUG`, and attempt to
