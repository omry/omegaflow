...
the driver does not return, the Envoy
terminates the persistent process group and emits `operation_failed` with
`cancel-timeout`. An operation for which cancellation wins never emits
...
range. A driver that does not return within
...
does not return within the grace period; `echo-span-unclosed` for an echo span
the Envoy cannot close; `shell-ended-unresolved` for a shell end leaving
declared inspections or an authored gate
unevaluable.
...
the shell-ended result winning and retaining its reaped status.
