...
commit point: from there through matching `started`, Envoy finishes the non-
interruptible bounded submission transaction. A complete cancel received on
telemetry during that interval is queued but not accepted; after Envoy emits
`operation_started`, it immediately accepts that cancel and takes the ordinary
running cancellation path. A controller already in
...
operation source or prompt input. A signal that
...
`STATUS`. The preceding prompt boundary requires `ERR`, `DEBUG`, and `RETURN`
