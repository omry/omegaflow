...
Envoy's monotonic microseconds since the session epoch established by `ready`.
A mark attributes every byte from its `offset` until the next mark's `offset`.
...
its exact raw-log range is logical stdout, and logical stderr is empty.
...
expects to use. Bytes arriving after completion stay outside that operation's
range, carry `stdout` or `stderr` marks under the originating operation's
publication policy, and are drained at a later barrier or the final drain. An
exclusive operation still requires the supervised writer set to be empty before
it starts, so these retained streams never enter a checked range.
...
published by its policy. They are published under the policy of the real
operation that created the writer, at their own mark times, which is why marks
are session-scoped rather than operation-scoped. A presentation-timed operation
therefore publishes only its own logical stdout and stderr on the authored
schedule, while surviving `pty` marks inside its range publish separately in
