...
one. That variant has no compiled lifetime policy behind it, so it differs in
three ways: whatever holds the terminal afterwards is legitimately there and is
not a writer-cleanup violation, the `cwd` is the last one the driver reported
rather than one observed at termination, and it carries no inspection results,
because resolving an inspection plan needs the control descriptor that has just
closed. Authored inspections over such an operation therefore do not evaluate.
Its range and non-exit assertions behave exactly as for planned finalization.
...
The Envoy emits `operation_finalized` with reason `shell_ended`, carrying the
shell's wait status in that reason so a crash stays diagnosable even though the
typed outcome is shared. `operation_finalized` already means what this case
needs: no status, an outcome that can neither satisfy nor fail an authored
exit-code assertion, and a complete closed range over which the controller may
evaluate non-exit assertions. Nothing here is a failure, so nothing discards
that evidence. The Envoy keeps reading the PTY, since whatever holds the
terminal now owns it and its bytes are ordinary real terminal output, and no
further operation can be executed without a control descriptor, so the session
moves to its terminal state and finalizes at the drain. A plan with a later beat
therefore still fails, because that beat cannot run. EOF after the Envoy has
requested shutdown remains clean.
