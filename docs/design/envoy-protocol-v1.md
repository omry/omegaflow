...
A foreground job may end between `tcgetpgrp` and `kill`, so `kill` reporting
that the group no longer exists is an expected completion race, not a
failure: Awsh re-reads the foreground process group once and delivers the
signal to the group it now names, which is the selected shell's own group
when the job has ended, and still reports `signalled`; the signal then reaches
Bash at top level or inside the `INT`-bracketed completion hook exactly as the
`returned` rules above describe. A failure to open the lease, read the
...
with the active operation identifier and the real parent-observed status:
ordinarily 137 from the `SIGKILL`, but a shell that had already exited before
the signal, whose `shell_exit` Envoy had not yet accepted at selection, reports
its own status. Envoy accepts any valid status in that frame as the required
reaping evidence, never publishes it, keeps the already-selected timeout
outcome authoritative, and withholds the public timeout result until census,
reap, and drain succeed.
...
result, including a shell that exited just before selection, and a blocked
helper dropped without a reply. Lifecycle cases also cover a `cancel` or
`finalize` forwarded after Awsh wrote `completed`, answered `returned`/`none`
and discarded by Envoy, and a foreground job ending between `tcgetpgrp` and
`kill`, re-read and signalled rather than failing. Resize-lane cases cover an
