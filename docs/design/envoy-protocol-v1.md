...
stateDiagram-v2
    ...
    Running --> ShellReplaced: Envoy operation_failed shell_replaced
    ShellReplaced --> Draining: Envoy draining
    Idle --> Draining: Envoy draining after shell_exit
    Gated --> Idle: failed
...
match the active identifiers. The two shell-exit transitions are entered by the
Envoy rather than by a controller message: `Idle --> Draining` follows an
operation whose `completed` arrived from the driver's exit trap, and
`Running --> ShellReplaced` follows the EOF that `exec` produces while the shell
process is still alive. Any other transition fails closed.
...
committed absolute time. The first sender-timed event after such a span sets the
offset to its own source time minus the last absolute time that span committed,
so it lands exactly where the authored schedule ended, and every later event
publishes at its source time minus that offset until the next re-anchor.
...
with no preceding `closed` while the process itself keeps running under the same
PID, and that is what separates the two remaining cases. The Envoy is the
shell's parent, so it decides by waiting on the child rather than by probing the
PID, which a reapable zombie would answer wrongly: EOF between frames while the
wait reports the child still running is a typed shell-replacement result, and
EOF between frames with the child already terminated and no preceding `closed`
is an unexpected shell failure.
On shell replacement the Envoy closes the operation's range with no status, so
exit-code assertions fail and non-exit assertions may still run over the closed
range. It keeps reading the PTY, since the replacing program owns the terminal
now and its bytes are ordinary real terminal output. No further operation can be
executed without a control descriptor, so the session moves to its terminal
state and finalizes at the drain. EOF after the Envoy has requested shutdown
remains clean.
