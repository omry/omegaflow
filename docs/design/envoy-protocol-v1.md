...
frame-entry path to be absent, frames one brace group, and returns the complete
NUL-free UTF-8 Readline sequence in `submit`. After Readline has delivered and
Bash has parsed that complete group but before Bash executes its first command,
expanding the immutable `PS0` enters the adapter critical section with inherited
tracing suppressed. Its frame-entry path first runs the marker's
redirection-only creation in a subshell with the Bash builtin `umask 077`, then
opens the `start` helper exchange. The parent shell's exact umask is therefore
unchanged, while the absent marker is atomically created as one mode-0600
regular file in Awsh's private Bash runtime directory before the helper sends
`start`. Awsh accepts that `start` only after validating the marker's type,
owner, and mode. The marker is not a telemetry report and source receives no
path contract.
...
`PROMPT_COMMAND`, the `PS0` frame-entry path, and the gate hook, then makes the
...
marker is fatal `adapter-framing` and Envoy ends the session without an
operation result.
...
| Process file-creation mask (`umask`) | Preserved with normal Bash semantics. The `PS0` frame-entry path performs its pre-`start` mode-0600 marker creation in a tracing-suppressed subshell with the Bash builtin `umask 077`; it neither reads nor changes the parent shell's mask. |
