...
For Bash, Awsh fresh-parses `SOURCE` with the fixed backend before arming
the persistent shell. Invalid source produces `rejected` with `source-invalid`;
...
`--noprofile --norc -i`. Neither path honors `AWSH_BASH`. Before launching
...
required asset is a fatal shell-launch failure; Envoy starts neither Awsh nor
Bash and exits nonzero. Neither bootstrap shell nor Bash can therefore open
...
The Bash bootstrap makes `PS0`, `PS1`, `PS2`, `PROMPT_COMMAND`, and the gate
hook readonly. Fresh-Bash syntax preflight rejects malformed source and syntax
that depends on persistent parser state such as a previously enabled `extglob`.
The Readline bracketed-paste-begin sequence is reserved. A redirection-only
marker at the start of every framed brace group detects its mutation: if Bash
returns to a prompt without entering that marker, Awsh reports fatal
...
validation; `submit`/`started`/`started_ack` ordering; the five-second
...
syntax rejection, persistent cwd/environment/arrays/functions/aliases/
