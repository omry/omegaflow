...
whatever holds the terminal afterwards is legitimately there and is not a
writer-cleanup violation, the `cwd` is the last one the driver reported rather
than one observed at termination, and it carries no inspection results, because
resolving an inspection plan needs the control descriptor that has just closed.
Authored inspections over such an operation therefore do not evaluate. Its range
and non-exit assertions behave exactly as for planned finalization.
...
`shell_wait_status` naming that operation.
The reason stays a stable discriminator a strict encoder and the golden fixtures
can match, while a crash remains diagnosable even though the typed outcome is
shared. `operation_finalized` already means what this case
