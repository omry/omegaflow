...
accepted a `cancel` or `finalize` for the operation, the controller has left
`Running`, `Gated`, or `Continuing` for `Cancelling` or `Finalizing`, and those
states have no gate transition; because the state machine fails closed on an
unspecified transition, Envoy publishes no gate event from that point on and
instead discards every gate result it reads for the operation, whether that
result is `gate_ready`, `gate_continued`, or `gate_interrupt`. It still writes
`gate_interrupt_ack` for a discarded `gate_interrupt`, because Awsh holds the
operation in phase `gated` until that acknowledgement arrives, and it writes
nothing for a discarded `gate_ready` or `gate_continued`, which Awsh does not
wait on. The lifecycle result resolves the gate in every one of those cases, so
an ordinary deadline race at gate entry or release cannot turn cancellation or
finalization into a fatal session failure. A `continue` naming any other gate,
or arriving after the acknowledgement, is a protocol failure.
...
| `gate_ready` or `gate_continued` read after a lifecycle request was accepted | lifecycle request | discard it; emit no public gate event and write nothing; the lifecycle result resolves the gate |
