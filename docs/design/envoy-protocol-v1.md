...
a crossed cancel or finalize as `settled`, and withholds `completed` until the
...
After an Awsh result — which for ordinary completion is withheld through the
post-source `prompt_ready` helper close and observed `READLINE_ACTIVE`
transition — Envoy performs mandatory descendant cleanup, drains the PTY master
...
ordering, post-source `completed` withheld until the matching prompt helper has
closed and `READLINE_ACTIVE` is observed, no prompt helper present when Envoy
begins operation cleanup, an output-empty primary-prompt hook with no real prompt
