...
@pytest.mark.parametrize(
    ...
    (
        (
            "reploy.workload.environment.mounts.omegaflow_run.writable",
            False,
            "protected writable",
        ),
        (
            "reploy.workload.environment.mounts.omegaflow_run.update_policy",
            "unmanaged",
            "protected writable",
        ),
        (
            "reploy.workload.docker.mounts.omegaflow_run.mode",
            "bind",
            "protected ephemeral",
        ),
        (
            "reploy.workload.docker.mounts.omegaflow_run.source",
            "/tmp/persistent-run",
            "protected ephemeral",
        ),
    ),
)
def test_reserved_run_mount_must_remain_a_fresh_writable_tmpfs(
    tmp_path: Path,
    path: str,
    value: object,
    message: str,
