...
def render_session_script(spec: dict[str, Any]) -> str:
    ...
    lines = [
        "#!/usr/bin/env bash",
        # Bash 3.2 treats expansions of empty arrays as unbound under `set -u`.
        # Recording sessions use empty arrays for optional progress and cleanup
        # state, so retain fail-fast and pipe handling without nounset.
        "set -eo pipefail",
    ...
    lines.extend(
        [
            ...
            "  run_script_cleanups",
            "  cleanup_status=$?",
            '  if [[ -n "${recording_user_shell_pid:-}" ]]; then',
            "    local shell_wait_index",
            "    for ((shell_wait_index = 0; shell_wait_index < 20; shell_wait_index += 1)); do",
            '      [[ -f "${recording_user_shell_dead:-}" ]] && break',
            "      sleep 0.05",
            "    done",
            '    if [[ -f "${recording_user_shell_pgid:-}" ]]; then',
            "      local user_shell_pgid",
            '      IFS= read -r user_shell_pgid <"$recording_user_shell_pgid" || user_shell_pgid=""',
            '      [[ -n "$user_shell_pgid" ]] && kill -TERM -- "-$user_shell_pgid" 2>/dev/null || true',
            "      sleep 0.1",
            '      [[ -n "$user_shell_pgid" ]] && kill -KILL -- "-$user_shell_pgid" 2>/dev/null || true',
            "    fi",
            '    wait "$recording_user_shell_pid" 2>/dev/null || true',
            "  fi",
            '  rm -f "${recording_user_shell_input:-}"',
            '  rm -f "${recording_user_shell_dead:-}"',
            '  rm -f "${recording_user_shell_pgid:-}"',
            "  local pid",
            ...
            "}",
            "trap cleanup EXIT",
            'recording_user_shell_input="$recording_run_dir/user-shell.input.pipe"',
            'recording_user_shell_dead="$recording_run_dir/user-shell.dead"',
            'recording_user_shell_pgid="$recording_run_dir/user-shell.pgid"',
            'rm -f "$recording_user_shell_input" "$recording_user_shell_dead" "$recording_user_shell_pgid"',
            'mkfifo "$recording_user_shell_input"',
            '"$recording_python" - "$recording_user_shell_input" "$recording_user_shell_dead" "$recording_user_shell_pgid" <<\'PY\' &',
            "import subprocess",
            "import sys",
            "from pathlib import Path",
            "",
            ...
            "PY",
            "recording_user_shell_pid=$!",
            'exec 9>"$recording_user_shell_input"',
            "",
            "record_failure() {",
            '  local kind="$1"',
            '  local step_id="$2"',
            ...
            '  local labels_json="$1"',
            "  progress_labels_result=()",
            '  [[ -n "$labels_json" ]] || return 0',
            "  while IFS= read -r label; do",
            '    progress_labels_result+=("$label")',
            ...
            '  mkfifo "$status_pipe"',
            '  "$recording_python" - "$status_pipe" "$status_result" "$recording_user_shell_dead" <<\'PY\' &',
            "import os",
            "import sys",
            "import time",
            "",
            ...
            "            try:",
            "                status = open(dead_path, encoding='utf-8').read().strip()",
            "            except OSError:",
            "                status = ''",
            "            if status:",
            "                break",
            ...
            '  encoded_command="$("$recording_python" - "$command_chunk" <<\'PY\'',
            "import base64",
            "import sys",
            ...
            '    printf \'eval "$(%q -c %q %q)" >>%q 2>>%q\\nprintf "%%s\\\\n" "$?" >%q\\n\' "$recording_python" "$decoder" "$encoded_command" "$stdout_target" "$stderr_target" "$status_pipe" >&9 || true',
            "  else",
            '    printf \'eval "$(%q -c %q %q)" >%q 2>%q\\nprintf "%%s\\\\n" "$?" >%q\\n\' "$recording_python" "$decoder" "$encoded_command" "$stdout_target" "$stderr_target" "$status_pipe" >&9 || true',
            "  fi",
            ...
            "  local filter_pid=$!",
            "  local status=0",
            '  run_user_shell_command "$command_chunk" "$stdout_pipe" "$stderr_pipe" overwrite "$status_pipe"',
            "  status=$?",
            "  local filter_wait_index",
            "  for ((filter_wait_index = 0; filter_wait_index < 10; filter_wait_index += 1)); do",
            '    kill -0 "$filter_pid" 2>/dev/null || break',
            "    sleep 0.01",
            "  done",
            '  if kill -0 "$filter_pid" 2>/dev/null; then',
            '    cleanup_pids+=("$filter_pid")',
            "  else",
            ...
            '  split_commands "$display_command"',
            '  display_chunks=("${split_commands_result[@]}")',
            '  split_commands "$command"',
            '  command_chunks=("${split_commands_result[@]}")',
            '  load_progress_labels "$progress_json"',
            '  progress_labels=("${progress_labels_result[@]}")',
