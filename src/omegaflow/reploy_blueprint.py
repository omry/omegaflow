...
def validate_workload_reserved_paths(config: Blueprint | DictConfig) -> None:
    """Reject application mounts that overlap OmegaFlow's reserved runtime paths."""
...
def write_reploy_blueprints(config: DictConfig, run_dir: Path) -> tuple[Path, Path]:
    ...
    validate_workload_reserved_paths(config.reploy.workload)
