...
func Run(ctx context.Context, config Config) (runErr error) {
	...
	terminalConnection, err := acceptTCP(terminalListener, config.ConnectTimeout)
	if err != nil {
		return fail(FailureConnection, "terminal-accept", err)
	}
	defer terminalConnection.Close()
	telemetryConnection, err := acceptTCP(telemetryListener, config.ConnectTimeout)
	if err != nil {
...
func acceptTCP(listener net.Listener, timeout time.Duration) (*net.TCPConn, error) {
