...
func TestEnvoyCleansDetachedBackgroundProcessGroupMember(t *testing.T) {
	...
	_, completed := session.execute(
		"background",
		"nohup sleep 30 >/dev/null 2>&1 & printf 'background-pid=%s\\n' \"$!\"",
	)
	output := session.outputThrough(completed.OutputThrough)
	match := regexp.MustCompile(`background-pid=([0-9]+)`).FindStringSubmatch(output)
	if len(match) != 2 {
		t.Fatalf("background pid missing from %q", output)
	}
