...
// an open, shape-bounded set. The map stays unexported so an importer cannot
// widen the closed set; IsOperationFailedCode is the read-only view.
...
// IsOperationFailedCode reports whether a code belongs to the closed v1
// operation_failed set.
func IsOperationFailedCode(code string) bool {
	return operationFailedCodes[code]
}
...
func validateTelemetry(message any) error {
	...
	case OperationFailed:
		if !IsOperationFailedCode(value.Code) {
