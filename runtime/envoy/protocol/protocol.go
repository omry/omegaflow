...
var telemetryFields = map[string]struct {
	...
	"execute":             fieldSet("seq", "operation_id", "source"),
...
func (decoder *StreamDecoder) Feed(data []byte) ([]any, error) {
	...
	for {
		...
		if newline < 0 {
			if len(decoder.buffer) >= MaxTelemetryFrameBytes {
				return nil, protocolError("frame-too-large", "unterminated frame")
			}
			return messages, nil
		}
		frame := append([]byte(nil), decoder.buffer[:newline+1]...)
...
func (decoder *AwshStreamDecoder) Feed(data []byte) ([]any, error) {
	...
	var messages []any
	for {
		delimiter := bytes.IndexByte(decoder.buffer, 0)
		if delimiter < 0 {
			if decoder.frameBytes+len(decoder.buffer) >= MaxAwshFrameBytes {
				return nil, protocolError("frame-too-large", "unterminated awsh frame")
			}
			return messages, nil
		}
		field := append([]byte(nil), decoder.buffer[:delimiter]...)
		decoder.buffer = decoder.buffer[delimiter+1:]
		decoder.fields = append(decoder.fields, field)
		decoder.frameBytes += len(field) + 1
		if decoder.frameBytes > MaxAwshFrameBytes {
...
func validateTelemetry(message any) error {
	...
	case Execute:
		return firstError(validateSeq(value.Seq), validateID("operation_id", value.OperationID), validateText("source", value.Source, MaxOperationSourceBytes))
	case Continue:
...
func EncodeAwshRequest(message any) ([]byte, error) {
	switch value := message.(type) {
	case AwshExecute:
		if err := firstError(validateID("operation_id", value.OperationID), validateText("source", value.Source, MaxOperationSourceBytes)); err != nil {
			return nil, err
		}
		return encodeAwsh("execute", value.OperationID, value.Source)
	case AwshContinue:
...
func DecodeAwshRequest(frame []byte) (any, error) {
	...
	case "execute":
		...
		value := AwshExecute{OperationID: fields[2], Source: fields[3]}
