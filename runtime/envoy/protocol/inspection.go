...
func validateInspectionResult(result InspectionResult) error {
	...
	if err := validateCanonicalPath(result.ResolvedPath); err != nil {
		return err
	}
	switch result.Kind {
	case InspectionFileExists:
	...
	case InspectionProduces:
		...
		case PathKindDirectory:
			// Everything this protocol produces is directory-v2; the
			// legacy directory tag identifies records retained from
			// the native runner and never travels on Envoy telemetry.
			if result.DigestAlgorithm != DigestDirectoryV2 {
				return protocolError("invalid-field", "envoy directory digests carry the directory-v2 tag")
			}
		default:
...
func DecodeInspectionSpecs(data []byte) ([]InspectionSpec, error) {
	...
	if specs == nil {
		return nil, protocolError("invalid-field", "inspections must be an array, never null")
	}
	if err := validateInspectionSpecs(specs, ObservationExclusive); err != nil {
		return nil, err
	}
...
func DecodeResolvedInspections(data []byte) ([]ResolvedInspection, error) {
	...
	if resolved == nil {
		return nil, protocolError("invalid-field", "resolved inspections must be an array, never null")
	}
	if err := validateResolvedInspections(resolved); err != nil {
		return nil, err
	}
...
func (resolver PathResolver) expandHome(value string) string {
	...
	if expanded == "" {
		// A bare ~ whose home is the root directory stays absolute.
		return "/"
	}
	return expanded
}

// validateCanonicalPath requires the absolute canonical form the Envoy
// reports after existence is established: no empty, dot, or dot-dot
// components. The private resolved plan is lexical pre-canonicalization
// evidence and is deliberately not held to this rule.
func validateCanonicalPath(value string) error {
	...
	for _, segment := range segments {
		if segment == "" || segment == "." || segment == ".." {
			return protocolError("invalid-field", "resolved_path must be the absolute canonical path")
		}
	}
	return nil
}
