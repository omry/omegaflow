...
func validateAwshExecute(value AwshExecute) error {
	...
	if value.Observation != ObservationExclusive {
		return protocolError("invalid-field", "split execution requires exclusive observation")
	}
...
// ValidateTerminalSubmission checks the paste framing, bounds, byte safety,
// and complete conditional-frame grammar of one terminal submission. The
// frame is accepted only when it is byte-identical to the capsule
// BuildTerminalSubmission generates from the exact arguments the frame
// carries, so literal condition arguments and byte-identical authored
// branches and redirections are required rather than assumed. Envoy still
// does not reinterpret the authored source inside an accepted capsule.
func ValidateTerminalSubmission(submission []byte) error {
	...
	source, histexpand, status, stdoutFIFO, stderrFIFO, err := parseConditionalFrame(interior)
	if err != nil {
		return err
	}
	canonical, err := BuildTerminalSubmission(source, histexpand, status, stdoutFIFO, stderrFIFO)
	if err != nil {
		return err
	}
	if !bytes.Equal(canonical, submission) {
		return protocolError("adapter-framing", "terminal submission is not the canonical generated capsule")
	}
	return nil
}

// parseConditionalFrame checks the generated conditional-frame grammar and
// recovers its exact arguments. The two authored branches are byte-identical and the
// generated redirection text carries no newline, so a canonical frame has
// exactly one parse; every other byte sequence either fails here or fails the
// canonical comparison in ValidateTerminalSubmission.
func parseConditionalFrame(interior []byte) (source, histexpand string, status int, stdoutFIFO, stderrFIFO string, err error) {
	framingError := func(detail string) (string, string, int, string, string, error) {
		return "", "", 0, "", "", protocolError("adapter-framing", detail)
	}
	condition, body, found := bytes.Cut(interior, []byte("; then\n"))
	if !found {
		return framingError("terminal submission must carry the generated conditional frame")
	}
	arguments, found := bytes.CutPrefix(condition, []byte("if "+InputStateFunctionName+" "))
	if !found {
		return framingError("terminal submission must carry the generated conditional frame")
	}
	parts := bytes.Split(arguments, []byte(" "))
	if len(parts) != 2 {
		return framingError("the frame condition takes exactly the histexpand and status arguments")
	}
	histexpand = string(parts[0])
	status, err = strconv.Atoi(string(parts[1]))
	if err != nil {
		return framingError("status must be one decimal byte value")
	}
	branches, found := bytes.CutSuffix(body, []byte("fi"))
	if !found {
		return framingError("terminal submission must carry the generated conditional frame")
	}
	separator := []byte("else\n")
	if len(branches) < len(separator) || (len(branches)-len(separator))%2 != 0 {
		return framingError("the frame carries two byte-identical authored branches")
	}
	size := (len(branches) - len(separator)) / 2
	branch := branches[:size]
	if !bytes.Equal(branches[size:size+len(separator)], separator) || !bytes.Equal(branch, branches[size+len(separator):]) {
		return framingError("the frame carries two byte-identical authored branches")
	}
	inner, found := bytes.CutPrefix(branch, []byte("    { "))
	if !found {
		return framingError("each authored branch opens the generated brace group")
	}
	inner, found = bytes.CutSuffix(inner, []byte("\n"))
	if !found {
		return framingError("each authored branch ends with one LF")
	}
	cut := bytes.LastIndexByte(inner, '\n')
	if cut < 0 {
		return framingError("each authored branch closes the generated brace group")
	}
	tail, found := bytes.CutPrefix(inner[cut+1:], []byte("    }"))
	if !found {
		return framingError("each authored branch closes the generated brace group")
	}
	source = string(inner[:cut])
	if len(tail) > 0 {
		redirection, found := bytes.CutPrefix(tail, []byte(" >"))
		if !found {
			return framingError("split redirections use the generated stdout and stderr form")
		}
		stdout, stderr, found := bytes.Cut(redirection, []byte(" 2>"))
		if !found {
			return framingError("split redirections use the generated stdout and stderr form")
		}
		stdoutFIFO, stderrFIFO = string(stdout), string(stderr)
	}
	return source, histexpand, status, stdoutFIFO, stderrFIFO, nil
}
