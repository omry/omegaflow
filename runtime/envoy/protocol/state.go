...
type SessionState struct {
	...
	// gateReleased records that an accepted cancel or finalize committed
	// the open gate's outcome: its helper is released by the private
	// disposition, so the gate is no longer unresolved evidence, while
	// the gate id stays matchable for a crossed interrupt proposal.
	gateReleased bool
	...
	// lastCWD is the most recently reported working directory — from
	// ready, or from the latest terminal operation result. A pre-start
	// terminal event repeats it, since the operation itself exchanged
...
func (state *SessionState) AcceptController(message any) (err error) {
	...
	case Continue:
		...
		if value.OperationID == state.operationID && state.interruptedGates[value.GateID] {
			...
			// satisfied by that event and is discarded here — but its
			// terminal bytes were still written, so the session
			// watermark advances before the discard.
			return state.advanceWatermark(value.InputThrough)
		}
	...
	case Cancel:
		...
		if phaseIn(state.Phase(), PhaseGated, PhaseContinuing) && state.currentGate != "" {
...
func (state *SessionState) acceptCompleted(value OperationCompleted) error {
	...
	if state.currentGate != "" && !state.gateReleased {
	...
	if shellEnded {
		// No cwd can be observed after the shell is gone: the result
		// carries the last one the adapter reported.
		if value.CWD != state.lastCWD {
			return protocolError("invalid-field", "a shell-ended result repeats the last adapter-reported cwd")
		}
	} else {
		state.lastCWD = value.CWD
	}
...
func (state *SessionState) acceptCancelled(value OperationCancelled) error {
	...
	if state.operationStarted {
		...
		if value.CWD != state.lastCWD {
			return protocolError("invalid-field", "a pre-start cancellation repeats the most recently reported cwd")
		}
...
func (state *SessionState) acceptFailed(value OperationFailed) error {
	...
	case "source-invalid", "source-unsupported", "input-barrier-timeout":
		...
		if shellEnded {
			return protocolError("invalid-field", "a pre-start rejection leaves the shell available")
		}
	case "cancel-timeout":
		if phase != PhaseCancelling || !shellEnded {
			return state.outOfState(value)
		}
		// The timed-out operation reported nothing new: the result
		// repeats the last cwd the adapter reported before it.
		if value.CWD != state.lastCWD {
			return protocolError("invalid-field", "a lifecycle timeout repeats the last adapter-reported cwd")
		}
	...
	case "shell-ended-unresolved":
		...
		if len(state.inspections) == 0 && (state.currentGate == "" || state.gateReleased) {
	...
	default:
		...
		// lifecycle request is live. Inspection runs only while the
		// persistent shell survives; a shell exit with declared
		// inspections is shell-ended-unresolved instead.
		if !phaseIn(phase, PhaseRunning, PhaseCancelling, PhaseFinalizing) {
			return state.outOfState(value)
		}
		if shellEnded {
			return protocolError("invalid-field", "an inspection failure reports a surviving shell")
		}
		if len(state.inspections) == 0 {
			return protocolError("out-of-state", "inspection failure codes require declared inspections")
		}
		// Inspection resolves only after the authored source returned;
		// an operation still blocked in a gate never reached it. A gate
		// released by an accepted finalize is resolved — its helper
		// returned with cancel and the source resumed to completion —
		// while cancellation skips inspection after cleanup, so a
		// cancel-held gate never reaches an inspection worker.
		if state.currentGate != "" && !(state.gateReleased && state.finalizePending) {
			return protocolError("out-of-state", "inspection failure codes require a resolved gate")
		}
	}
	if !state.operationStarted {
		...
		if value.CWD != state.lastCWD {
			return protocolError("invalid-field", "a pre-start failure repeats the most recently reported cwd")
		}
