...
func TestSessionStateGateAndOrderedShutdown(t *testing.T) {
	...
	steps := []struct {
		...
		{true, Execute{Seq: 3, OperationID: "op-1", Source: "printf ok"}},
...
func TestSessionStateCancellationAndFailure(t *testing.T) {
	state := readyState(t)
	if err := state.AcceptController(Execute{Seq: 2, OperationID: "op-1", Source: "sleep 30"}); err != nil {
		t.Fatal(err)
	}
	...
	state = readyState(t)
	if err := state.AcceptController(Execute{Seq: 2, OperationID: "op-1", Source: "true"}); err != nil {
		t.Fatal(err)
	}
...
func TestSessionStateRejectsInvalidTransitions(t *testing.T) {
	state := NewSessionState()
	requireCode(t, "out-of-state", state.AcceptController(Execute{Seq: 1, OperationID: "op-1", Source: "true"}))
