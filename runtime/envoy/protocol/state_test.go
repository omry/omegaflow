...
func TestInspectionFailureRequiresAResolvedGate(t *testing.T) {
	driver := newSessionDriver(t)
	driver.controller(Execute{
		OperationID:     "op-1",
		Source:          "./serve",
	...
	driver.controller(Cancel{OperationID: "op-1", Reason: "deadline"})
	driver.mark(0, 30)
	// The gate is still unresolved: source never returned, so no
	// inspection ran; the typed cancellation is the legal outcome.
	err := driver.tryEnvoy(OperationFailed{OperationID: "op-1", Code: "inspection-read", Message: "m", CWD: "/w", OutputStart: 0, OutputThrough: 0})
	mustCode(t, err, "out-of-state")
	driver.envoy(OperationCancelled{OperationID: "op-1", Status: intPointer(130), CWD: "/w", Reason: "deadline", OutputStart: 0, OutputThrough: 0})
}

func TestLifecycleTimeoutsRepeatTheRetainedCWD(t *testing.T) {
	driver := newSessionDriver(t)
	driver.startRealtime("op-1", 10)
	driver.controller(Cancel{OperationID: "op-1", Reason: "deadline"})
	driver.mark(0, 20)
	err := driver.tryEnvoy(OperationFailed{OperationID: "op-1", Code: "cancel-timeout", Message: "m", CWD: "/moved", OutputStart: 0, OutputThrough: 0, ShellEnded: boolPointer(true)})
	mustCode(t, err, "invalid-field")
	driver.envoy(OperationFailed{OperationID: "op-1", Code: "cancel-timeout", Message: "m", CWD: "/w", OutputStart: 0, OutputThrough: 0, ShellEnded: boolPointer(true)})
}

func TestGatedFinalizationReachesInspection(t *testing.T) {
	// Recording end finalizes a gated operation that declared
	// inspections: the private gate-cancelled disposition releases the
	// helper, source resumes and returns, and the inspection worker may
	// legitimately fail.
	driver := newSessionDriver(t)
	driver.controller(Execute{
		OperationID:     "op-1",
	...
	driver.controller(Finalize{OperationID: "op-1", Reason: "recording-end"})
	driver.mark(0, 30)
	driver.envoy(OperationFailed{OperationID: "op-1", Code: "inspection-read", Message: "read failed after gated finalization", CWD: "/w", OutputStart: 0, OutputThrough: 0})
	if driver.state.Phase() != PhaseIdle {
		t.Fatalf("expected idle, got %s", driver.state.Phase())
	}
}

func TestShellEndedResultsRepeatTheRetainedCWD(t *testing.T) {
	driver := newSessionDriver(t)
	driver.startRealtime("op-1", 10)
	driver.mark(0, 20)
	err := driver.tryEnvoy(OperationCompleted{OperationID: "op-1", Status: 7, CWD: "/moved", OutputStart: 0, OutputThrough: 0, InspectionResults: []InspectionResult{}, ShellEnded: boolPointer(true)})
	mustCode(t, err, "invalid-field")
	driver.envoy(OperationCompleted{OperationID: "op-1", Status: 7, CWD: "/w", OutputStart: 0, OutputThrough: 0, InspectionResults: []InspectionResult{}, ShellEnded: boolPointer(true)})
}
