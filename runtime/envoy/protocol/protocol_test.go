...
func TestClosedFailureCodeSetIsImmutable(t *testing.T) {
	if !IsOperationFailedCode("cancel-timeout") || IsOperationFailedCode("invented") {
		t.Fatal("the closed set answers only its twelve members")
	}
}
