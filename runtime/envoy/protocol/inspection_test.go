...
func TestEnvoyDirectoryDigestsCarryTheV2Tag(t *testing.T) {
	native := InspectionResult{
		InspectionID: "inspection-1", Kind: InspectionProduces, ResolvedPath: "/w/build",
		PathKind: PathKindDirectory, ProducerID: "p", OutputID: "o",
		SHA256: FileDigest(nil), DigestAlgorithm: DigestDirectoryNative,
	}
	mustCode(t, validateInspectionResult(native), "invalid-field")
	native.DigestAlgorithm = DigestDirectoryV2
	if err := validateInspectionResult(native); err != nil {
		t.Fatalf("directory-v2 tag: %v", err)
	}
}

func TestInspectionDecodersResetReusedReceivers(t *testing.T) {
	// A decoded value is determined solely by the decoded bytes: a receiver
	// reused after a produces entry must not retain its optional fields.
	var spec InspectionSpec
	if err := json.Unmarshal([]byte(`{"inspection_id":"inspection-1","kind":"produces","path":"out.txt","producer_id":"p","output_id":"o"}`), &spec); err != nil {
		t.Fatalf("produces spec: %v", err)
	}
	if err := json.Unmarshal([]byte(`{"inspection_id":"inspection-2","kind":"file_exists","path":"out.txt"}`), &spec); err != nil {
		t.Fatalf("reused spec receiver: %v", err)
	}
	if spec.ProducerID != "" || spec.OutputID != "" {
		t.Fatalf("stale spec producer fields: %+v", spec)
	}

	var result InspectionResult
	if err := json.Unmarshal([]byte(`{"inspection_id":"inspection-1","kind":"produces","resolved_path":"/w/build","path_kind":"directory","producer_id":"p","output_id":"o","sha256":"`+FileDigest(nil)+`","digest_algorithm":"`+DigestDirectoryV2+`"}`), &result); err != nil {
		t.Fatalf("produces result: %v", err)
	}
	if err := json.Unmarshal([]byte(`{"inspection_id":"inspection-2","kind":"file_exists","resolved_path":"/w/f","path_kind":"file"}`), &result); err != nil {
		t.Fatalf("reused result receiver: %v", err)
	}
	if result.ProducerID != "" || result.OutputID != "" || result.SHA256 != "" || result.DigestAlgorithm != "" {
		t.Fatalf("stale result producer fields: %+v", result)
	}

	var resolved ResolvedInspection
	if err := json.Unmarshal([]byte(`{"inspection_id":"inspection-1","kind":"produces","resolved_path":"/w/build","producer_id":"p","output_id":"o"}`), &resolved); err != nil {
		t.Fatalf("produces resolved: %v", err)
	}
	if err := json.Unmarshal([]byte(`{"inspection_id":"inspection-2","kind":"file_exists","resolved_path":"/w/f"}`), &resolved); err != nil {
		t.Fatalf("reused resolved receiver: %v", err)
	}
	if resolved.ProducerID != "" || resolved.OutputID != "" {
		t.Fatalf("stale resolved producer fields: %+v", resolved)
	}
}
